from src.socmint.report_builder_events_v27_5 import (
...
def test_v27_5_create_revise_and_generate_package(tmp_path, monkeypatch):
    ...
    sections = [
        {"section_type": "saved_view", "title": "Findings", "source_id": "view-1"},
        {"section_type": "text", "title": "Notes", "text": "Analyst summary"},
    ]
    created = create_report_definition(
        name="Case Report",
        owner="alice",
        description="report",
        visibility="private",
    ...
    revised = revise_report_definition(
        created["report_id"],
        actor="alice",
        name="Case Report v2",
        description="updated",
    ...
    monkeypatch.setattr(
        exports,
        "find_report",
        lambda report_id, user: {**revised, "report_status": "active"},
    )
    resolved = [
        {
            "status": "ready",
            "source": {"saved_view_id": "view-1"},
            "summary": {
                "result_count": 1,
...
def test_v27_5_requires_valid_sections_and_confirmation(tmp_path):
    ...
    result = create_report_definition(
        ...
        description="",
        visibility="private",
        sections=[{"section_type": "text", "title": "T", "text": "x"}],
        export_formats=["json"],
        confirmed=False,
    )
    assert result["status"] == "blocked"
