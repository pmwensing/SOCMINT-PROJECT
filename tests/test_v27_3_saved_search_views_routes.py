...
def _app(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "SOCMINT_SECRET_KEY", "v27-3-route-test-secret-key-with-more-than-32-characters"
    )
    app = create_app()
...
def test_v27_3_routes_require_login_and_dispatch(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        routes,
        "create_view",
        lambda **kwargs: {"status": "saved_view_created", "saved_view_id": "view-1"},
    )
    monkeypatch.setattr(
        routes,
        "revise_view",
        lambda *args, **kwargs: {
            ...
            "saved_view_id": "view-2",
        },
    )
    monkeypatch.setattr(
        routes,
        "deactivate_view",
        lambda *args, **kwargs: {"status": "saved_view_deactivated"},
    )
    monkeypatch.setattr(
        routes,
        "run_saved_view",
    ...
    revised = client.post(
        "/api/v1/global-search/saved-views/view-1/revise",
        json={
            "name": "Preset 2",
            "query": "alpha",
            "filters": {},
    ...
    deactivated = client.post(
        "/api/v1/global-search/saved-views/view-2/deactivate",
        json={"reason": "obsolete", "confirmed": True},
        headers=headers,
    )
...
def test_v27_3_release_note_and_no_migration():
    note = Path("release/V27_3_SAVED_VIEWS_SEARCH_PRESETS.md").read_text(
        encoding="utf-8"
    )
    for phrase in (
        "Saved Views and Search Presets",
        "named query and filter definitions",
        "immutable revisions",
        "private and shared visibility",
        "duplicate-safe naming",
        "current case access scope",
        "saved views do not grant access",
        "append-only",
        "no migration",
    ):
    ...
    migrations = [
        ...
        if directory.exists()
        for path in directory.rglob("*v27_3*")
    ]
    assert migrations == []
