...
def signup_with_invite(client, invite_code, username='operator', password='StrongPass123!'):
    response = client.get('/signup')
    token = csrf_token(response)
    return client.post('/signup', data={
        'username': username,
...
def test_signup_requires_invite_code(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv('SOCMINT_ALLOW_SIGNUP', 'true')
    monkeypatch.setenv('SOCMINT_AUTO_CREATE_DB', 'true')
    monkeypatch.setenv('SOCMINT_SIGNUP_INVITE_CODE', 'invite-123')
    monkeypatch.setenv('SOCMINT_DATA_DIR', str(tmp_path))
    ...
    with app.test_client() as client:
        ...
        response = signup_with_invite(client, 'invite-123')
        assert response.status_code == 302
...
def test_csrf_required_for_login_post(app):
    with app.test_client() as client:
        response = client.post('/login', data={'username': 'operator', 'password': 'StrongPass123!'})
...
def test_healthz_only_allows_localhost(app):
    with app.test_client() as client:
        ...
        assert client.get('/healthz', environ_base={'REMOTE_ADDR': '10.0.0.5'}).status_code == 404


def test_admin_can_export_and_delete_dossier(app):
    db.create_user('admin', 'StrongAdmin123!', is_admin=True)
    db.save_dossier({'target': 'operator_1', 'type': 'username', 'data': {'sherlock': {'found': True}}})
    ...
    with app.test_client() as client:
        ...
        response = client.post(f'/target/{target_id}/delete', data={'csrf_token': token})
        assert response.status_code == 302
