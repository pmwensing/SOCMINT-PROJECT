...
def _workspace():
    return {
        ...
        "delivery_recovery_state": {"delivery_failed": False, "delivery_succeeded": True, "acknowledgement_received": True, "next_action": "monitor_delivery_recovery"},
        "retention_policies": [],
...
def test_v23_4_route_and_ui(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(routes, "generate_case_archive_package", lambda *a, **k: {
    ...
    assert b"does not change any closure, retention, dossier, release, delivery, or audit source event" in ui.data
...
def test_v23_4_release_note_client_and_no_migration():
    ...
    assert "closure, retention, dossier, release, delivery, and audit references" in note
