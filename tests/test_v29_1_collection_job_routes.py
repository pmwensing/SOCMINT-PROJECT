...
def _app(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "SOCMINT_SECRET_KEY", "v29-1-route-test-secret-key-with-more-than-32-characters"
    )
    app = create_app()
...
def test_v29_1_routes_require_admin_and_dispatch(tmp_path, monkeypatch):
    ...
    payload = {
        ...
        "collection_job_history": [],
        "collection_job_event_count": 0,
        "append_only": True,
    ...
    transitioned = client.post(
        "/api/v1/collection-operations/jobs/collection-job-1/transition",
        json={
            "to_state": "authorized",
    ...
    denied = client.post(
        ...
        json={
            "to_state": "authorized",
            "authorization_binding": {"decision": "deny"},
            "reason": "authorize",
            "confirmed": True,
        },
        headers=headers,
    ...
    assert (
        denied.get_json()["blockers"][0]["key"]
        == "allowing_collection_policy_evaluation_required"
    )


def test_v29_1_release_note_and_no_migration():
    note = Path("release/V29_1_COLLECTION_JOB_CONTRACT_STATE_MACHINE.md").read_text(
        encoding="utf-8"
    )
    for phrase in (
        "Collection Job Contract and State Machine",
        "drafted",
        "authorized",
        "queued",
        "running",
    ...
    migrations = [
        ...
        if directory.exists()
        for path in directory.rglob("*v29_1*")
    ]
    assert migrations == []
