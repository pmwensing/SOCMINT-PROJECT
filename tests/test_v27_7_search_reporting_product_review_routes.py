...
def _app(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "SOCMINT_SECRET_KEY", "v27-7-route-test-secret-key-with-more-than-32-characters"
    )
    app = create_app()
...
def _payload(ready=True):
    return {
        ...
        "duplicate_routes": [],
        "migration_artifacts": [],
        "journey": [{"step": "global_search", "route": "/global-search"}],
        "journey_step_count": 1,
        "blocker_count": 0 if ready else 1,
        "blockers": [] if ready else [{"key": "missing_v27_route"}],
...
def test_v27_7_routes_require_login_and_render_ready_checkpoint(tmp_path, monkeypatch):
    ...
    assert (
        client.get("/api/v1/global-search/product-review-checkpoint").status_code == 401
    )
    ...
    for phrase in (
        b"Search and Reporting Product Review",
        b"Checkpoint Status",
        b"Product Journey",
        b"ready_for_browser_e2e",
    ):
...
def test_v27_7_blocked_checkpoint_returns_503_and_release_contract(
    ...
    assert (
        client.get("/api/v1/global-search/product-review-checkpoint").status_code == 503
    )
    note = Path("release/V27_7_PRODUCT_REVIEW_BROWSER_E2E_CHECKPOINT.md").read_text(
        encoding="utf-8"
    )
    for phrase in (
        "Product Review and Browser E2E Checkpoint",
        "complete v27 journey",
        "global search",
        "advanced filters",
        "saved views",
        "watchlists",
    ...
    migrations = [
        ...
        if directory.exists()
        for path in directory.rglob("*v27_7*")
    ]
    assert migrations == []
