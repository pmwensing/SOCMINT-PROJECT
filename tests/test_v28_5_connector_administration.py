from src.socmint.connector_administration_events_v28_5 import (
    register_connector,
    revise_connector,
    set_connector_enabled,
    update_auth_readiness,
)
from src.socmint.connector_administration_workspace_v28_5 import (
    build_connector_administration_workspace,
)
...
def test_v28_5_connector_lifecycle_health_and_history(tmp_path, monkeypatch):
    ...
    created = register_connector(
        actor="admin",
        name="Example",
        connector_type="api",
        authorization_scopes=["read:cases"],
    ...
    readiness = update_auth_readiness(
        connector_id,
        actor="admin",
        auth_readiness="configured",
        auth_expires_at="2027-01-01T00:00:00+00:00",
        reason="configured",
        confirmed=True,
    )
    assert readiness["status"] == "connector_auth_readiness_updated"
    enabled = set_connector_enabled(
        connector_id, actor="admin", enabled=True, reason="enable", confirmed=True
    )
    assert enabled["status"] == "connector_state_updated"
    revised = revise_connector(
        connector_id,
        actor="admin",
        name="Example v2",
        connector_type="api",
        authorization_scopes=["read:cases", "read:evidence"],
    ...
    monkeypatch.setattr(
        ...
        lambda: {
            "Example v2": {
                ...
                "latest_run_at": "2026-06-18T00:00:00+00:00",
                "error_count": 0,
            }
        },
    )
...
def test_v28_5_blocks_duplicate_and_unchanged_state(tmp_path):
    ...
    unchanged = set_connector_enabled(
