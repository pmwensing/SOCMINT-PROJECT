...
def test_v26_3_create_request_and_handoff(monkeypatch):
    monkeypatch.setattr(
        service, "_case_state", lambda case_id: {"case": {"case_id": case_id}}
    )
    monkeypatch.setattr(
        service, "_record", lambda *args, **kwargs: (10, "2026-06-16T15:00:00+00:00")
    )
    req = service.create_request(
        ...
        reason="Review package",
        priority="high",
        due_at="2026-06-17",
        source_records=[{"id": 1, "sha256": "a" * 64}],
        confirmed=True,
        allowed_case_ids={"case-a"},
        ip_address=None,
    )
    hand = service.create_handoff(
        "case-a",
        actor="paul",
        other="bob",
        item_type="review_task",
        reason="Take over review",
        priority="normal",
        due_at=None,
        source_records=[],
        confirmed=True,
...
def test_v26_3_transition_binds_source_and_blocks_terminal(monkeypatch):
    item = {
        ...
        "collaboration_request_sha256": "b" * 64,
        "action_record_id": 5,
        "workflow_status": "requested",
    }
    monkeypatch.setattr(
        service, "current_items", lambda case_id: {"requests": [item], "handoffs": []}
    )
    monkeypatch.setattr(
        service, "_record", lambda *args, **kwargs: (11, "2026-06-16T16:00:00+00:00")
    )
    ...
    monkeypatch.setattr(
        ...
        "current_items",
        lambda case_id: {
            "requests": [{**item, "workflow_status": "completed"}],
            "handoffs": [],
        },
    )
...
def test_v26_3_access_and_catalog_validation(monkeypatch):
    ...
    invalid = service.create_handoff(
        "case-a",
        actor="paul",
        other="alice",
        item_type="unknown",
