        ...
        "graph": {
            "nodes": [
                ...
                },
                {
                    ...
                    "label": "evidence-9",
                    "confirmed_link_ids": ["confirmed-link-1"],
                    "source_occurrences": [{"case_id": "case-bravo"}],
                },
                {
                    "node_id": "hidden",
                    "node_type": "entity",
                    "label": "other",
                    "confirmed_link_ids": ["confirmed-link-2"],
                    "source_occurrences": [],
                },
            ],
            "edges": [
...
def test_v25_4_calculates_all_affected_operational_surfaces(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda case_ids: [
            ...
            },
            {
                ...
                "actor": "delivery",
                "occurred_at": "2026-06-16T05:30:00+00:00",
                "details": {"receipt_id": "receipt-b"},
            },
        ],
    )
    workload = {
    ...
    assert {node["node_id"] for node in result["impact"]["affected_entities"]} == {
        "entity-1",
        "evidence-1",
    }
    assert result["impact"]["review_queues"][0]["assigned_reviewer"] == "alice"
    assert result["impact"]["review_queues"][0]["supervisor_queue"].endswith(
        "assigned_reviewer=alice"
    )
    assert {
        item["current_closure_state"] for item in result["impact"]["closure_states"]
    } == {"closed", "reopened"}
...
def test_v25_4_impact_hash_is_deterministic(monkeypatch):
    monkeypatch.setattr(
        ...
        "build_case_closure_history",
        lambda case_id: {
            "current_closure_state": "open",
            "current_archive_state": "not_generated",
