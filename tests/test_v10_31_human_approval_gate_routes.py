...
from socmint.v10_31_human_approval_gate_routes import register_v10_31_human_approval_gate_routes
...
def dashboard():
    bundle = build_master_delivery_export_bundle(delivery_index(), bundle_name="Route Approval")
...
def test_approval_gate_route_accepts_registry_shape():
    ...
    response = client.post(
        ...
        json={"registry": registry(), "decision": "approved", "operator": "analyst", "notes": "Approved."},
    )
...
def test_approval_gate_route_accepts_index_shape():
    ...
    response = client.post(
        "/api/v1/v10/final-delivery/cases/case-123/approval-gate",
        json={"index": delivery_index(), "bundle_name": "Index Route Approval", "decision": "approved"},
    )
...
def test_summary_route_returns_summary_only():
    ...
    response = client.post(
        ...
        json={"registry": registry(), "decision": "needs_correction", "notes": "Fix missing item."},
    )
...
def test_no_connector_execution_function_is_called(monkeypatch):
    ...
    client = app_client()
    response = client.post("/api/v1/v10/final-delivery/cases/case-123/approval-gate", json={"index": delivery_index()})
