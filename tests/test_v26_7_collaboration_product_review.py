...
from src.socmint.collaboration_product_review_v26_7 import (
    REQUIRED_ROUTES,
    build_collaboration_product_review,
)
...
def test_v26_7_ready_product_checkpoint(tmp_path):
    ...
    result = build_collaboration_product_review(
        tmp_path, routes=[Route(rule) for rule in REQUIRED_ROUTES]
    )
...
def test_v26_7_detects_blockers(tmp_path):
    ...
    result = build_collaboration_product_review(
        tmp_path, routes=[Route("/collaboration"), Route("/collaboration")]
    )
