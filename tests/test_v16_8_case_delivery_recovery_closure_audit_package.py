...
def test_case_delivery_recovery_closure_audit_package_id_is_deterministic_and_owner_sensitive():
    ...
    assert first["audit_package"]["audit_package_id"] == second["audit_package"]["audit_package_id"]
    assert first["audit_package"]["audit_package_id"] != changed["audit_package"]["audit_package_id"]
...
def test_case_delivery_recovery_closure_audit_package_blocks_failed_closure_verification():
    ...
    tampered_verification = {"status": "blocked", "verified": False, "blockers": [{"key": "tampered", "detail": "tampered"}]}
    ...
    assert any(blocker["key"] == "closure_verification_blocked" for blocker in result["blockers"])
...
def test_case_delivery_recovery_closure_audit_package_blocks_linkage_mismatch():
    ...
    assert result["status"] == "blocked"
    assert any(blocker["key"] == "closure_receipt_mismatch" for blocker in result["blockers"])


def test_case_delivery_recovery_closure_audit_package_route_requires_login(tmp_path, monkeypatch):
...
def test_case_delivery_recovery_closure_audit_package_route_returns_package(tmp_path, monkeypatch):
    monkeypatch.setenv("SOCMINT_DATABASE_URL", f"sqlite:///{tmp_path / 'app.db'}")
    app = create_app()
    register_case_delivery_workspace_routes_v15(app)
    client = app.test_client()
...
def test_v16_8_release_note_and_changelog_are_present():
    note = Path("release/V16_8_DELIVERY_RECOVERY_CLOSURE_AUDIT_PACKAGE.md").read_text(encoding="utf-8")
