...
def _app(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "SOCMINT_SECRET_KEY", "v26-route-test-secret-key-with-more-than-32-characters"
    )
    app = create_app()
...
def test_v26_7_routes_require_login_and_render(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        routes, "build_collaboration_product_review", lambda **kwargs: _payload()
    )
    client = _app(tmp_path, monkeypatch).test_client()
    assert (
        client.get("/api/v1/collaboration/product-review-checkpoint").status_code == 401
    )
    ...
    for phrase in (
        b"Collaboration Product Review",
        b"Product Journey",
        b"Module Checks",
        b"Route Checks",
        b"Blockers",
