from src.socmint.team_organization_events_v28_3 import (
    append_team_event,
    create_team,
    revise_team,
)
from src.socmint.team_organization_workspace_v28_3 import (
    build_team_organization_workspace,
)
...
def test_v28_3_team_lifecycle_structure_and_history(tmp_path, monkeypatch):
    ...
    assert (
        append_team_event(
            ...
            organizational_scope="national",
            ownership_boundaries=["case-intake"],
            reason="scope",
            confirmed=True,
        )["status"]
        == "team_updated"
...
def test_v28_3_blocks_duplicate_team_name_and_unconfirmed_change(tmp_path):
    ...
    created = create_team(
        actor="admin", name="Alpha", description="", reason="create", confirmed=True
    )
    duplicate = create_team(
        actor="admin", name="Alpha", description="", reason="create", confirmed=True
    )
