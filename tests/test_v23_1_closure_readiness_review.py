...
def _setup(tmp_path, monkeypatch, eligible=True):
    ...
    monkeypatch.setattr(service, "build_case_closure_workspace", lambda case_id: {
        "version": "v23.0.0",
        "status": "eligible_for_closure_review" if eligible else "blocked",
        "closure_eligible": eligible,
        "archive_ready": eligible,
        "current_release_outcome": "delivered_and_acknowledged" if eligible else "delivery_failed",
        "blockers": [] if eligible else [{"key": "release_closure_readiness_required"}],
        "release_history": {
            "closure_summary": {
                "case_id": case_id,
...
def test_v23_1_blocks_ready_when_case_not_eligible_and_allows_not_ready(tmp_path, monkeypatch):
