...
from src.socmint.case_delivery_operations_reentry_envelope_v16_16 import (
...
from tests.test_v16_15_case_delivery_recovery_resume_operations_snapshot_verification import (
...
def _reentry_artifacts():
    ...
    resume_verification = verify_case_delivery_recovery_resume_operations_snapshot(
...
def test_v16_16_builds_ready_reentry_envelope():
    ...
    result = build_case_delivery_operations_reentry_envelope(
        *artifacts, reentry_operator="delivery-ops"
    )
...
def test_v16_16_reentry_envelope_id_is_deterministic_and_operator_sensitive():
    ...
    first = build_case_delivery_operations_reentry_envelope(
        *artifacts, reentry_operator="delivery-ops"
    )
    second = build_case_delivery_operations_reentry_envelope(
        *artifacts, reentry_operator="delivery-ops"
    )
    changed = build_case_delivery_operations_reentry_envelope(
        *artifacts, reentry_operator="other-ops"
    )

    assert (
        first["reentry_envelope"]["reentry_envelope_id"]
        == second["reentry_envelope"]["reentry_envelope_id"]
    )
    assert (
        first["reentry_envelope"]["reentry_envelope_id"]
        != changed["reentry_envelope"]["reentry_envelope_id"]
    )
...
def test_v16_16_blocks_failed_resume_snapshot_verification():
    ...
    assert any(
        blocker["key"] == "resume_snapshot_verification_blocked"
...
def test_v16_16_release_note_and_changelog_are_present():
    note = Path("release/V16_16_DELIVERY_OPERATIONS_REENTRY_ENVELOPE.md").read_text(
        encoding="utf-8"
    )
