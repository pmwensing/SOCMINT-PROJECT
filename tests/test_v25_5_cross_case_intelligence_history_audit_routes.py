        ...
        "history": [
            {
                ...
                "source_action": "cross_case_correlation_candidate_review",
                "source_record_id": 1,
                "source_binding_sha256": "a" * 64,
            }
        ],
        "event_count": 1,
        "event_type_counts": {"analyst_decision": 1},
        "actor_counts": {"reviewer-one": 1},
        "correlation_count": 1,
        "confirmed_link_count": 0,
        "case_count": 2,
        "source_bound_event_count": 1,
        "current_cross_case_intelligence_state": {
            "candidate_discovery": {"status": "ready"},
            "reviews": {"disposition_counts": {"accept": 1}},
            "confirmed_links": {"count": 0},
            "relationship_graph": {"status": "ready"},
            "impact_analyses": {"count": 0},
...
def test_v25_5_release_note_and_no_migration():
    note = Path("release/V25_5_CROSS_CASE_INTELLIGENCE_HISTORY_AUDIT.md").read_text(
        encoding="utf-8"
    )
    migrations = [
        p
        ...
        if d.exists()
        for p in d.rglob("*v25_5*")
    ]
    for phrase in (
        "candidate discovery",
        "analyst decisions",
        "confirmed-link registrations",
        "graph projections",
        "impact analyses",
        "one ordered history",
        "actor",
        ...
        "access scope",
        "event counts",
        "current cross-case intelligence state",
        "read-only",
    ):
