...
from socmint.v10_27_final_delivery_evidence_capsule_routes import (
    register_v10_27_final_delivery_evidence_capsule_routes,
)
...
def audit_trail():
    bundle = build_master_delivery_export_bundle(
        delivery_index(), bundle_name="Route Capsule"
    )
...
def test_no_connector_execution_function_is_called(monkeypatch):
    ...
    client = app_client()
    response = client.post(
        "/api/v1/v10/final-delivery/evidence-capsule", json={"index": delivery_index()}
    )
