...
def test_v26_1_records_append_only_assignment_with_source_binding(monkeypatch):
    ...
    monkeypatch.setattr(service, "latest_active_assignment", lambda *args, **kwargs: {
...
def test_v26_1_records_revocation_bound_to_active_assignment(monkeypatch):
    _patch_recording(monkeypatch)
    monkeypatch.setattr(service, "current_case_team", lambda case_id: [{
...
def test_v26_1_reducer_and_validation_blockers(monkeypatch):
    monkeypatch.setattr(service, "case_team_history", lambda case_id: [
        {"event_type": "assignment", "case_team_assignment_id": "a1", "case_team_event_sha256": "1" * 64, "user_identity": "alice", "role": "analyst", "recorded_at": "2026-06-16T10:00:00+00:00"},
        {"event_type": "assignment", "case_team_assignment_id": "a2", "case_team_event_sha256": "2" * 64, "supersedes_assignment_id": "a1", "user_identity": "alice", "role": "analyst", "recorded_at": "2026-06-16T11:00:00+00:00"},
        {"event_type": "revocation", "case_team_assignment_id": "a2", "case_team_event_id": "r2", "reason": "done", "recorded_by": "supervisor", "recorded_at": "2026-06-16T12:00:00+00:00"},
    ])
    team = {item["case_team_assignment_id"]: item for item in service.current_case_team("case-a")}
    assert team["a1"]["assignment_status"] == "superseded"
    assert team["a2"]["assignment_status"] == "revoked"

    denied = service.assign_case_team_role(
        "case-hidden", user_identity="alice", role="analyst", assigned_by="supervisor",
        reason="test", confirmed=True, allowed_case_ids={"case-a"},
    )
    invalid_role = service.assign_case_team_role(
        "case-a", user_identity="alice", role="administrator", assigned_by="supervisor",
        reason="test", confirmed=True, allowed_case_ids={"case-a"},
    )
