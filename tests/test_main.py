...
def test_retrieve_path_prints_existing_dossier(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        socmint_main, "get_dossier", lambda target: {"target": target, "data": {}}
    )
...
def test_generate_secrets_command_outputs_safe_env(monkeypatch, capsys):
    monkeypatch.setattr(sys, "argv", ["socmint", "generate-secrets"])

    socmint_main.main()

    output = capsys.readouterr().out
    assert "SOCMINT_SECRET_KEY=" in output
    assert "SOCMINT_ADMIN_PASSWORD=" in output
    assert "replace-with" not in output


def test_process_jobs_command_processes_queued_job(monkeypatch, tmp_path, capsys):
    from src.socmint import database as db

    monkeypatch.setenv("DATABASE_URL", f"sqlite:///{tmp_path / 'socmint-jobs.db'}")
    monkeypatch.setenv("SOCMINT_DATA_DIR", str(tmp_path))
    db.configure_database(f"sqlite:///{tmp_path / 'socmint-jobs.db'}")
    db.create_scan_job("operator_job", "username", tools=set(), enrich=False)

    monkeypatch.setattr(
        "src.socmint.jobs.build_dossier",
        lambda target, target_type, enabled_tools=None: {
            "target": target,
            "type": target_type,
            "data": {"job": "ok"},
        },
    )
    monkeypatch.setattr(sys, "argv", ["socmint", "process-jobs", "--max-jobs", "1"])

    socmint_main.main()

    output_text = capsys.readouterr().out
    output = json.loads(output_text[output_text.find("[") :])
    assert output[0]["status"] == "completed"
    assert db.get_dossier("operator_job")["data"]["job"] == "ok"
...
def test_init_admin_command_creates_admin(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        sys, "argv", ["socmint", "init-admin", "admin", "StrongAdmin123!"]
    )
