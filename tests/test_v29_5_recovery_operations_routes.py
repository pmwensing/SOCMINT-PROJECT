...
def _app(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "SOCMINT_SECRET_KEY", "v29-5-route-test-secret-key-with-more-than-32-characters"
    )
    app = create_app()
...
def test_v29_5_routes_require_admin_and_dispatch(tmp_path, monkeypatch):
    ...
    payload = {
        ...
        "retry_request_state_counts": {},
        "pending_retry_requests": [],
        "pending_retry_request_count": 0,
        "approved_retry_requests": [],
        "approved_retry_request_count": 0,
        "retry_window_open": [],
        "retry_window_open_count": 0,
        "retry_window_expired": [],
        "retry_window_expired_count": 0,
        "recovery_plans": [],
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            ...
            "operator_intervention_id": "intervention-1",
        },
    )
    ...
    requested = client.post(
        "/api/v1/collection-operations/jobs/job-1/retry-requests",
        json={
    ...
    decided = client.post(
        "/api/v1/collection-operations/retry-requests/retry-1/decision",
        json={"approved": True, "decision_reason": "transient", "confirmed": True},
        headers=headers,
    )
    planned = client.post(
        "/api/v1/collection-operations/jobs/job-1/recovery-plans",
        json={
            "retry_request_id": "retry-1",
            "plan_type": "retry",
            "steps": ["wait", "requeue"],
    ...
    intervened = client.post(
        "/api/v1/collection-operations/jobs/job-1/interventions",
        json={
    ...
    assert [
        ...
        decided.status_code,
        planned.status_code,
        intervened.status_code,
    ] == [200, 200, 200, 200]


def test_v29_5_release_note_and_no_migration():
    note = Path("release/V29_5_RETRY_RECOVERY_OPERATOR_INTERVENTION.md").read_text(
        encoding="utf-8"
    )
    for phrase in (
        "Retry, Recovery, and Operator Intervention",
        "retry requests",
        "retry approvals",
        "idempotency key",
        "backoff",
    ...
    migrations = [
        ...
        if directory.exists()
        for path in directory.rglob("*v29_5*")
    ]
    assert migrations == []
