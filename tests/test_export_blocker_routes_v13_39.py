...
def test_export_gate_decision_route_surfaces_verification_summary():
    ...
    response = client.get(
        "/api/v1/dossier-builder/v3/export-gate/case-route-139/subject-route-139/decision"
    )
    payload = response.get_json()
...
def test_export_blockers_ui_route_renders_operator_panel():
    ...
    response = client.get(
        "/dossier/export-blockers?case_id=case-route-139&subject_id=subject-route-139"
    )
