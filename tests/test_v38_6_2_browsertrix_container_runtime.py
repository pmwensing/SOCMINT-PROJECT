...
def execution_plan() -> dict:
    return {
        ...
        "container": {
            ...
            "process_limit": 128,
            "mounts": [{"source": "private://case/capture", "target": "/crawls"}],
        },
...
def test_runtime_requires_exact_plan_binding() -> None:
    ...
    assert result["blockers"] == [{"key": "runtime_execution_plan_binding_mismatch"}]
...
def test_runtime_requires_full_network_containment_proof() -> None:
    ...
    assert result["blockers"] == [{"key": "verified_network_containment_required"}]


def test_runtime_requires_local_digest_match() -> None:
    def mismatch(runtime: str, image_reference: str) -> RuntimeInspection:
        inspected = runtime_inspector(runtime, image_reference)
        return RuntimeInspection(**{**inspected.__dict__, "local_image_digest": "sha256:" + "d" * 64})
...
def test_failed_process_is_single_attempt_and_quarantined() -> None:
    ...
    def runner(command: list[str], timeout: int) -> ProcessOutcome:
        ...
        return ProcessOutcome(exit_code=124, stdout="", stderr="timeout", timed_out=True)
