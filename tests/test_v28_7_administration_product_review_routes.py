...
def _app(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "SOCMINT_SECRET_KEY", "v28-7-route-test-secret-key-with-more-than-32-characters"
    )
    app = create_app()
...
def test_v28_7_routes_require_login_and_render_ready_checkpoint(tmp_path, monkeypatch):
    ...
    assert (
        client.get("/api/v1/administration/product-review-checkpoint").status_code
        == 401
    )
    ...
    for phrase in (
        b"Administration Product Review",
        b"Checkpoint Status",
        b"Administration Journey",
...
def test_v28_7_blocked_checkpoint_returns_503_and_release_contract(
    tmp_path, monkeypatch
):
    from src.socmint import administration_product_review_routes_v28_7 as routes

    monkeypatch.setattr(
        routes, "build_administration_product_review", lambda **kwargs: _payload(False)
    )
    client = _app(tmp_path, monkeypatch).test_client()
    with client.session_transaction() as sess:
        sess["user"] = "admin"
    ...
    note = Path("release/V28_7_PRODUCT_REVIEW_BROWSER_E2E_CHECKPOINT.md").read_text(
        encoding="utf-8"
    )
    for phrase in (
    ...
    migrations = [
        ...
        if directory.exists()
        for path in directory.rglob("*v28_7*")
    ]
    assert migrations == []
