...
def _queue():
    return {
        ...
        "queue": [{
            ...
            "remediation_links": {"case_review": "/case-intelligence-review/case-alpha"},
        }],
    }
...
def test_v24_4_records_bound_immutable_control_events(tmp_path, monkeypatch):
    ...
    escalated = service.record_escalation(
        ...
        reason="Critical blocker exceeded thresholds.", note="Immediate review."
    )
    ...
    reassigned = service.reassign_escalation(
        "case-alpha", confirmed=True, supervisor="supervisor",
        assigned_reviewer="bob", note="Move to available reviewer."
    )
    resolved = service.resolve_escalation(
        "case-alpha", confirmed=True, supervisor="supervisor",
        resolution="Blocker remediated and follow-up scheduled.", note="Resolved."
    )
    ...
    assert acknowledged["payload"]["escalation_event_id"] == escalated["control_event_id"]
    ...
    assert [item["control"] for item in history] == [
        "escalate", "acknowledge", "reassign", "resolve"
    ]
...
def test_v24_4_enforces_confirmation_source_and_required_values(tmp_path, monkeypatch):
    _setup(tmp_path, monkeypatch)
    assert service.record_escalation(
        "case-alpha", confirmed=False, supervisor="supervisor", reason="reason"
    )["blockers"][0]["key"] == "explicit_escalation_confirmation_required"
    assert service.record_escalation(
        "case-alpha", confirmed=True, supervisor="supervisor", reason=""
    )["blockers"][0]["key"] == "escalation_reason_required"
    assert service.acknowledge_escalation(
        "case-alpha", confirmed=True, supervisor="supervisor"
    )["blockers"][0]["key"] == "escalation_required"
