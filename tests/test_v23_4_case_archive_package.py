...
def _setup(tmp_path, monkeypatch, include_assignment=True):
    ...
    monkeypatch.setattr(service, "latest_retention_assignment", lambda case_id: assignment if include_assignment else None)
    monkeypatch.setattr(service, "latest_supervisor_closure_decision", lambda case_id: {
        "closure_decision_id": "closure-decision-1",
        "closure_decision_sha256": "decision-hash",
        "decision": "close",
        "case_closed": True,
    })
    monkeypatch.setattr(service, "latest_closure_readiness_review", lambda case_id: {
        "review_id": "closure-readiness-1",
        "review_sha256": "review-hash",
        "decision": "ready",
    })
    monkeypatch.setattr(service, "_latest_export", lambda case_id: {
    ...
    monkeypatch.setattr(service, "build_release_delivery_history", lambda case_id: {
        "closure_ready": True,
        "closure_summary": {"case_id": case_id, "release_outcome": "delivered_and_acknowledged", "acknowledgement_id": "ack-1"},
...
def test_v23_4_builds_and_records_deterministic_archive(tmp_path, monkeypatch):
    ...
    generated = service.generate_case_archive_package("case-alpha", actor="archive-supervisor")
    ...
    assert first["components"]["retention"]["retention_assignment_id"] == "retention-assignment-1"
