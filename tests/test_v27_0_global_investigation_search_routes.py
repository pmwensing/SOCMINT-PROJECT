...
def _app(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "SOCMINT_SECRET_KEY", "v27-route-test-secret-key-with-more-than-32-characters"
    )
    app = create_app()
...
def test_v27_0_routes_require_login_scope_and_render(tmp_path, monkeypatch):
    ...
    assert captured == [
        (
            "entity-42",
            {"result_types": [], "allowed_case_ids": {"case-a"}, "limit": 50},
        ),
        (
            "entity-42",
            {"result_types": [], "allowed_case_ids": {"case-a"}, "limit": 50},
...
def test_v27_0_release_note_and_no_migration():
    note = Path("release/V27_0_GLOBAL_INVESTIGATION_SEARCH.md").read_text(
        encoding="utf-8"
    )
    for phrase in (
        "Global Investigation Search",
        "cases, entities, identifiers, infrastructure, evidence, findings",
        "case access scope",
        "normalized results",
        "ranking metadata",
        "direct links",
