...
def test_v25_0_routes_apply_session_case_scope_and_render(tmp_path, monkeypatch):
    ...
    assert all(
        call["allowed_case_ids"] == {"case-alpha", "case-bravo"} for call in calls
    )
    assert all(call["minimum_case_count"] == 2 for call in calls)


def test_v25_0_invalid_minimum_falls_back_and_empty_scope_is_restricted(
...
def test_v25_0_release_note_and_no_migration():
    note = Path("release/V25_0_CROSS_CASE_INTELLIGENCE_WORKSPACE.md").read_text(
        encoding="utf-8"
    )
    migrations = [
        path
    ...
    assert (
        "entities, identifiers, infrastructure, evidence, timelines, and repeated patterns"
        in note
    )
