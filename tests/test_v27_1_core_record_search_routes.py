...
def _app(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "SOCMINT_SECRET_KEY", "v27-1-route-test-secret-key-with-more-than-32-characters"
    )
    app = create_app()
...
def _payload():
    return {
        ...
        "results": [
            {
                ...
                "preview": {
                    "fields": [
                        {"field": "entity_id", "value": "entity-42", "matched": True},
                        {"field": "name", "value": "Example Person", "matched": False},
                    ],
                    "field_count": 2,
                ...
                "links": {
                    "primary": "/case-intelligence-review/case-a",
        ...
        "result_count": 1,
        "record_type_counts": {"entity": 1},
        "visible_case_ids": ["case-a"],
        "search_sha256": "b" * 64,
...
def test_v27_1_routes_require_login_scope_filters_and_render(tmp_path, monkeypatch):
    ...
    assert (
        client.get("/api/v1/global-search/core-records?q=entity-42").status_code == 401
    )
    assert client.get("/global-search/core-records?q=entity-42").status_code in {
        302,
        303,
    }
    ...
    for phrase in (
        b"Case, Entity, Evidence, and Finding Search",
        b"Facets",
        b"Field-Aware Results",
        b"Example Person",
    ):
...
def test_v27_1_release_note_and_no_migration():
    note = Path("release/V27_1_CASE_ENTITY_EVIDENCE_FINDING_SEARCH.md").read_text(
        encoding="utf-8"
    )
    for phrase in (
