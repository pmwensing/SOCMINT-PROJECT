...
def test_v22_5_routes_and_ui(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(routes, "build_delivery_receipt_state", lambda case_id: {
        "status": "tracking", "delivery_succeeded": False,
        "acknowledgement_received": False, "acknowledgement_outstanding": False,
        "next_action": "review_delivery_failure",
        "latest_delivery_receipt": None, "latest_recipient_acknowledgement": None,
    })
    monkeypatch.setattr(routes, "build_delivery_recovery_state", lambda case_id: {
        "status": "tracking",
    ...
    monkeypatch.setattr(routes, "review_failed_delivery", lambda *a, **k: {
        "status": "failed_delivery_review_recorded", "record_id": 71,
    })
    monkeypatch.setattr(routes, "request_recall", lambda *a, **k: {
        "status": "recall_requested", "record_id": 72,
    })
    monkeypatch.setattr(routes, "authorize_reissue", lambda *a, **k: {
        "status": "reissue_authorized", "record_id": 73,
    })

    client = _app(tmp_path, monkeypatch).test_client()
    assert client.get("/api/v1/dossier-release/case-alpha/delivery-recovery").status_code == 401
    ...
    assert b"original dispatch, receipt, and acknowledgement events remain unchanged" in ui.data
...
def test_v22_5_release_note_client_and_no_migration():
    note = Path("release/V22_5_FAILED_DELIVERY_RECALL_REISSUE_CONTROLS.md").read_text(encoding="utf-8")
    ...
    assert "reissue authorization tied to the original package and recipient history" in note
    assert "without altering the original dispatch, receipt, or acknowledgement events" in note
