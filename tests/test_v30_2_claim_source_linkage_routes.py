...
from src.socmint.claim_source_linkage_routes_v30_2 import (
    register_claim_source_linkage_routes_v30_2,
)
...
def test_v30_2_linkage_routes_require_admin_and_link(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.socmint.claim_source_linkage_routes_v30_2.actor_is_administrator",
        lambda actor: actor == "admin",
    )
    monkeypatch.setattr(
        "src.socmint.claim_source_linkage_routes_v30_2.claim_linkages",
        lambda claim_id: [],
    )
    monkeypatch.setattr(
    ...
    assert (
        client.get("/api/v1/analytic-review/claims/claim-1/source-linkages").status_code
        == 401
    )
