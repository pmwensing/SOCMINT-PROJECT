...
def test_v19_3_cross_case_counts_oldest_age_and_reviewers(tmp_path, monkeypatch):
    _configure(tmp_path, monkeypatch)
    first = _seed("case-one", "approve_review", "analyst-a", "2026-06-10T00:00:00+00:00")
    second = _seed("case-two", "needs_follow_up", "analyst-b", "2026-06-11T00:00:00+00:00")
    ...
    set_persistent_decision_review_state("case-two", second["decision_record_id"], "needs_follow_up", actor="reviewer-a", note="verify")
    set_persistent_decision_review_state("case-two", third["decision_record_id"], "accepted", actor="reviewer-b")

    result = build_persistent_decision_supervisor_queue(now=datetime(2026, 6, 13, tzinfo=UTC))

    assert result["schema"] == PERSISTENT_DECISION_SUPERVISOR_QUEUE_SCHEMA
    assert result["counts"] == {"accepted": 1, "needs_follow_up": 1, "reviewed": 0, "unreviewed": 1}
...
def test_v19_3_case_summaries_filters_and_links(tmp_path, monkeypatch):
    ...
    set_persistent_decision_review_state("case-one", first["decision_record_id"], "reviewed", actor="reviewer-a")
    set_persistent_decision_review_state("case-two", second["decision_record_id"], "needs_follow_up", actor="reviewer-b")
    ...
    assert summaries["case-one"]["case_workspace_href"] == "/case-intelligence-review/case-one"
    assert build_persistent_decision_supervisor_queue(case_id="case-one")["entry_count"] == 1
    assert build_persistent_decision_supervisor_queue(review_state="needs_follow_up")["entries"][0]["case_id"] == "case-two"
    assert build_persistent_decision_supervisor_queue(assigned_reviewer="reviewer-a")["entries"][0]["case_id"] == "case-one"
