...
from src.socmint.v10_32_productization_ux_routes import (
    register_v10_32_productization_ux_routes,
)
...
def test_summary_endpoint_returns_expected_fields(app_client, sample_registry):
    ...
    response = client.post(
        "/api/v1/v10/productization/cases/case-123/summary",
        json={"registry": sample_registry},
    )
...
def test_ui_endpoint_returns_expected_fields(app_client, sample_registry):
    client = app_client
    response = client.post(
        "/api/v1/v10/productization/cases/case-123/ui",
        json={"registry": sample_registry},
    )
    data = response.get_json()
