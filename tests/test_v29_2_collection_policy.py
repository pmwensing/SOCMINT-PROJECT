...
from src.socmint.collection_policy_v29_2 import (
    create_collection_policy,
    evaluate_collection_job_policy,
    revise_collection_policy,
)
from src.socmint.collection_policy_workspace_v29_2 import (
    build_collection_policy_workspace,
)
...
def test_v29_2_policy_allow_deny_revision_and_workspace(tmp_path):
    ...
    job = create_collection_job_contract(
        actor="admin",
    ...
    policy = create_collection_policy(
        ...
        permitted_source_classes=["public_web"],
        permitted_purposes=["investigation"],
        jurisdictions=["CA"],
        case_ids=["case-a"],
        entity_ids=[],
        source_ids=[],
        ...
        review_at=(datetime.now(timezone.utc) + timedelta(days=10)).isoformat(),
        reason="define",
        confirmed=True,
    )
    assert policy["status"] == "collection_policy_created"
    allowed = evaluate_collection_job_policy(
        actor="admin",
    ...
    revised = revise_collection_policy(
        policy["policy_id"],
        actor="admin",
        definition={
            "name": "Public Web v2",
            ...
            "entity_ids": [],
            "source_ids": [],
            "deny_rules": [{"field": "target_value", "value": "alice"}],
            "exclusions": [],
            "valid_from": None,
            "expires_at": None,
            "review_at": None,
    ...
    denied = evaluate_collection_job_policy(
        actor="admin",
        collection_job_id=job["collection_job_id"],
        jurisdiction="CA",
        reason="reevaluate",
...
def test_v29_2_blocks_missing_scope_and_duplicate_name(tmp_path):
    ...
    first = create_collection_policy(
        actor="admin",
        name="Policy",
        description="",
        permitted_source_classes=["public_web"],
    ...
    duplicate = create_collection_policy(
        actor="admin",
        name="Policy",
        description="",
        permitted_source_classes=["public_web"],
