...
def _base(monkeypatch):
    ...
    monkeypatch.setattr(workspace, "current_contribution_decisions", lambda: [])
...
def test_v30_0_builds_read_only_inventory(monkeypatch):
    _base(monkeypatch)
    monkeypatch.setattr(
        workspace, "current_artifacts", lambda: [{"artifact_id": "artifact-1"}]
    )
    monkeypatch.setattr(
        workspace,
        "observations",
    ...
    monkeypatch.setattr(
        ...
        lambda: [{"quality_assessment_id": "qa-1", "decision": "approved"}],
    )
    monkeypatch.setattr(
        workspace,
        "_claim_inventory",
    ...
    assert result["analytic_dossier_contribution_count"] == 0
...
def test_v30_0_surfaces_contradictions(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: [
            {
                "quality_assessment_id": "qa-pending",
                "quality_score": 70,
