...
def test_v30_4_penalizes_unresolved_conflicts_and_blocks_unlinked_claim(
    ...
    penalty = next(
        item
        for item in result["explanation"]["components"]
        if item["key"] == "unresolved_conflict_penalty"
    )
