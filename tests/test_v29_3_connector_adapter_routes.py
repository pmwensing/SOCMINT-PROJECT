...
def _app(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "SOCMINT_SECRET_KEY", "v29-3-route-test-secret-key-with-more-than-32-characters"
    )
    app = create_app()
...
def test_v29_3_routes_require_admin_and_dispatch(tmp_path, monkeypatch):
    ...
    payload = {
        ...
        "active_adapter_contract_count": 0,
        "adapter_summaries": [],
        "error_class_catalog": [],
        "conformance_evaluations": [],
        "conformance_evaluation_count": 0,
        "conformance_counts": {},
        "adapter_findings": [],
    ...
    revised = client.post(
        "/api/v1/collection-operations/adapters/adapter-1/revise",
        json={
            "definition": {"capabilities": ["lookup"], "error_classes": []},
            "reason": "revise",
            "confirmed": True,
        },
        headers=headers,
    ...
    evaluated = client.post(
        "/api/v1/collection-operations/adapters/adapter-1/evaluate",
        json={
            "observed_capabilities": ["lookup"],
...
def test_v29_3_release_note_and_no_migration():
    note = Path("release/V29_3_CONNECTOR_NORMALIZATION_ADAPTER_CONTRACT.md").read_text(
        encoding="utf-8"
    )
    for phrase in (
        "Connector Normalization and Adapter Contract",
    ...
    migrations = [
        ...
        if directory.exists()
        for path in directory.rglob("*v29_3*")
    ]
    assert migrations == []
