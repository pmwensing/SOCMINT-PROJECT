...
def _verification_artifacts():
    ...
    resume_snapshot_verification = (
        verify_case_delivery_recovery_resume_operations_snapshot(base[-1], *base[:-1])
    )
    envelope = build_case_delivery_operations_reentry_envelope(
        *base, resume_snapshot_verification, reentry_operator="delivery-ops"
    )["reentry_envelope"]
...
def test_v16_17_blocks_tampered_payload_hash_and_envelope_id():
    ...
    assert any(
        blocker["key"] == "reentry_envelope_id_mismatch"
...
def test_v16_17_blocks_not_ready_and_next_action_mismatch():
    ...
    assert result["status"] == "blocked"
    assert any(
        blocker["key"] == "not_ready_for_operations_dispatch"
        for blocker in result["blockers"]
    )
    assert any(
