...
def _citations():
    return {
        ...
        "sections": [{
            "section_id": "key_findings", "title": "Key Findings", "position": 1,
...
def _review(review_id="review-1"):
    return {
        ...
        "review_id": review_id, "review_sha256": "a" * 64,
    }
...
def _setup(tmp_path, monkeypatch, approval=None, review_id="review-1"):
    ...
    monkeypatch.setattr(service, "build_dossier_citation_mapping", lambda *a, **k: _citations())
    monkeypatch.setattr(service, "build_dossier_quality_review", lambda *a, **k: _review(review_id))


def test_v21_6_refuses_missing_returned_and_held(tmp_path, monkeypatch):
    _setup(tmp_path, monkeypatch, None)
    assert service.build_final_export_package("case-alpha", subject_id=42)["blockers"][0]["key"] == "supervisor_approval_required"
    for status in ("returned", "held"):
        monkeypatch.setattr(service, "_latest_decision", lambda case_id, s=status: _approval(s))
...
def test_v21_6_deterministic_package_and_generation(tmp_path, monkeypatch):
    ...
    assert one["dossier_content"]["sections"][0]["citation_ready_narrative"].endswith("[C1]")
    assert one["approval_record"]["approval_record_id"] == 11
    assert set(one["integrity"]) == {
        "content_sha256", "dossier_sha256", "citation_catalog_sha256",
        "source_manifest_sha256", "approval_record_sha256", "quality_review_sha256"
    }
