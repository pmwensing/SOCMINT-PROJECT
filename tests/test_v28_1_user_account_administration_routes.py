...
def _app(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "SOCMINT_SECRET_KEY", "v28-1-route-test-secret-key-with-more-than-32-characters"
    )
    app = create_app()
...
def test_v28_1_routes_require_admin_csrf_and_dispatch(tmp_path, monkeypatch):
    ...
    payload = {
        ...
        "roles": ["viewer", "analyst", "reviewer", "supervisor", "admin"],
        "account_history": [],
    ...
    monkeypatch.setattr(
        routes,
        "provision_user",
        lambda **kwargs: {"status": "user_provisioned", "user": {"username": "alice"}},
    )
    monkeypatch.setattr(
        routes,
        "update_user",
    ...
    activated = client.post(
        "/api/v1/administration/users/alice/activate",
        json={"reason": "ready", "confirmed": True},
        headers=headers,
    )
    suspended = client.post(
        "/api/v1/administration/users/alice/suspend",
        json={"reason": "leave", "confirmed": True},
        headers=headers,
    )
    updated = client.post(
        "/api/v1/administration/users/alice/update",
        json={"role": "reviewer", "reason": "promotion", "confirmed": True},
        headers=headers,
    )
    assert [
        created.status_code,
...
def test_v28_1_release_note_and_no_migration():
    note = Path("release/V28_1_USER_ACCOUNT_ADMINISTRATION.md").read_text(
        encoding="utf-8"
    )
    for phrase in (
        ...
        "immutable account audit history",
        "last active administrator",
        "Credentials are never returned",
        "case access scope is unchanged",
        "no migration",
    ):
    ...
    migrations = [
        ...
        if directory.exists()
        for path in directory.rglob("*v28_1*")
    ]
    assert migrations == []
