from src.socmint.access_review_events_v28_4 import (
    assign_review,
    close_review,
    create_review,
    decide_review,
...
def test_v28_4_review_assignment_decisions_remediation_and_closure(
    ...
    created = create_review(
        ...
        scope={"users": ["alice"], "case_ids": ["case-a"]},
    ...
    decided = decide_review(
        created["review_id"],
        actor="admin",
        assignment_id=assigned["review_assignment_id"],
        decision="reduce",
        retained_permissions=["case.read"],
    ...
    closed = close_review(
        created["review_id"], actor="admin", reason="complete", confirmed=True
    )
...
def test_v28_4_blocks_duplicate_decision_and_unresolved_closure(tmp_path):
    ...
    unresolved = close_review(
        review["review_id"], actor="admin", reason="early", confirmed=True
    )
    assert unresolved["status"] == "blocked"
    first = decide_review(
        review["review_id"],
        actor="admin",
        assignment_id=assignment["review_assignment_id"],
        decision="certify",
