from src.socmint.saved_search_view_events_v27_3 import (
    create_view,
    current_views,
    deactivate_view,
    revise_view,
...
def test_v27_3_create_revise_visibility_and_deactivate(tmp_path, monkeypatch):
    ...
    created = create_view(
        ...
        owner="alice",
        query="reuse",
        filters={"record_types": ["finding"]},
        visibility="private",
        confirmed=True,
    )
    assert created["status"] == "saved_view_created"
    duplicate = create_view(
        name="my findings",
        owner="alice",
        query="x",
        filters={},
    ...
    revised = revise_view(
        ...
        name="My Findings v2",
        query="reuse",
        filters={"record_types": ["finding"], "statuses": ["open"]},
        visibility="shared",
    ...
    assert any(
        item["saved_view_id"] == revised["saved_view_id"]
        for item in visible_views("bob")
    )
    deactivated = deactivate_view(
        revised["saved_view_id"], actor="alice", reason="obsolete", confirmed=True
    )
...
def test_v27_3_run_uses_current_access_scope(monkeypatch):
    ...
    view = {
        ...
        "view_status": "active",
        "definition": {"query": "alpha", "filters": {"record_types": ["case"]}},
    }
    ...
    result = run_saved_view(
        "view-1", user_identity="alice", allowed_case_ids={"case-a"}, limit=25
    )
