...
def test_case_delivery_recovery_resume_operations_snapshot_marks_ready():
    ...
    result = build_case_delivery_recovery_resume_operations_snapshot(
    ...
    assert (
        snapshot["schema"] == CASE_DELIVERY_RECOVERY_RESUME_OPERATIONS_SNAPSHOT_SCHEMA
    )
...
def test_case_delivery_recovery_resume_operations_snapshot_id_is_deterministic_and_operator_sensitive():
    ...
    first = build_case_delivery_recovery_resume_operations_snapshot(
        *artifacts, resume_operator="delivery-ops"
    )
    second = build_case_delivery_recovery_resume_operations_snapshot(
        *artifacts, resume_operator="delivery-ops"
    )
    changed = build_case_delivery_recovery_resume_operations_snapshot(
        *artifacts, resume_operator="other-ops"
    )

    assert (
        first["resume_snapshot"]["resume_snapshot_id"]
        == second["resume_snapshot"]["resume_snapshot_id"]
    )
    assert (
        first["resume_snapshot"]["resume_snapshot_id"]
        != changed["resume_snapshot"]["resume_snapshot_id"]
    )
...
def test_case_delivery_recovery_resume_operations_snapshot_blocks_failed_gate_verification():
    ...
    assert any(
        blocker["key"] == "continuation_gate_verification_blocked"
...
def test_case_delivery_recovery_resume_operations_snapshot_route_requires_login(
...
def test_case_delivery_recovery_resume_operations_snapshot_route_returns_ready(
    tmp_path, monkeypatch
):
    monkeypatch.setenv("SOCMINT_DATABASE_URL", f"sqlite:///{tmp_path / 'app.db'}")
    app = create_app()
