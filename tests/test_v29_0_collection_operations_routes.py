...
def _app(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "SOCMINT_SECRET_KEY", "v29-0-route-test-secret-key-with-more-than-32-characters"
    )
    app = create_app()
...
def test_v29_0_release_note_and_no_migration():
    note = Path("release/V29_0_COLLECTION_OPERATIONS_WORKSPACE.md").read_text(
        encoding="utf-8"
    )
    for phrase in (
    ...
    migrations = [
        ...
        if directory.exists()
        for path in directory.rglob("*v29_0*")
    ]
    assert migrations == []
