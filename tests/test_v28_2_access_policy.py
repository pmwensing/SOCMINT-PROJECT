from src.socmint.access_policy_events_v28_2 import (
    create_case_access_rule,
    define_role,
    revise_role,
    revoke_case_access_rule,
)
from src.socmint.access_policy_workspace_v28_2 import (
...
def test_v28_2_roles_inheritance_denies_and_history(tmp_path, monkeypatch):
    ...
    reviewer = define_role(
        actor="admin",
        name="reviewer",
        permissions=["review.request"],
        inherits_role_ids=[base["role_id"]],
        description="review",
        reason="define",
        confirmed=True,
    )
    ...
    assert resolution["effective_permissions"] == [
        "case.read",
        "evidence.read",
        "review.request",
    ]
    revised = revise_role(
        ...
        actor="admin",
        name="reviewer",
        permissions=["review.request", "review.decide"],
        inherits_role_ids=[base["role_id"]],
        description="updated",
        reason="revise",
        confirmed=True,
    ...
    revoked = revoke_case_access_rule(
        allow["access_rule_id"], actor="admin", reason="remove", confirmed=True
    )
...
def test_v28_2_rejects_invalid_permission_and_self_inheritance(tmp_path):
    ...
    revised = revise_role(
        ...
        name="viewer",
        permissions=["case.read"],
        inherits_role_ids=[role["role_id"]],
        description="",
        reason="bad",
        confirmed=True,
    )
