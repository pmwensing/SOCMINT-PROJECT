...
| Connector execution | existing connector and governance execution layers | record adapter and run bindings | no hidden execution engine or background collector |
...
v36 may introduce only the records needed to represent information that has no current authoritative owner:
...
- `CanonicalObservationEnvelope` — a deterministic adapter envelope that references rather than replaces an existing observation
...
No v36.1 schema or runtime implementation may begin until this ownership map passes focused and full regression checks and the v35 program is formally closed.
