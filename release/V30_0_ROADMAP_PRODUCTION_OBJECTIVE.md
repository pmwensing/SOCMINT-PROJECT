...
| v30.6 | Dossier Contribution and Reassessment | Approve, hold, reject, or reassess analytic contributions while preserving prior decisions. | Next |
...
- Claims, source linkages, conflict events, confidence assessments, and human reviews are append-only.
- Human approval requires substantial confidence and no unresolved analytic conflict.
- Review reassessment preserves every prior decision and records the superseded review ID.
- Human approval permits consequential analytic use but does not authorize dossier contribution.
- Claims, evidence, observations, confidence records, and dossiers are not rewritten.
- No database migration was introduced.

## Validation contract

```bash
python3 -m pytest -q tests/test_v30_0*.py tests/test_v30_1*.py tests/test_v30_2*.py tests/test_v30_3*.py tests/test_v30_4*.py tests/test_v30_5*.py
python3 -m pytest -q tests/test_v30*.py
python3 -m pytest -q
```

The program remains open. The next implementation target is v30.6 Dossier Contribution and Reassessment.
