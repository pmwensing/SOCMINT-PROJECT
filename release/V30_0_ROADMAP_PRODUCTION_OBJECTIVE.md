...
v30.0 implements the read-only workspace. v30.1 adds append-only corroboration claims. v30.2 binds those claims to accepted evidence and observations. v30.3 preserves contradictions and disagreements. v30.4 adds bounded explainable confidence assessments.
...
| v30.5 | Human Analytic Review and Decision Record | Require explicit analyst review before consequential use. | Next |
...
- Claims, source linkages, conflict events, and confidence assessments are append-only.
- Confidence inputs, penalties, limitations, and score caps are visible.
- Confidence scores are capped at 79 until human review.
- Confidence is not truth and does not complete review or authorize dossier contribution.
...
```bash
python3 -m pytest -q tests/test_v30_0*.py tests/test_v30_1*.py tests/test_v30_2*.py tests/test_v30_3*.py tests/test_v30_4*.py
python3 -m pytest -q tests/test_v30*.py
python3 -m pytest -q
```

The program remains open. The next implementation target is v30.5 Human Analytic Review and Decision Record.
