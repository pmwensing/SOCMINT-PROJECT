...
print("[+] v12.10.31H summary from", p)
for key in [
    "overall_status",
    "drift_lock",
    "framework",
...
# Authoritative route proof after v12.10.31G.
assert summary.get("missing_v12_endpoint_suffixes", 0) == 0
PY

# Keep audit status as source of truth. If this exits non-zero now, it should be
# because remaining drift is schema/model reconciliation, not route registration.
exit "$AUDIT_STATUS"
