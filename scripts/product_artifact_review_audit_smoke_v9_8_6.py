    ...
    with tempfile.TemporaryDirectory(prefix="socmint-v986-") as tmp:
        ...
        with app.test_client() as client:
            ...
            ok = (
                ...
                and "Artifact Review Audit Trail" in body
                and "second audit state" in body
            )
            print(
                ("[PASS]" if ok else "[FAIL]"),
                "GET artifact audit UI",
                response.status_code,
            )
            if not ok:
                failures.append(
                    ("GET artifact audit UI", response.status_code, body[:1500])
                )
            ...
            ok = (
                response.status_code == 200
                and "Artifact Audit Trail" in body
                and "/product/artifacts/audit/" in body
            )
            print(
                ("[PASS]" if ok else "[FAIL]"),
                "GET artifact browser audit links",
                response.status_code,
            )
            if not ok:
                failures.append(
