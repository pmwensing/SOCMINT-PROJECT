...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="socmint-v997-") as tmp:
        ...
        with app.test_client() as client:
            ...
            client.post("/api/v1/product/final-gate/write", headers={"X-CSRF-Token": "v997-csrf"})
            ...
            response = client.post(
                ...
                json={"decision": "ready", "release_name": release_name, "reason": "ready handoff smoke"},
                headers={"X-CSRF-Token": "v997-csrf"},
            )
            ...
            response = client.get(f"/api/v1/product/final/handoff?release_name={release_name}")
            ...
            print(("[PASS]" if ok else "[FAIL]"), "GET handoff preview", response.status_code)
            if not ok:
                failures.append(("GET handoff preview", response.status_code, response.get_data(as_text=True)[:3000]))

            response = client.post(
                "/api/v1/product/final/handoff/build",
                json={"release_name": release_name, "handoff_name": "v9_9_7_handoff_smoke"},
                headers={"X-CSRF-Token": "v997-csrf"},
            )
            ...
            print(("[PASS]" if ok else "[FAIL]"), "build operator handoff pack", response.status_code)
            if not ok:
                failures.append(("build operator handoff pack", response.status_code, response.get_data(as_text=True)[:3000]))

            if handoff_root.exists():
                manifest = json.loads((handoff_root / "HANDOFF_MANIFEST.json").read_text())
                versions = {item.get("version") for item in manifest.get("copied", [])}
                required_versions = {"v9.9.0", "v9.9.1", "v9.9.2", "v9.9.3", "v9.9.4", "v9.9.5", "v9.9.6"}
                ok = required_versions.issubset(versions) and manifest.get("copied_count") >= 16
                print(("[PASS]" if ok else "[FAIL]"), "handoff contains every final release artifact version")
                if not ok:
                    failures.append(("handoff contains every final release artifact version", 0, json.dumps(manifest, indent=2)[:3000]))

            response = client.get(f"/product/final/handoff?release_name={release_name}")
            body = response.get_data(as_text=True)
            ok = response.status_code == 200 and "Product Release Closeout" in body and "Required Handoff Artifacts" in body
            print(("[PASS]" if ok else "[FAIL]"), "GET handoff UI", response.status_code)
            if not ok:
            ...
            print(("[PASS]" if ok else "[FAIL]"), "GET final dashboard handoff link", response.status_code)
            if not ok:
                failures.append(("GET final dashboard handoff link", response.status_code, body[:2500]))
