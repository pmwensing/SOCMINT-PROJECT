...
def _app(db: Path):
    ...
    os.environ["SOCMINT_SECRET_KEY"] = (
        "v18-browser-e2e-test-secret-key-2026-7f82b74d4cb54a42a5d69dd6f8f51a93"
    )
...
def run(output: Path) -> dict:
    report = {
        "schema": "socmint.case_intelligence_browser_e2e.v18_7",
        "version": "v18.7.0",
        "status": "passed",
        "checks": [],
    }
        ...
        try:
            ...
            browser.add_cookie(
                {
                    ...
                    "value": serializer.dumps(
                        {"user": "analyst", "_csrf_token": "v18-csrf"}
                    ),
                    "path": "/",
                }
            )
            ...
            wait.until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, "[data-case-intelligence-review]")
                )
            )
            report["checks"].append(
                {
                    "key": "workspace_render",
                    "ok": "Case Intelligence Review Workspace" in browser.page_source,
                }
            )
            ...
            wait.until(
                EC.text_to_be_present_in_element(
                    (By.ID, "case-review-feedback"), "Decision recorded"
                )
            )
            report["checks"].append({"key": "decision_feedback", "ok": True})
            wait.until(
                lambda drv: len(
                    drv.find_elements(By.CSS_SELECTOR, "#case-review-history-body tr")
                )
                == 1
            )
            ...
            wait.until(
                EC.text_to_be_present_in_element(
                    (By.ID, "case-review-feedback"), "history refreshed"
                )
            )
...
def main() -> int:
    ...
    parser.add_argument(
        "--output", default="artifacts/v18_7_case_intelligence_browser_e2e.json"
    )
