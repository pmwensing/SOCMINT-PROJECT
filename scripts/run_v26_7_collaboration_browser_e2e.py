...
def _workspace() -> dict:
    return {
        ...
        "user_identity": USER,
        "participating_cases": [
            {"case_id": CASE_ID, "assigned_roles": ["lead_analyst"]}
        ],
...
def _responses() -> dict:
    return {
        ...
        "counts": {"history": 0, "targets": 0, "unresolved": 0, "resolved": 0},
        "history": [],
        "source_records_mutated": False,
    }
...
def _queue() -> dict:
    return {
        ...
        "workload_imbalance": [],
        "counts": {},
        "average_collaboration_load": 0.0,
        "access_scope": {"mode": "restricted", "allowed_case_ids": [CASE_ID]},
        "queue_sha256": "q" * 64,
        "read_only": True,
    }
...
def _history() -> dict:
    return {
        ...
        "current_collaboration_state_sha256": "h" * 64,
        "source_records_mutated": False,
...
def _app(db: Path):
    ...
    os.environ["SOCMINT_SECRET_KEY"] = (
        "v26-browser-e2e-stable-secret-key-32chars-minimum"
    )
    ...
    from src.socmint import (
        collaboration_requests_handoffs_routes_v26_3 as request_routes,
    )
    from src.socmint import (
        collaboration_responses_resolution_routes_v26_4 as response_routes,
    )
    from src.socmint import (
        team_workload_collaboration_queue_routes_v26_5 as queue_routes,
    )
    ...
    team_routes.assign_case_team_role = lambda *a, **k: {
        "status": "case_team_assignment_recorded",
        "case_team_assignment_id": "assignment-e2e",
    }
    note_routes.build_collaboration_notes_workspace = lambda *a, **k: _notes()
    note_routes.create_note = lambda *a, **k: {
        "status": "collaboration_note_recorded",
        "collaboration_note_id": "note-e2e",
    }
    request_routes.build_workspace = lambda case_id: _requests()
    request_routes.create_request = lambda *a, **k: {
        "status": "collaboration_request_recorded",
        "collaboration_request_id": "request-e2e",
    }
    request_routes.create_handoff = lambda *a, **k: {
        "status": "collaboration_handoff_recorded",
        "collaboration_handoff_id": "handoff-e2e",
    }
    response_routes.build_collaboration_response_workspace = (
        lambda case_id: _responses()
    )
    response_routes.record_collaboration_response = lambda *a, **k: {
        "status": "collaboration_response_recorded",
        "collaboration_response_id": "response-e2e",
    }
...
def run() -> dict:
    report = {
        "schema": "socmint.collaboration_browser_e2e.v26_7",
        "version": "v26.7.0",
        "checks": [],
    }
    ...
    try:
        ...
        pages = [
            ...
            (
                "notes_page",
                f"/cases/{CASE_ID}/collaboration-notes",
                "Collaboration Notes",
            ),
            (
                "requests_page",
                f"/cases/{CASE_ID}/collaboration-requests",
                "Review Requests",
            ),
            (
                "responses_page",
                f"/cases/{CASE_ID}/collaboration-responses",
                "Responses",
            ),
        ...
        posts = [
            (
                "team_assignment_post",
                f"/api/v1/cases/{CASE_ID}/team/assignments",
                {
                    "user_identity": "analyst-b",
                    "role": "reviewer",
            ...
            (
                ...
                {
                    "body": "E2E collaboration note",
                    "target_type": "case",
                    "mentioned_users": ["analyst-b"],
            ...
            (
                "handoff_post",
                f"/api/v1/cases/{CASE_ID}/collaboration-handoffs",
                {
                    "handoff_to": "analyst-b",
            ...
            (
                ...
                {
                    "target_type": "request",
                    "target_id": "request-e2e",
                    "response_type": "acknowledgement",
                    "reason": "E2E acknowledgement",
                    "confirmed": True,
                },
            ),
        ]
    ...
    report["next_action"] = (
        "begin_v27" if report["v26_closed"] else "resolve_v26_browser_e2e_failures"
    )
    return report
