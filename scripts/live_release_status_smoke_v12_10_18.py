...
def run(base_url: str) -> dict[str, Any]:
    ...
    try:
        ...
        check(
            checks,
            "release_status_ui_route_registered",
            "/release/status" in routes,
            "/release/status",
        )
        check(
            checks,
            "release_gates_ui_route_registered",
            "/release/gates" in routes,
            "/release/gates",
        )
        check(
            checks,
            "release_status_api_route_registered",
            "/api/v1/release/status" in routes,
            "/api/v1/release/status",
        )
        check(
            checks,
            "release_gates_api_route_registered",
            "/api/v1/release/gates/latest" in routes,
    ...
    for template in templates:
        check(
            checks, f"template_exists:{template.name}", template.exists(), str(template)
        )
    ...
    if base_url:
        ...
        check(
            checks, "live_readyz", ready.get("status") == 200, str(ready.get("status"))
        )
        check(
            checks,
            "live_release_status_ui_protected",
            status_page.get("status") in {200, 302, 401},
            str(status_page.get("status")),
        )
        check(
            checks,
            "live_release_gates_ui_protected",
            gates_page.get("status") in {200, 302, 401},
            str(gates_page.get("status")),
        )
        check(
            checks,
            "live_release_status_api_auth_required",
            status_api.get("status") in {401, 403},
            str(status_api.get("status")),
        )
        check(
            checks,
            "live_release_gates_api_auth_required",
            gates_api.get("status") in {401, 403},
            str(gates_api.get("status")),
        )
        live = {
            "readyz": ready,
            "status_page": status_page,
            "gates_page": gates_page,
