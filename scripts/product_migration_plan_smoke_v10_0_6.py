    ...
    with tempfile.TemporaryDirectory(prefix="socmint-v1006-") as tmp:
        ...
        with app.test_client() as client:
            ...
            ok = (
                ...
                and all(
                    route.get("readiness_gate_passed") is True for route in safe_routes
                )
                and all(route.get("risk_score", 100) < 55 for route in safe_routes)
            )
            print(
                ("[PASS]" if ok else "[FAIL]"),
                "GET migration plan API",
                response.status_code,
            )
            if not ok:
                failures.append(
            ...
            ok = not any(
                route.get("safe_to_migrate")
                and plan.get("module_health_status") != "healthy"
                for route in plan.get("candidates", [])
            )
            print(
                ("[PASS]" if ok else "[FAIL]"),
                "safe routes gated by healthy module health",
                0,
            )
            if not ok:
                failures.append(
            ...
            ok = (
                response.status_code == 200
                and "Blueprint Ownership Migration Plan" in body
                and "First Wave Routes" in body
            )
            print(
                ("[PASS]" if ok else "[FAIL]"),
                "GET migration plan UI",
                response.status_code,
            )
            if not ok:
                failures.append(
                    ("GET migration plan UI", response.status_code, body[:2500])
                )

            response = client.post(
                "/api/v1/product/v10/migration-plan/write",
                headers={"X-CSRF-Token": "v1006-csrf"},
            )
            ...
            for route in [
                ...
                print(
                    ("[PASS]" if ok else "[FAIL]"),
                    f"migration link {route}",
                    response.status_code,
                )
                if not ok:
                    failures.append(
                        (f"migration link {route}", response.status_code, body[:2500])
                    )
