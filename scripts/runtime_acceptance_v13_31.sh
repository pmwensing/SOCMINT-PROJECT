...
echo "[+] v13.31 runtime acceptance"
echo "[+] root=$ROOT_DIR subject=$SUBJECT_ID base=$BASE_URL"
...
echo "[+] Forced-session route acceptance"
...
paths = [
    ...
    f"/api/v1/spine/subjects/{subject_id}/full-report/retention",
]
...
echo "[+] Controlled export acceptance"
docker compose exec -T "$APP_SERVICE" python - <<'PY'
...
subject_id = 4
root = dossier_root()
probe = root / ".acceptance-write-test"
probe.write_text("ok")
probe.unlink()
...
for key in required_paths:
    ...
    print(key, path, path.exists(), path.stat().st_size if path.exists() else 0)
...
echo "[+] v13.31 runtime acceptance complete"
