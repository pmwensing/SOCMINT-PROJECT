...
def main() -> int:
    ...
    checks = [
        ...
        run(
            "artifact-pipeline-extraction-smoke",
            ["make", "product-artifact-pipeline-extraction-smoke"],
        ),
        run(
            "module-registry-smoke-v10-0-4",
            [sys.executable, "scripts/product_module_registry_smoke_v10_0_4.py"],
        ),
    ]
    ...
    report = {
        ...
        "next_action": "Merge and tag v10.0.4 product module registry"
        if status == "pass"
        else "Fix v10.0.4 module registry failures before merge.",
    }
    ...
    storage_path = Path(
        "storage/product_qa/V10_0_4_PRODUCT_MODULE_REGISTRY_HARDENING_REPORT.json"
    )
