...
WORK_ROOT="var/test_v12_10_16"
VENV_DIR="$WORK_ROOT/.venv"
DB_PATH="$WORK_ROOT/socmint_v12_10_16.db"
DATA_DIR="$WORK_ROOT/data"
LOG_FILE="$WORK_ROOT/socmint.log"
STATUS_JSON="$REPORT_ROOT/socmint_v12_10_16_fresh_db_gate_status.json"
STATUS_MD="$REPORT_ROOT/socmint_v12_10_16_fresh_db_gate_status.md"
...
payload = {
  "schema": "socmint.release.fresh_db_gate.v12_10_16",
  "version": "12.10.16",
...
echo "[+] SOCMINT v12.10.16 Fresh DB Release Gate"
echo "[+] 1. git clean checkout"
if [ "$CI_MODE" = "true" ]; then
  echo "[+] SOCMINT_CI_MODE=true, allowing CI checkout state"
else
  git status --short
...
: > "$STATUS_MD"
section "SOCMINT v12.10.16 Fresh DB Release Gate"
section "1. git clean checkout"
if [ "$CI_MODE" = "true" ]; then pass_line "CI checkout accepted"; else pass_line "working tree clean"; fi
...
export SOCMINT_SECRET_KEY="test-v12-10-16-secret-key-000000000000000000000000"
...
section "5-7. app import, route list, tor diagnostics route"
python scripts/runtime_route_release_gate_v12_10_16.py
pass_line "runtime route release gate passed"

section "8-15. subject-to-dossier E2E and diagnostics payload"
python scripts/subject_to_dossier_e2e_v12_10_16.py
pass_line "subject-to-dossier E2E release gate passed"

record_json "pass" "GO" "SOCMINT v12.10.16 fresh DB release gate passed"
cat >> "$STATUS_MD" <<EOF
...
echo
echo "[+] SOCMINT v12.10.16 fresh DB release gate PASSED"
