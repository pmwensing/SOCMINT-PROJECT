...
def main() -> int:
    ...
    checks = [
        ...
        run(
            "artifact-review-audit-smoke",
            ["make", "product-artifact-review-audit-smoke"],
        ),
        run(
            "export-manifest-smoke", ["make", "product-artifact-export-manifest-smoke"]
        ),
        run("release-package-smoke", ["make", "product-release-package-smoke"]),
        run("zip-export-smoke", ["make", "product-release-package-zip-smoke"]),
        run(
            "release-candidate-smoke-v9-9-0",
            [sys.executable, "scripts/product_release_candidate_smoke_v9_9_0.py"],
        ),
    ]
    ...
    report = {
        ...
        "next_action": "Merge and tag v9.9.0 release candidate"
        if status == "pass"
        else "Fix release candidate failures before merge.",
    }
    ...
    storage_path = Path(
        "storage/product_qa/V9_9_0_RELEASE_CANDIDATE_HARDENING_REPORT.json"
    )
