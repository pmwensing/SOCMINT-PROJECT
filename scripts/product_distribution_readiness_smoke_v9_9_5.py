    ...
    with tempfile.TemporaryDirectory(prefix="socmint-v995-") as tmp:
        ...
        with app.test_client() as client:
            ...
            response = client.post(
                ...
                json={"decision": "ready", "release_name": "missing-release", "reason": "should be denied"},
            ...
            print(("[PASS]" if ok else "[FAIL]"), "deny ready when verification missing", response.status_code)
            if not ok:
                failures.append(("deny ready when verification missing", response.status_code, response.get_data(as_text=True)[:2000]))
            ...
            client.post("/api/v1/product/final-gate/write", headers={"X-CSRF-Token": "v995-csrf"})

            package_root = Path("storage/product_packages/v9_9_5_distribution_smoke_package")
            ...
            package_zip = Path("storage/product_packages/v9_9_5_distribution_smoke_package.zip")
            ...
            response = client.post(
                "/api/v1/product/final-release/distribution/decision",
                json={"decision": "lock", "release_name": release_name, "reason": "lock smoke release"},
                headers={"X-CSRF-Token": "v995-csrf"},
            )
            payload = response.get_json() if response.is_json else {}
            ok = (
            ...
            print(("[PASS]" if ok else "[FAIL]"), "lock verified release", response.status_code)
            if not ok:
                failures.append(("lock verified release", response.status_code, response.get_data(as_text=True)[:2500]))

            response = client.post(
                "/api/v1/product/final-release/distribution/decision",
                json={"decision": "ready", "release_name": release_name, "reason": "ready smoke release"},
                headers={"X-CSRF-Token": "v995-csrf"},
            )
            payload = response.get_json() if response.is_json else {}
            ok = (
                ...
                and payload.get("distribution", {}).get("distribution_status") == "ready"
            )
            print(("[PASS]" if ok else "[FAIL]"), "mark ready to distribute", response.status_code)
            if not ok:
                failures.append(("mark ready to distribute", response.status_code, response.get_data(as_text=True)[:2500]))

            response = client.get(f"/api/v1/product/final-release/distribution?release_name={release_name}")
            distribution = response.get_json() if response.is_json else {}
            ok = response.status_code == 200 and distribution.get("distribution_status") == "ready"
            print(("[PASS]" if ok else "[FAIL]"), "GET distribution ready state", response.status_code)
            if not ok:
                failures.append(("GET distribution ready state", response.status_code, response.get_data(as_text=True)[:2500]))
            ...
            ok = (
                ...
                and any(event.get("action") == "ready_denied" for event in audit.get("events", []))
                and any(event.get("action") == "distribution_lock" for event in audit.get("events", []))
                and any(event.get("action") == "distribution_ready" for event in audit.get("events", []))
            )
            print(("[PASS]" if ok else "[FAIL]"), "GET distribution audit", response.status_code)
            if not ok:
                failures.append(("GET distribution audit", response.status_code, response.get_data(as_text=True)[:2500]))
            ...
            print(("[PASS]" if ok else "[FAIL]"), "write distribution readiness report", response.status_code)
            if not ok:
                failures.append(("write distribution readiness report", response.status_code, response.get_data(as_text=True)[:2500]))

            response = client.get(f"/product/final-release/distribution?release_name={release_name}")
            body = response.get_data(as_text=True)
            ok = response.status_code == 200 and "Final Release Distribution Readiness" in body and "Mark Ready to Distribute" in body
            print(("[PASS]" if ok else "[FAIL]"), "GET distribution UI", response.status_code)
            if not ok:
                failures.append(("GET distribution UI", response.status_code, body[:2000]))
