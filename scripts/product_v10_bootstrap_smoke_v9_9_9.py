    ...
    with tempfile.TemporaryDirectory(prefix="socmint-v999-") as tmp:
        ...
        with app.test_client() as client:
            ...
            response = client.post(
                ...
                json={"decision": "approve_v10_bootstrap", "release_name": "missing-release", "reason": "should be denied"},
            ...
            print(("[PASS]" if ok else "[FAIL]"), "deny v10 bootstrap before v9.9.8 safe gate", response.status_code)
            if not ok:
                failures.append(("deny v10 bootstrap before v9.9.8 safe gate", response.status_code, response.get_data(as_text=True)[:2500]))
            ...
            client.post("/api/v1/product/final-gate/write", headers={"X-CSRF-Token": "v999-csrf"})
            ...
            response = client.post(f"/api/v1/product/final-release/archive/{release_name}/create", headers={"X-CSRF-Token": "v999-csrf"})
            ...
            response = client.post(
                ...
                json={"decision": "ready", "release_name": release_name, "reason": "ready bootstrap smoke"},
                headers={"X-CSRF-Token": "v999-csrf"},
            )
            ...
            response = client.post("/api/v1/product/final/write", json={"release_name": release_name}, headers={"X-CSRF-Token": "v999-csrf"})
            ...
            response = client.post(
                ...
                json={"release_name": release_name, "handoff_name": "v9_9_9_bootstrap_handoff"},
                headers={"X-CSRF-Token": "v999-csrf"},
            )
            ok = response.status_code == 200 and response.get_json().get("status") == "ready"
            ...
            response = client.post(
                ...
                json={"decision": "safe_to_start_v10", "release_name": release_name, "reason": "safe bootstrap smoke"},
                headers={"X-CSRF-Token": "v999-csrf"},
            )
            ok = response.status_code == 200 and response.get_json().get("state", {}).get("safe_to_start_v10") is True
            print(("[PASS]" if ok else "[FAIL]"), "mark v9.9.8 safe to start v10", response.status_code)
            if not ok:
                failures.append(("mark v9.9.8 safe to start v10", response.status_code, response.get_data(as_text=True)[:3000]))

            response = client.get(f"/api/v1/product/final/v10-bootstrap?release_name={release_name}")
            ...
            print(("[PASS]" if ok else "[FAIL]"), "GET v10 bootstrap closure payload", response.status_code)
            if not ok:
                failures.append(("GET v10 bootstrap closure payload", response.status_code, response.get_data(as_text=True)[:4000]))
            ...
            ok = (
                ...
                and Path("release/V9_9_9_V10_BOOTSTRAP_READINESS_MANIFEST.json").exists()
                and Path("release/V9_9_9_V10_BOOTSTRAP_READINESS_MANIFEST.md").exists()
                and written.get("safe_to_start_v10") is True
            )
            print(("[PASS]" if ok else "[FAIL]"), "write closure/bootstrap manifests", response.status_code)
            if not ok:
                failures.append(("write closure/bootstrap manifests", response.status_code, response.get_data(as_text=True)[:3000]))

            response = client.post(
                "/api/v1/product/final/v10-bootstrap/decision",
                json={"decision": "close_v9", "release_name": release_name, "reason": "close v9 smoke"},
                headers={"X-CSRF-Token": "v999-csrf"},
            )
            ok = response.status_code == 200 and response.get_json().get("state", {}).get("v9_closed") is True
            print(("[PASS]" if ok else "[FAIL]"), "close v9 line", response.status_code)
            if not ok:
                failures.append(("close v9 line", response.status_code, response.get_data(as_text=True)[:3000]))

            response = client.post(
                "/api/v1/product/final/v10-bootstrap/decision",
                json={"decision": "approve_v10_bootstrap", "release_name": release_name, "reason": "approve v10 bootstrap smoke"},
                headers={"X-CSRF-Token": "v999-csrf"},
            )
            payload = response.get_json() if response.is_json else {}
            ok = (
            ...
            print(("[PASS]" if ok else "[FAIL]"), "approve v10 bootstrap", response.status_code)
            if not ok:
                failures.append(("approve v10 bootstrap", response.status_code, response.get_data(as_text=True)[:3000]))
            ...
            ok = (
                ...
                and any(event.get("action") == "approve_v10_bootstrap_denied" for event in audit.get("events", []))
                and any(event.get("action") == "bootstrap_close_v9" for event in audit.get("events", []))
                and any(event.get("action") == "bootstrap_approve_v10_bootstrap" for event in audit.get("events", []))
            )
            print(("[PASS]" if ok else "[FAIL]"), "GET v10 bootstrap audit", response.status_code)
            if not ok:
                failures.append(("GET v10 bootstrap audit", response.status_code, response.get_data(as_text=True)[:3000]))

            response = client.get(f"/product/final/v10-bootstrap?release_name={release_name}")
            body = response.get_data(as_text=True)
            ok = response.status_code == 200 and "Final v9 Line Closure" in body and "Approve v10 Bootstrap" in body
            print(("[PASS]" if ok else "[FAIL]"), "GET v10 bootstrap UI", response.status_code)
            if not ok:
                failures.append(("GET v10 bootstrap UI", response.status_code, body[:2500]))
            ...
            print(("[PASS]" if ok else "[FAIL]"), "GET self-test bootstrap link", response.status_code)
            if not ok:
                failures.append(("GET self-test bootstrap link", response.status_code, body[:2500]))
