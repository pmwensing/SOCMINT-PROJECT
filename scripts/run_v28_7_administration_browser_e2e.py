...
def _admin_payloads():
    return {
        ...
        "operations": {
            ...
            "open_operational_incident_count": 0,
            "operational_history": [],
...
def _app(db: Path):
    ...
    os.environ["SOCMINT_SECRET_KEY"] = (
        "v28-browser-e2e-stable-secret-key-32chars-minimum"
    )
    ...
    policy_routes.evaluate_effective_access = lambda username, case_id: {
        "status": "ready",
        "username": username,
        "case_id": case_id,
    ...
    policy_writes.define_role = lambda **kwargs: {
        "status": "role_defined",
        "role_id": "role-e2e",
    }
    ...
    team_routes.create_team = lambda **kwargs: {
        "status": "team_created",
        "team_id": "team-e2e",
    }
    ...
    review_routes.create_review = lambda **kwargs: {
        "status": "access_review_created",
        "review_id": "review-e2e",
    }
    connector_routes.build_connector_administration_workspace = lambda: payloads[
        "connectors"
    ]
    connector_routes.actor_is_administrator = lambda actor: actor == USER
    connector_routes.register_connector = lambda **kwargs: {
        "status": "connector_registered",
        "connector_id": "connector-e2e",
    }
    operations_routes.build_platform_operations_workspace = lambda **kwargs: payloads[
        "operations"
    ]
    operations_routes.actor_is_administrator = lambda actor: actor == USER
    operations_routes.open_incident = lambda **kwargs: {
    ...
    try:
        if not dbs.query(database.User).filter(database.User.username == USER).first():
            dbs.add(
                database.User(
                    username=USER,
                    password_hash=generate_password_hash("v28-e2e-internal"),
                    is_admin=True,
                    role="admin",
                    is_active=True,
                )
...
def run() -> dict:
    report = {
        "schema": "socmint.administration_browser_e2e.v28_7",
        "version": "v28.7.0",
        "checks": [],
    }
    ...
    try:
        ...
        for path in apis:
            ...
            _check(
                report,
                "api_" + path.rsplit("/", 1)[-1].replace("-", "_"),
        ...
        writes = [
            ...
            (
                "team_create",
                "/api/v1/administration/teams",
                {"name": "E2E Team", "reason": "e2e", "confirmed": True},
            ),
            (
                "review_create",
                "/api/v1/administration/access-reviews",
        ...
        checkpoint = _get_json(
            driver, base + "/api/v1/administration/product-review-checkpoint"
        )
        _check(
            report,
            "checkpoint_ready",
    ...
    report.update(
        {
            ...
            if not failed
            else "resolve_v28_browser_e2e_failures",
        }
    )
    return report
