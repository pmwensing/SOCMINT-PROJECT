...
def run() -> dict:
    temp_dir = Path(tempfile.mkdtemp(prefix="socmint-v35-5-e2e-"))
    port = _port()
    ...
    try:
        ...
        driver.get(f"http://127.0.0.1:{port}/_v35_5_e2e_login")
        source = driver.page_source
        required = (
            'data-execution-recovery-observability="v35.5"',
            'data-read-only="true"',
            'data-attention-queue="true"',
            'data-reconciled-executions="true"',
            'data-automatic-retry="false"',
            'data-reconciliation-action="false"',
            'data-delegate-invocation="false"',
        )
        for key in required:
    ...
    return {
        "schema": "socmint.execution_recovery_observability_browser_e2e.v35_5",
        "version": "v35.5.0",
        "checks": checks,
