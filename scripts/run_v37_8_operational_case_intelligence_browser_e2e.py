...
def _app(temp_dir: Path):
    ...
    @app.get("/_v37_8_e2e_login")
    def _login():
        session["user"] = USER
        session["is_admin"] = True
        return redirect("/operational-case-intelligence")

    return app


def run() -> dict:
    temp_dir = Path(tempfile.mkdtemp(prefix="socmint-v37-8-e2e-"))
    port = _port()
    ...
    try:
        ...
        driver.get(f"http://127.0.0.1:{port}/_v37_8_e2e_login")
    ...
    return {
        "schema": "socmint.operational_case_intelligence_browser_e2e.v37_8",
        "version": "v37.8.0",
        "checks": checks,
