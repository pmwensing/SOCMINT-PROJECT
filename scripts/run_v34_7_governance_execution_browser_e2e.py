...
def _app(temp_dir: Path):
    ...
    from src.socmint import governance_execution_product_review_routes_v34_7 as routes
    ...
    @app.get("/_v34_e2e_login")
    def _login():
        session["user"] = USER
        return redirect("/dissemination-governance/v34-product-review")

    return app


def run() -> dict:
    temp_dir = Path(tempfile.mkdtemp(prefix="socmint-v34-e2e-"))
    port = _port()
    ...
    try:
        ...
        driver.get(f"http://127.0.0.1:{port}/_v34_e2e_login")
    ...
    return {
        "schema": "socmint.governance_execution_browser_e2e.v34_7",
        "version": "v34.7.0",
        "checks": checks,
        "passed_count": len(checks) - len(failed),
        "failed_count": len(failed),
        "status": "passed" if not failed else "failed",
        "v34_closed": not failed,
    }
