...
from .connector_administration_events_v28_5 import (
    register_connector,
    revise_connector,
    set_connector_enabled,
    update_auth_readiness,
)
from .connector_administration_workspace_v28_5 import (
...
def register_connector_administration_routes_v28_5(app):
    ...
    def connector_administration_workspace_get_v28_5():
        ...
        if not actor_is_administrator(actor):
            return render_template(
                "connector_administration_v28_5.html",
                title="Connector and Integration Administration",
                payload={
                    "status": "forbidden",
        ...
        return render_template(
            "connector_administration_v28_5.html",
            title="Connector and Integration Administration",
            payload=build_connector_administration_workspace(),
        )
    ...
    def api_connector_register_post_v28_5():
        ...
        result = register_connector(
            ...
            authorization_scopes=payload.get("authorization_scopes"),
    ...
    def api_connector_auth_readiness_post_v28_5(connector_id: str):
        ...
        if error:
            return error
        payload = _payload()
        result = update_auth_readiness(
            connector_id,
            actor=actor,
            auth_readiness=str(payload.get("auth_readiness") or ""),
            auth_expires_at=str(payload.get("auth_expires_at") or ""),
            reason=str(payload.get("reason") or ""),
            confirmed=payload.get("confirmed") is True,
            ip_address=request.remote_addr,
        )
