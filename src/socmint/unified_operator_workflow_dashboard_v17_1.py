...
UNIFIED_OPERATOR_WORKFLOW_DASHBOARD_SCHEMA = "socmint.unified_operator_workflow_dashboard.v17_1"
VERSION = "v17.1.0"
...
def _workspace_blockers(workspace: dict[str, Any]) -> list[dict[str, str]]:
    ...
    return [
        _blocker("case_delivery", str(check.get("key") or "delivery_gate"), str(check.get("detail") or "blocked"))
        for check in checks
...
def _operations_blockers(operations: dict[str, Any]) -> list[dict[str, str]]:
    blockers = operations.get("blockers") if isinstance(operations.get("blockers"), list) else []
    return [
        _blocker("operations", str(item.get("key") or "operations_blocker"), str(item.get("detail") or item.get("reason") or "blocked"))
...
def _recommended_action(
    ...
    evaluation = release_console.get("evaluation") if isinstance(release_console.get("evaluation"), dict) else {}
    ...
    if evaluation.get("decision") == "PAUSE_FOR_REPAIR" or release_console.get("status") == "needs_review":
        return {
...
def build_unified_operator_workflow_dashboard(
    ...
    *,
    ...
    blockers.extend(
        _blocker("recovery_chain", str(item.get("key") or "recovery_chain"), str(item.get("detail") or "blocked"))
        for item in recovery_chain.get("blockers", [])
        if isinstance(item, dict)
    )
    blockers.extend(
        _blocker("release_console", str(item.get("key") or "release_console"), str(item.get("detail") or "needs review"))
    ...
    release_health = release_console.get("release_health") if isinstance(release_console.get("release_health"), dict) else {}
    release_health_state = release_health.get("freshness", {}).get("status") or release_health.get("status") or "unknown"
    ...
    return {
        ...
        "version": VERSION,
        "case_id": case_id,
        "status": "ready" if case_ready and recovery_closed and operations_ready else "attention_required",
        "summary": {
