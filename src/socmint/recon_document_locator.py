...
DORK_TEMPLATES = [
    ...
    },
    {
        ...
        "template": '"{target}" (filetype:doc OR filetype:docx OR filetype:xls OR filetype:xlsx)',
    },
    {
        "id": "public_index",
        "label": "Open directory indexes",
        "template": 'intitle:"index of" "{target}"',
    },
    {
        "id": "public_cloud",
        "label": "Public cloud links",
        "template": '"{target}" (site:drive.google.com OR site:docs.google.com OR site:dropbox.com OR site:onedrive.live.com)',
    },
    {
        "id": "email_mentions",
        "label": "Email mentions",
        "template": '"{target}" email OR contact OR "@"',
    },
    {
        "id": "paste_mentions",
        "label": "Paste/code mentions",
        "template": '"{target}" (site:pastebin.com OR site:gist.github.com OR site:github.com)',
    },
    {
        "id": "court_records",
        "label": "Court/public records",
        "template": '"{target}" (court OR docket OR filing OR tribunal OR affidavit)',
    },
    {
        "id": "foi_disclosures",
        "label": "FOI/disclosure documents",
        "template": '"{target}" (FOI OR disclosure OR "freedom of information" OR "access to information")',
    },
]
...
class DocumentLocatorResult:
    ...
    legal_safety: list[str] = field(
        default_factory=lambda: [
            LEGAL_LABELS["manual_review"],
            LEGAL_LABELS["not_evidence"],
        ]
    )
...
def _safe_request_json(
    url: str, headers: dict[str, str] | None = None, timeout: int = 12
) -> Any:
...
def render_dork_templates(target: str) -> list[dict[str, str]]:
    ...
    return [
        {**item, "query": item["template"].format(target=safe_target)}
        for item in DORK_TEMPLATES
    ]


def _mock_result(
    connector: str, query: str, source: str, title: str, url: str, snippet: str = ""
) -> DocumentLocatorResult:
    return DocumentLocatorResult(
        ...
        risk={
            ...
            "public_only": True,
            "note": "Stubbed result unless live recon is explicitly enabled.",
        },
    )
...
def brave_search(query: str, count: int = 10) -> list[DocumentLocatorResult]:
    ...
    live = os.environ.get("SOCMINT_RECON_LIVE", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
    ...
    if not api_key or not live:
        return [
            _mock_result(
                "brave",
                query,
                "brave_search_api_stub",
                "Brave Search API configured as preferred backend",
                f"https://search.brave.com/search?q={quote_plus(query)}",
                "Set BRAVE_SEARCH_API_KEY and SOCMINT_RECON_LIVE=1 for live API calls.",
            )
        ]
    ...
    for rank, item in enumerate(payload.get("web", {}).get("results", []), start=1):
        ...
        results.append(
            DocumentLocatorResult(
                ...
                source_trust="review"
                if not _is_public_file(url)
                else "document-candidate",
                legal_safety=[
                    LEGAL_LABELS["api_search"],
                    LEGAL_LABELS["manual_review"],
                    LEGAL_LABELS["not_evidence"],
...
def wayback_cdx(query: str, count: int = 10) -> list[DocumentLocatorResult]:
    ...
    if not live:
        return [
            _mock_result(
                ...
                query,
                "wayback_cdx_stub",
                "Wayback CDX public archive lookup",
                f"https://web.archive.org/cdx?url={quote_plus(target)}&output=json",
                "Enable SOCMINT_RECON_LIVE=1 for live CDX queries.",
            )
        ]
    endpoint = "https://web.archive.org/cdx?" + urlencode(
        {
            "url": target,
            "output": "json",
            "limit": count,
    ...
    for rank, row in enumerate(
        rows[1:] if isinstance(rows, list) and rows else [], start=1
    ):
        ...
        results.append(
            DocumentLocatorResult(
                ...
                provenance={
                    ...
                    "timestamp": timestamp,
                    "digest": digest,
                    "archive": "Internet Archive Wayback CDX",
                },
                source_trust="archive",
                legal_safety=[
                    LEGAL_LABELS["archive"],
                    LEGAL_LABELS["manual_review"],
                    LEGAL_LABELS["not_evidence"],
                ],
...
def commoncrawl_index(query: str, count: int = 10) -> list[DocumentLocatorResult]:
    ...
    if not live:
        return [
            _mock_result(
                ...
                query,
                "commoncrawl_stub",
                "Common Crawl public CDX lookup",
                f"https://index.commoncrawl.org/?url={quote_plus(target)}",
                "Enable SOCMINT_RECON_LIVE=1 for live Common Crawl index queries.",
            )
        ]
    ...
    latest = (
        indexes[0]["cdx-api"]
        if indexes
        else "https://index.commoncrawl.org/CC-MAIN-2025-18-index"
    )
    endpoint = (
        latest + "?" + urlencode({"url": target, "output": "json", "limit": count})
    )
    ...
    if not token or not live:
        return [
            _mock_result(
                ...
                f"https://github.com/search?q={quote_plus(query)}&type=code",
                "Set GITHUB_TOKEN and SOCMINT_RECON_LIVE=1 for live API calls.",
            )
        ]
    endpoint = "https://api.github.com/search/code?" + urlencode(
        {"q": query, "per_page": min(count, 100)}
    )
    payload = _safe_request_json(
        endpoint,
        headers={
...
def document_locator_search(
    query: str, connectors: list[str] | None = None, count: int = 10
) -> dict[str, Any]:
...
def queue_for_manual_acquisition(
    result: dict[str, Any], actor: str | None = None, root: str | None = None
) -> dict[str, Any]:
    ...
    item = {
        ...
        "result": result,
        "legal_safety": result.get("legal_safety")
        or [LEGAL_LABELS["manual_review"], LEGAL_LABELS["not_evidence"]],
