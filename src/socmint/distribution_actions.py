...
def distribution_action_log_path(
    case_id: str, subject_id: str, root: str | Path = DISTRIBUTION_ACTIONS_ROOT
) -> Path:
...
def distribution_action_summary_path(
    case_id: str, subject_id: str, root: str | Path = DISTRIBUTION_ACTIONS_ROOT
) -> Path:
    base = Path(root) / _safe_component(case_id)
    base.mkdir(parents=True, exist_ok=True)
...
def latest_distribution_action(
    case_id: str, subject_id: str, root: str | Path = DISTRIBUTION_ACTIONS_ROOT
) -> dict[str, Any] | None:
...
def record_distribution_action(
    ...
    if action == "approve" and not safe_to_distribute:
        raise ValueError(
            "Cannot approve distribution while certification blockers remain."
        )
...
def distribution_action_summary(
    case_id: str, subject_id: str, root: str | Path = DISTRIBUTION_ACTIONS_ROOT
) -> dict[str, Any]:
    events = load_distribution_actions(case_id, subject_id, root=root)
...
def distribution_action_packet(
    case_id: str, subject_id: str, root: str | Path = DISTRIBUTION_ACTIONS_ROOT
) -> dict[str, Any]:
    entry = certification_index_entry(case_id=case_id, subject_id=subject_id)
    ...
    return {
        ...
        "certification": entry,
        "actions": actions,
        "distribution_ready": bool(entry.get("safe_to_distribute"))
        and actions.get("approved") is True,
        "recommended_bundle": entry.get("recommended_bundle")
...
def distribution_action_markdown(
    case_id: str, subject_id: str, root: str | Path = DISTRIBUTION_ACTIONS_ROOT
) -> str:
    ...
    for event in actions.get("events", []):
        lines.append(
            f"- {event.get('created_at')} — {event.get('actor')} — {event.get('action')} — {event.get('note')}"
        )
