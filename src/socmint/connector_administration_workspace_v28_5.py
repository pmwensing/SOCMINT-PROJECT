...
def _run_health() -> dict[str, dict[str, Any]]:
    ...
    try:
        ...
        for name, items in grouped.items():
            statuses = Counter(
                str(getattr(item, "status", None) or "unknown").lower()
                for item in items
            )
            latest = items[0]
            result[name] = {
                ...
                "error_count": sum(
                    bool(getattr(item, "error", None)) for item in items
                ),
            }
        return result
    finally:
...
def build_connector_administration_workspace() -> dict[str, Any]:
    ...
    disabled = [
        item for item in connectors if item.get("connector_status") == "disabled"
    ]
    health = _run_health()
    findings = []
    ...
    for item in connectors:
        ...
        if item.get("enabled") and readiness not in {"configured", "expiring"}:
            findings.append(
                {
                    "severity": "high",
                    "key": "enabled_connector_not_auth_ready",
        ...
        if not (definition.get("authorization_scopes") or []):
            findings.append(
                {
                    "severity": "medium",
                    "key": "connector_without_authorization_scope",
                    "connector_id": item.get("connector_id"),
                }
            )
        if not (definition.get("rate_limit_policy") or {}):
            findings.append(
                {
                    "severity": "low",
                    "key": "connector_without_rate_limit_policy",
                    "connector_id": item.get("connector_id"),
                }
            )
        run_state = health.get(
            name,
            {
                "run_count": 0,
                "status_counts": {},
        ...
        if run_state.get("error_count"):
            findings.append(
                {
                    "severity": "medium",
                    "key": "connector_run_errors",
                    "connector_id": item.get("connector_id"),
                    "error_count": run_state.get("error_count"),
                }
            )
        summaries.append(
            {
    ...
    readiness_counts = Counter(
        str(item.get("auth_readiness") or "not_configured")
        for item in connectors
