    ...
    for event in history():
        ...
        if event_type in {"team_created", "team_revised"}:
            ...
            if previous in teams:
                teams[previous] = {
                    **teams[previous],
                    "team_status": "superseded",
                    "superseded_by_team_id": team_id,
                }
...
def find_team(team_id: str) -> dict[str, Any] | None:
    return next(
        (item for item in current_teams() if item.get("team_id") == team_id), None
    )
...
def create_team(
    *,
    ...
    if confirmed is not True:
        return blocked("explicit_team_creation_confirmation_required")
    if not name:
        return blocked("team_name_required")
    if not reason:
        return blocked("administrative_reason_required")
    if name.lower() in {
        str((item.get("definition") or {}).get("name") or "").lower()
        for item in current_teams()
        if item.get("team_status") == "active"
    }:
        return blocked("active_team_name_must_be_unique")
    ...
    event = {
        ...
        "version": VERSION,
        **content,
        "team_id": f"team-{digest[:24]}",
        "team_event_id": f"team-event-{digest[:24]}",
        "team_event_sha256": digest,
        "team_records_mutated": False,
        "case_access_scope_changed": False,
    }
    return {
        **_record(ACTIONS[0], actor, name, event, ip_address),
        "status": "team_created",
        "next_action": "configure_team_structure",
    }
...
def revise_team(
    ...
    *,
    ...
    if previous is None or previous.get("team_status") != "active":
        return blocked("active_team_required")
    if confirmed is not True:
        return blocked("explicit_team_revision_confirmation_required")
    if not str(reason or "").strip():
        return blocked("administrative_reason_required")
    definition = {
        "name": str(
            name or (previous.get("definition") or {}).get("name") or ""
        ).strip(),
        "description": str(description or "").strip(),
    }
    binding = {
        "team_id": team_id,
        "team_event_id": previous.get("team_event_id"),
        "team_event_sha256": previous.get("team_event_sha256"),
        "definition_sha256": previous.get("definition_sha256"),
        "revision": previous.get("revision"),
    }
    content = {
        "event_type": "team_revised",
        "definition": definition,
        "definition_sha256": _sha(definition),
        "reason": str(reason).strip(),
        "revision": int(previous.get("revision") or 1) + 1,
        "supersedes_team_id": team_id,
        "previous_team_binding": binding,
        "previous_team_binding_sha256": _sha(binding),
        "member_usernames": previous.get("member_usernames") or [],
        "supervisor_username": previous.get("supervisor_username"),
        "organizational_scope": previous.get("organizational_scope"),
        "ownership_boundaries": previous.get("ownership_boundaries") or [],
        "workload_group": previous.get("workload_group"),
    }
    digest = _sha(content)
    event = {
        "schema": SCHEMA,
        ...
        **content,
        ...
        "team_event_id": f"team-event-{digest[:24]}",
        "team_event_sha256": digest,
        "prior_team_event_mutated": False,
        "case_access_scope_changed": False,
    }
    return {
        **_record(ACTIONS[1], actor, definition["name"], event, ip_address),
        "status": "team_revised",
        "next_action": "review_team_structure",
    }
...
def append_team_event(
    ...
    **fields: Any,
    ...
    if team is None or team.get("team_status") != "active":
        return blocked("active_team_required")
    if confirmed is not True:
        return blocked("explicit_team_change_confirmation_required")
    if not str(reason or "").strip():
        return blocked("administrative_reason_required")
    action_map = {
        "team_member_added": ACTIONS[2],
        "team_member_removed": ACTIONS[3],
        "team_supervisor_assigned": ACTIONS[4],
        "team_scope_bound": ACTIONS[5],
        "team_workload_group_set": ACTIONS[6],
    }
    if event_type not in action_map:
        return blocked("team_event_type_invalid")
    content = {
    ...
    return {
        **_record(action_map[event_type], actor, team_id, event, ip_address),
        "status": "team_updated",
        "next_action": "review_team_structure",
    }
