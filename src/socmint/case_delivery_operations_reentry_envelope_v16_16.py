...
from .case_delivery_recovery_resume_operations_snapshot_v16_14 import build_case_delivery_recovery_resume_operations_snapshot
...
CASE_DELIVERY_OPERATIONS_REENTRY_ENVELOPE_SCHEMA = "socmint.case_delivery_operations_reentry_envelope.v16_16"
CASE_DELIVERY_OPERATIONS_REENTRY_ENVELOPE_RESULT_SCHEMA = "socmint.case_delivery_operations_reentry_envelope.v16_16.result"
VERSION = "v16.16.0"
...
def _reentry_payload(
    ...
    return {
        ...
        "resume_snapshot_verification_status": resume_snapshot_verification.get("status"),
        "resume_snapshot_verified": resume_snapshot_verification.get("verified") is True,
        "safe_to_reenter_operations": resume_snapshot_verification.get("safe_to_reenter_operations") is True,
        "ready_for_operations_dispatch": resume_snapshot_verification.get("next_action") == "execute_delivery_operations",
...
def _reentry_blockers(
    ...
    if not resume_snapshot:
        blockers.append(_blocker("missing_resume_snapshot", "resume snapshot is missing"))
    if not resume_snapshot_verification:
        blockers.append(_blocker("missing_resume_snapshot_verification", "resume snapshot verification is missing"))
    if resume_snapshot_verification and resume_snapshot_verification.get("verified") is not True:
    ...
    if resume_snapshot_verification and resume_snapshot_verification.get("safe_to_reenter_operations") is not True:
        blockers.append(_blocker("not_safe_to_reenter_operations", "resume snapshot verification is not safe to re-enter operations"))
    if resume_snapshot_verification and resume_snapshot_verification.get("next_action") != "execute_delivery_operations":
    ...
    if recovery and resume_snapshot and recovery.get("queue_id") != resume_snapshot.get("queue_id"):
        blockers.append(_blocker("resume_snapshot_queue_mismatch", "resume snapshot queue_id does not match recovery queue_id"))
    if resume_snapshot and resume_snapshot_verification and resume_snapshot.get("resume_snapshot_id") != resume_snapshot_verification.get("resume_snapshot_id"):
        blockers.append(_blocker("resume_snapshot_id_mismatch", "resume snapshot id does not match verification"))
    return blockers
...
def build_case_delivery_operations_reentry_envelope(
    ...
    *,
    ...
    if not safe_resume_snapshot_verification:
        safe_resume_snapshot_verification = verify_case_delivery_recovery_resume_operations_snapshot(
    ...
    blockers = _reentry_blockers(safe_recovery, safe_resume_snapshot, safe_resume_snapshot_verification)
    if blockers:
        return {
            "schema": CASE_DELIVERY_OPERATIONS_REENTRY_ENVELOPE_RESULT_SCHEMA,
            "version": VERSION,
            "case_id": safe_recovery.get("case_id") or safe_resume_snapshot.get("case_id"),
            "queue_id": safe_recovery.get("queue_id") or safe_resume_snapshot.get("queue_id"),
    ...
    reentry_envelope = {
        **payload,
        ...
        "reentry_envelope_id": sha256_text(canonical_json({**payload, "payload_sha256": payload_hash})),
    }
    return {
        "schema": CASE_DELIVERY_OPERATIONS_REENTRY_ENVELOPE_RESULT_SCHEMA,
        "version": VERSION,
...
def build_case_delivery_operations_reentry_envelope_from_request(case_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    reentry_operator = safe_payload.get("reentry_operator") if isinstance(safe_payload.get("reentry_operator"), str) else None
