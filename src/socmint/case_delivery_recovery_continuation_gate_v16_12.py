...
from .case_delivery_recovery_finalization_record_v16_10 import build_case_delivery_recovery_finalization_record
...
CASE_DELIVERY_RECOVERY_CONTINUATION_GATE_SCHEMA = "socmint.case_delivery_recovery_continuation_gate.v16_12"
CASE_DELIVERY_RECOVERY_CONTINUATION_GATE_RESULT_SCHEMA = "socmint.case_delivery_recovery_continuation_gate.v16_12.result"
VERSION = "v16.12.0"
...
def _gate_payload(
    ...
    return {
        ...
        "ready_for_delivery_continuation": finalization_verification.get("ready_for_delivery_continuation") is True,
...
def _gate_blockers(
    ...
    if not finalization:
        blockers.append(_blocker("missing_finalization", "finalization artifact is missing"))
    if not finalization_verification:
        blockers.append(_blocker("missing_finalization_verification", "finalization verification artifact is missing"))
    if finalization_verification and finalization_verification.get("verified") is not True:
    ...
    if finalization_verification and finalization_verification.get("ready_for_delivery_continuation") is not True:
    ...
    if recovery and finalization and recovery.get("queue_id") != finalization.get("queue_id"):
        blockers.append(_blocker("finalization_queue_mismatch", "finalization queue_id does not match recovery queue_id"))
    if finalization and finalization_verification and finalization.get("finalization_id") != finalization_verification.get("finalization_id"):
        blockers.append(_blocker("finalization_id_mismatch", "finalization id does not match verification"))
    if finalization and finalization_verification and finalization.get("audit_package_id") != finalization_verification.get("audit_package_id"):
        blockers.append(_blocker("audit_package_id_mismatch", "audit package id does not match verification"))
    return blockers
...
def build_case_delivery_recovery_continuation_gate(
    ...
    *,
    ...
    if not safe_finalization_verification:
        safe_finalization_verification = verify_case_delivery_recovery_finalization_record(
            safe_finalization,
            safe_recovery,
            safe_receipt,
            safe_closure,
    ...
    blockers = _gate_blockers(safe_recovery, safe_finalization, safe_finalization_verification)
    if blockers:
        return {
            ...
            "queue_id": safe_recovery.get("queue_id") or safe_finalization.get("queue_id"),
    ...
    payload = _gate_payload(safe_recovery, safe_finalization, safe_finalization_verification, gate_operator)
    payload_hash = sha256_text(canonical_json(payload))
    continuation_gate = {
        **payload,
        "payload_sha256": payload_hash,
        "continuation_gate_id": sha256_text(canonical_json({**payload, "payload_sha256": payload_hash})),
    }
    return {
        "schema": CASE_DELIVERY_RECOVERY_CONTINUATION_GATE_RESULT_SCHEMA,
        "version": VERSION,
...
def build_case_delivery_recovery_continuation_gate_from_request(case_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    gate_operator = safe_payload.get("gate_operator") if isinstance(safe_payload.get("gate_operator"), str) else None
