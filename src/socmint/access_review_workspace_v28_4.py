...
def _excessive_access_findings(
    ...
    for role in roles:
        ...
        if (
            resolved.get("effective_permission_count", 0) >= 12
            and str(definition.get("name") or "").lower() != "admin"
        ):
            findings.append(
                {
                    "finding": "broad_non_admin_role",
                    "role_id": role_id,
                    "role_name": definition.get("name"),
                    "permission_count": resolved.get("effective_permission_count"),
                }
            )
    for rule in rules:
        if rule.get("rule_status") != "active":
        ...
        if definition.get("effect") == "allow" and len(permissions) >= 8:
            findings.append(
                {
                    "finding": "broad_case_access_rule",
                    "access_rule_id": rule.get("access_rule_id"),
                    "subject_type": definition.get("subject_type"),
                    "subject_id": definition.get("subject_id"),
                    "case_id": definition.get("case_id"),
...
def build_access_review_workspace() -> dict[str, Any]:
    ...
    decisions = [
        decision for review in reviews for decision in review.get("decisions", [])
    ]
    decision_counts = Counter(
        str((item.get("decision_record") or {}).get("decision") or "unknown")
    ...
    for review in open_reviews:
        decided_ids = {
            str(item.get("review_assignment_id"))
        ...
        for assignment in review.get("assignments", []):
            if str(assignment.get("review_assignment_id")) not in decided_ids:
                pending_assignments.append(
                    {"review_id": review.get("review_id"), **assignment}
                )
                ...
                remediation_queue.append(
                    {
                        ...
                        "retained_permissions": record.get("retained_permissions")
                        or [],
                        "reason": record.get("reason"),
                        "remediation_status": "pending",
                        "source_event_sha256": decision.get("review_event_sha256"),
                    }
                )
