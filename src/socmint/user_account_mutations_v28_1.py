...
from .user_account_events_v28_1 import (
    ACTIONS,
    ALLOWED_ROLES,
    SCHEMA,
    VERSION,
...
def _active_admin_count(session) -> int:
    return (
        ...
        .filter(database.User.is_admin.is_(True), database.User.is_active.is_(True))
        .count()
    )
    ...
    *,
    ...
    try:
        ...
        return {
            ...
            "credential_hash_returned": False,
            "activation_required": True,
            "case_access_scope_changed": False,
            "next_action": "complete_secure_credential_onboarding",
        }
    finally:
        session.close()
...
def update_user(
    ...
    *,
    ...
    try:
        ...
        desired_active = (
            bool(is_active) if is_active is not None else bool(user.is_active)
        )
        if (
            user.is_admin
            and user.is_active
            and (not desired_admin or not desired_active)
        ...
        return {
            ...
            "user": after,
            "account_event": event,
            "prior_user_snapshot_mutated": False,
            "case_access_scope_changed": False,
            "next_action": "review_user_account",
        }
    finally:
        session.close()
