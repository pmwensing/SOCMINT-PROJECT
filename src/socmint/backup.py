...
def create_backup(destination, encrypt=True):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        raw_backup = Path(temp_dir) / ('socmint.sqlite' if is_sqlite(settings.database_url) else 'socmint.pgsql')
        ...
        if encrypt:
            if not settings.backup_passphrase:
                raise RuntimeError('SOCMINT_BACKUP_PASSPHRASE is required for encrypted backups.')
...
def restore_backup(source, destination_database_url=None, encrypted=True):
    settings = load_settings(require_secret=False, database_url=destination_database_url)
    source = Path(source)
    with tempfile.TemporaryDirectory() as temp_dir:
        raw_backup = Path(temp_dir) / 'socmint.restore'
        if encrypted:
            if not settings.backup_passphrase:
                raise RuntimeError('SOCMINT_BACKUP_PASSPHRASE is required to restore encrypted backups.')
        ...
        if is_sqlite(settings.database_url):
            ...
            subprocess.run(['psql', settings.database_url, '-f', str(raw_backup)], check=True)
