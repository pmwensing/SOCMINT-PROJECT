...
from .collection_quality_v29_6 import (
    CONTRIBUTION_STATES,
    TRUST_TIERS,
    contribution_reviews,
    latest_contribution_review,
    quality_assessments,
)
...
def build_collection_quality_workspace() -> dict[str, Any]:
    ...
    assessment_by_artifact = {
        str(item.get("artifact_id") or ""): item for item in assessments
    }
    queue = []
    findings = []
    for artifact in artifacts:
        ...
        if assessment is None:
            ...
            continue
        review = latest_contribution_review(
            str(assessment.get("quality_assessment_id") or "")
        )
        if review is None:
            queue.append(
                {
                    "artifact_id": artifact_id,
                    "quality_assessment_id": assessment.get("quality_assessment_id"),
                    "quality_score": assessment.get("quality_score"),
                    "trust_tier": assessment.get("trust_tier"),
                    "reason": "dossier_contribution_review_required",
                }
            )
        if assessment.get("trust_tier") in {"untrusted", "limited"}:
            findings.append(
                {
                    "severity": "high"
                    if assessment.get("trust_tier") == "untrusted"
                    else "medium",
                    "key": "low_trust_collection_output",
                    "artifact_id": artifact_id,
                    "quality_assessment_id": assessment.get("quality_assessment_id"),
                }
            )
    trust_counts = Counter(
        str(item.get("trust_tier") or "untrusted") for item in assessments
    )
    decision_counts = Counter(
        str(item.get("decision") or "pending_review") for item in reviews
    )
