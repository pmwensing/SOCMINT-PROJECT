...
CASE_DELIVERY_RECOVERY_FINALIZATION_RECORD_VERIFICATION_SCHEMA = "socmint.case_delivery_recovery_finalization_record_verification.v16_11"
VERSION = "v16.11.0"
...
def verify_case_delivery_recovery_finalization_record(
    ...
    if not safe_finalization:
        blockers.append(_blocker("missing_finalization", "finalization record is missing"))
    if not audit_check.get("verified"):
        blockers.extend(deepcopy(audit_check.get("blockers") or []))
        blockers.append(_blocker("audit_verification_blocked", "audit package verification did not pass"))

    expected_payload_hash = sha256_text(canonical_json(_finalization_payload(safe_finalization))) if safe_finalization else None
    if safe_finalization and safe_finalization.get("payload_sha256") != expected_payload_hash:
        blockers.append(_blocker("payload_hash_mismatch", "finalization payload hash does not match finalization fields"))

    expected_finalization_id = (
        sha256_text(canonical_json({**_finalization_payload(safe_finalization), "payload_sha256": safe_finalization.get("payload_sha256")}))
        if safe_finalization
        else None
    )
    if safe_finalization and safe_finalization.get("finalization_id") != expected_finalization_id:
        blockers.append(_blocker("finalization_id_mismatch", "finalization id does not match canonical finalization payload"))

    if safe_finalization and safe_finalization.get("queue_id") != safe_recovery.get("queue_id"):
        blockers.append(_blocker("queue_id_mismatch", "finalization queue_id does not match recovery queue"))
    if safe_finalization and safe_finalization.get("case_id") != safe_recovery.get("case_id"):
        blockers.append(_blocker("case_id_mismatch", "finalization case_id does not match recovery queue"))
    if safe_finalization and safe_finalization.get("receipt_id") != safe_receipt.get("receipt_id"):
        blockers.append(_blocker("receipt_id_mismatch", "finalization receipt_id does not match receipt"))
    if safe_finalization and safe_finalization.get("closure_id") != safe_closure.get("closure_id"):
    ...
    if safe_finalization and safe_finalization.get("ready_for_delivery_continuation") is not True:
        blockers.append(_blocker("not_ready_for_delivery_continuation", "finalization readiness flag is not true"))
    if safe_finalization and safe_finalization.get("finalized") is not True:
        blockers.append(_blocker("not_finalized", "finalization finalized flag is not true"))
    ...
    return {
        ...
        "audit_package_id": safe_finalization.get("audit_package_id") or safe_audit_package.get("audit_package_id"),
        "status": status,
...
def verify_case_delivery_recovery_finalization_record_from_request(case_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if finalization is None:
        ...
        finalization = finalization_result.get("finalization") if isinstance(finalization_result.get("finalization"), dict) else None
