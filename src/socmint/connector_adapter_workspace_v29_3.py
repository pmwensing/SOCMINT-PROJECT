...
def build_connector_adapter_workspace() -> dict[str, Any]:
    ...
    evaluations = [
        item
        for item in events
        if item.get("event_type") == "adapter_conformance_evaluated"
    ]
    ...
    for contract in active:
        ...
        if not definition.get("capabilities"):
            findings.append(
                {
                    "severity": "high",
                    "key": "adapter_capabilities_missing",
                    "adapter_contract_id": adapter_id,
                }
            )
        if not definition.get("input_schema"):
            findings.append(
                {
                    "severity": "high",
                    "key": "adapter_input_schema_missing",
                    "adapter_contract_id": adapter_id,
                }
            )
        if not definition.get("output_schema"):
            findings.append(
                {
                    "severity": "high",
                    "key": "adapter_output_schema_missing",
                    "adapter_contract_id": adapter_id,
                }
            )
        if not definition.get("authorization_requirements"):
            findings.append(
                {
                    "severity": "medium",
                    "key": "adapter_authorization_requirements_missing",
                    "adapter_contract_id": adapter_id,
                }
            )
        if not definition.get("rate_limit_metadata"):
            findings.append(
                {
                    "severity": "medium",
                    "key": "adapter_rate_limit_metadata_missing",
                    "adapter_contract_id": adapter_id,
                }
            )
        if not definition.get("error_classes"):
            findings.append(
                {
                    "severity": "medium",
                    "key": "adapter_error_classes_missing",
                    "adapter_contract_id": adapter_id,
                }
            )
        if not definition.get("provenance_requirements"):
            findings.append(
                {
                    "severity": "high",
                    "key": "adapter_provenance_requirements_missing",
                    "adapter_contract_id": adapter_id,
                }
            )
        if not definition.get("health_contract"):
            findings.append(
                {
                    "severity": "medium",
                    "key": "adapter_health_contract_missing",
                    "adapter_contract_id": adapter_id,
                }
            )
        if not definition.get("dossier_value_declaration"):
            findings.append(
                {
                    "severity": "high",
                    "key": "adapter_dossier_value_missing",
                    "adapter_contract_id": adapter_id,
                }
            )
        if evaluation and not (evaluation.get("evaluation") or {}).get("conformant"):
            findings.append(
                {
                    "severity": "high",
                    "key": "adapter_nonconformant",
                    "adapter_contract_id": adapter_id,
                    "finding_count": (evaluation.get("evaluation") or {}).get(
                        "finding_count", 0
                    ),
                }
            )
        summaries.append(
            {
                "adapter_contract_id": adapter_id,
                "connector_id": connector_id,
                "connector_name": ((connector or {}).get("definition") or {}).get(
                    "name"
                ),
                "revision": contract.get("revision"),
                "capabilities": definition.get("capabilities") or [],
                "authorization_requirements": definition.get(
                    "authorization_requirements"
                )
                or {},
                "rate_limit_metadata": definition.get("rate_limit_metadata") or {},
                "error_classes": definition.get("error_classes") or [],
                "provenance_requirements": definition.get("provenance_requirements")
                or {},
                "health_contract": definition.get("health_contract") or {},
                "dossier_value_declaration": definition.get("dossier_value_declaration")
                or {},
    ...
    conformance_counts = Counter(
        "conformant"
        if (item.get("evaluation") or {}).get("conformant")
        else "nonconformant"
