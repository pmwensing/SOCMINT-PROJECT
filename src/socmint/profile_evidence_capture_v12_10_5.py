...
def _write_capture_file(
    base_dir: Path, filename: str, content: bytes
) -> dict[str, Any]:
...
def username_needs_rewrite(username: Any) -> bool:
    ...
    return (
        value in ROUTE_PLACEHOLDER_USERNAMES
        or value.endswith(".aspx")
        or not _norm(username)
    )
...
def is_asset_only_url(url: str) -> bool:
    ...
    has_asset_domain = any(
        domain == item or domain.endswith("." + item) for item in ASSET_ONLY_DOMAINS
    )
    has_asset_ext = any(
        path.endswith(ext) or f"{ext}?" in value for ext in NON_PROFILE_EXTENSIONS
    )
    has_asset_hint = bool(path_tokens.intersection(ASSET_DOMAIN_PATH_HINTS)) or any(
        hint in path
        for hint in (
            "/avatar",
            "favicon",
...
def _capture_screenshot_placeholder(metadata: dict[str, Any]) -> bytes:
    label = html.escape(
        str(metadata.get("profile_url") or metadata.get("candidate_id") or "candidate")
    )
...
def capture_candidate_profile_artifacts(
    subject_id: int, candidate: dict[str, Any], live_capture_enabled: bool = False
) -> dict[str, Any]:
    fp = candidate.get("profile_fingerprint") or {}
    profile_url = _norm(fp.get("profile_url"))
    candidate_id = _norm(candidate.get("candidate_id")) or _stable_phash(
        json.dumps(candidate, sort_keys=True)
    )
    capture_id = hashlib.sha256(
        f"{subject_id}:{candidate_id}:{profile_url}".encode()
    ).hexdigest()[:20]
    capture_dir = (
        artifact_root()
        / "profile-evidence-captures"
    ...
    screenshot_file = _write_capture_file(
        capture_dir, "screenshot.svg", _capture_screenshot_placeholder(metadata)
    )
    metadata_file = _write_capture_file(
        capture_dir,
        "metadata.json",
    ...
    manifest = {
        ...
        "chain_of_custody": [
            {
                "event": "profile_capture_created",
                "timestamp": metadata["captured_at"],
                "actor": "socmint-worker",
                "sha256": metadata_file["sha256"],
            }
        ],
    }
    manifest_file = _write_capture_file(
        capture_dir,
...
def suppress_asset_only_candidate(candidate: dict[str, Any]) -> None:
    ...
    if not fp.get("asset_only_url"):
        return
    candidate["identity_score"] = min(
        float(candidate.get("identity_score") or 0.0), 0.05
    )
    candidate["collision_status"] = "asset_only_not_profile"
    candidate.setdefault("negative_reasons", [])
    if (
        "asset/CDN URL is not a profile page; excluded from identity-link assertion"
        not in candidate["negative_reasons"]
    ):
        candidate["negative_reasons"].append(
            "asset/CDN URL is not a profile page; excluded from identity-link assertion"
        )
...
def enrich_candidate_profile_fingerprint(
    subject_id: int, candidate: dict[str, Any], live_capture_enabled: bool = False
) -> dict[str, Any]:
    ...
    linked_urls = sorted(
        set((fp.get("linked_urls") or []) + _extract_linked_urls(context, profile_url))
    )
    ...
    capture = capture_candidate_profile_artifacts(
        subject_id, candidate, live_capture_enabled=live_capture_enabled
    )
    ...
    fp["text_fingerprint_hash"] = hashlib.sha256(
        json.dumps(
            {
                "display_name": fp.get("display_name"),
                "bio_text": fp.get("bio_text"),
    ...
    fp["visual_fingerprint_hash"] = hashlib.sha256(
        json.dumps(
            {
                "avatar_phash": fp.get("avatar_phash"),
                "banner_phash": fp.get("banner_phash"),
                "screenshot_sha256": fp.get("screenshot_sha256"),
            },
            sort_keys=True,
        ).encode()
    ).hexdigest()
...
def enrich_profile_payload_with_evidence(
    profile_payload: dict[str, Any], subject_id: int, live_capture_enabled: bool = False
) -> dict[str, Any]:
    captured = 0
    asset_only = 0
    text_ready = 0
    ...
    for candidate in profile_payload.get("candidates") or []:
        old_username = _norm(
            (candidate.get("profile_fingerprint") or {}).get("username")
        )
        enrich_candidate_profile_fingerprint(
            subject_id, candidate, live_capture_enabled=live_capture_enabled
        )
        fp = candidate.get("profile_fingerprint") or {}
        ...
        suppressed += (
            1 if candidate.get("dossier_assertion_gate", {}).get("suppressed") else 0
        )
        rewritten += 1 if old_username != _norm(fp.get("username")) else 0
        text_ready += (
            1
            if fp.get("display_name") or fp.get("bio_text") or fp.get("linked_urls")
            else 0
        )
        visual_ready += (
            1
            if fp.get("avatar_phash")
            or fp.get("banner_phash")
            or fp.get("screenshot_sha256")
            else 0
        )
