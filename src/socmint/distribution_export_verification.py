...
DISTRIBUTION_EXPORT_VERIFICATION_SCHEMA = (
    "socmint.distribution_export_verification.v10_16_0"
)
REQUIRED_ZIP_FILES = {
    "README.txt",
...
def _zip_status(zip_path: Path, manifest: dict) -> dict:
    try:
        ...
        return {
            "status": "fail",
            "zip_files": [],
            "required_missing": sorted(REQUIRED_ZIP_FILES),
            "artifact_files": [],
    ...
    expected = [
        ...
        if item.get("role") == "dossier_artifact"
    ]
    ...
    return {
        ...
        "actual_artifact_count": len(artifact_files),
        "blockers": blockers,
    }


def verify_distribution_export(
    case_id: str,
    subject_id: str,
    packet_root: str | Path = DISTRIBUTION_PACKET_EXPORT_ROOT,
) -> dict:
    manifest = distribution_packet_export_summary(
        case_id=case_id, subject_id=subject_id, packet_root=packet_root
    )
    if manifest.get("status") == "missing":
        return {
            "schema": DISTRIBUTION_EXPORT_VERIFICATION_SCHEMA,
            "status": "missing",
            "case_id": case_id,
            "subject_id": subject_id,
            "verified": False,
            "blockers": ["missing_distribution_export_manifest"],
            "manifest": manifest,
        }
...
def distribution_export_verification_markdown(
    case_id: str,
    subject_id: str,
    packet_root: str | Path = DISTRIBUTION_PACKET_EXPORT_ROOT,
) -> str:
    result = verify_distribution_export(
        case_id=case_id, subject_id=subject_id, packet_root=packet_root
    )
    lines = [
        f"# Distribution Export Verification — {case_id} / {subject_id}",
        "",
    ...
    for item in result.get("file_checks", []):
        lines.append(
            f"- {item.get('role')} — {item.get('arcname')} — verified={item.get('verified')}"
        )
