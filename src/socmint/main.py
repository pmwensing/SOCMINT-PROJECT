...
from .database import configure_database, create_user, get_dossier, get_user_by_username, save_dossier
...
def run_command(command, tool_name):
    try:
        ...
        if result.returncode != 0:
            error_text = result.stderr.strip() or f"{tool_name} failed with exit code {result.returncode}"
...
def run_social_analyzer(username):
    return run_command(['social-analyzer', '--username', username, '--metadata'], 'Social Analyzer')
...
def run_spiderfoot(target):
    return run_command(['spiderfoot', '-s', target, '-t', 'all', '-f', 'json'], 'Spiderfoot')
...
def run_recon_ng(target):
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile('w', prefix='socmint-recon-', suffix='.rc', delete=False) as f:
            script_path = f.name
            f.write(script)
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'dashboard':
        ...
        parser.add_argument('--port', default=5000, type=int, help='Dashboard bind port')
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'backup':
        parser = argparse.ArgumentParser(description='Create an encrypted SOCMINT database backup')
        parser.add_argument('destination', help='Backup output path')
        parser.add_argument('--no-encrypt', action='store_true', help='Write an unencrypted backup')
        args = parser.parse_args(sys.argv[2:])
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'restore':
        parser = argparse.ArgumentParser(description='Restore a SOCMINT database backup')
        parser.add_argument('source', help='Backup path to restore')
        parser.add_argument('--no-decrypt', action='store_true', help='Restore an unencrypted backup')
        args = parser.parse_args(sys.argv[2:])
        database_url = restore_backup(args.source, encrypted=not args.no_decrypt)
        print(f'Restored backup into {database_url}')
        return

    parser = argparse.ArgumentParser(description='SOCMINT Full Profile Dossier Generator')
    parser.add_argument('target', nargs='?', help='Target username, email, or phone for dossier')
    parser.add_argument('--retrieve', action='store_true', help='Retrieve existing dossier from database')
    parser.add_argument('--serve', action='store_true', help='Launch the local dashboard')
    parser.add_argument('--no-enrich', action='store_true', help='Skip enrichment and media scraping')
    parser.add_argument('--tools', help='Comma-separated tool names to run, for example sherlock,maigret')
    parser.add_argument('--output-json', action='store_true', help='Print only the generated dossier JSON')
    parser.add_argument('--export', help='Write the generated dossier JSON to the given path')
