...
def _record(actor: str, claim_id: str, event: dict[str, Any], ip_address: str | None) -> dict[str, Any]:
    ...
    session = database.Session()
    try:
        row = database.AuditLog(actor=actor, action=ACTION, target_value=claim_id, ip_address=ip_address, details=_canonical(event))
        session.add(row)
        session.commit()
        session.refresh(row)
...
def _score(linkages: list[dict[str, Any]], conflicts: list[dict[str, Any]], limitations: list[str]) -> tuple[int, list[dict[str, Any]]]:
    artifact_ids: set[str] = set()
    observation_ids: set[str] = set()
    for linkage in linkages:
        manifest = linkage.get("source_manifest") or {}
        artifact_ids.update(str(item.get("artifact_id")) for item in manifest.get("artifact_bindings") or [] if item.get("artifact_id"))
        observation_ids.update(str(item.get("observation_id")) for item in manifest.get("observation_bindings") or [] if item.get("observation_id"))
    ...
    components = [
        {"key": "source_linkage_present", "points": 20 if linkages else 0, "detail": len(linkages)},
        {"key": "accepted_artifact_diversity", "points": min(25, len(artifact_ids) * 10), "detail": len(artifact_ids)},
        {"key": "observation_support", "points": min(25, len(observation_ids) * 8), "detail": len(observation_ids)},
        {"key": "resolved_conflict_context", "points": min(10, len(resolved) * 5), "detail": len(resolved)},
        {"key": "unresolved_conflict_penalty", "points": -min(30, len(unresolved) * 15), "detail": len(unresolved)},
        {"key": "declared_limitation_penalty", "points": -min(20, len(limitations) * 5), "detail": len(limitations)},
    ]
    score = max(0, min(79, sum(int(item["points"]) for item in components)))
    return score, components


def assess_confidence(*, actor: str, claim_id: str, methodology: str, limitations: list[str] | None, reason: str, confirmed: bool, ip_address: str | None = None) -> dict[str, Any]:
    claim = find_claim(claim_id)
    methodology = str(methodology or "").strip()
    reason = str(reason or "").strip()
    limitations = sorted({str(item).strip() for item in (limitations or []) if str(item).strip()})
    if claim is None:
    ...
    related_conflicts = [item for item in current_conflicts() if claim_id in {item.get("claim_a_id"), item.get("claim_b_id")}]
    ...
    explanation = {
        ...
        "unresolved_conflict_ids": [item.get("conflict_id") for item in related_conflicts if item.get("resolution") == "unresolved"],
    ...
    binding = {
        ...
        "conflict_event_sha256": [item.get("conflict_event_sha256") for item in related_conflicts],
    ...
    return {**result, "status": "analytic_confidence_assessed", "next_action": "human_analytic_review"}
