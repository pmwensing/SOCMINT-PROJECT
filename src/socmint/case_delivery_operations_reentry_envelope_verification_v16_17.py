...
from .case_delivery_operations_reentry_envelope_v16_16 import build_case_delivery_operations_reentry_envelope
...
CASE_DELIVERY_OPERATIONS_REENTRY_ENVELOPE_VERIFICATION_SCHEMA = "socmint.case_delivery_operations_reentry_envelope_verification.v16_17"
VERSION = "v16.17.0"
...
def verify_case_delivery_operations_reentry_envelope(
    ...
    snapshot_check = deepcopy(resume_snapshot_verification or {}) or verify_case_delivery_recovery_resume_operations_snapshot(
    ...
    if not safe_envelope:
        blockers.append(_blocker("missing_reentry_envelope", "operations re-entry envelope is missing"))
    if snapshot_check.get("verified") is not True:
        blockers.extend(deepcopy(snapshot_check.get("blockers") or []))
        blockers.append(_blocker("resume_snapshot_verification_blocked", "resume snapshot verification did not pass"))
    if snapshot_check.get("safe_to_reenter_operations") is not True:
        blockers.append(_blocker("not_safe_to_reenter_operations", "resume snapshot verification is not safe to re-enter operations"))
    if snapshot_check.get("next_action") != "execute_delivery_operations":
        blockers.append(_blocker("resume_snapshot_next_action_mismatch", "resume snapshot verification next_action is not execute_delivery_operations"))

    expected_payload_hash = sha256_text(canonical_json(_envelope_payload(safe_envelope))) if safe_envelope else None
    if safe_envelope and safe_envelope.get("payload_sha256") != expected_payload_hash:
        blockers.append(_blocker("payload_hash_mismatch", "re-entry envelope payload hash does not match envelope fields"))

    expected_envelope_id = (
        sha256_text(canonical_json({**_envelope_payload(safe_envelope), "payload_sha256": safe_envelope.get("payload_sha256")}))
        if safe_envelope
        else None
    )
    if safe_envelope and safe_envelope.get("reentry_envelope_id") != expected_envelope_id:
        blockers.append(_blocker("reentry_envelope_id_mismatch", "re-entry envelope id does not match canonical envelope payload"))

    if safe_envelope and safe_envelope.get("queue_id") != safe_recovery.get("queue_id"):
        blockers.append(_blocker("queue_id_mismatch", "re-entry envelope queue_id does not match recovery queue"))
    if safe_envelope and safe_envelope.get("case_id") != safe_recovery.get("case_id"):
        blockers.append(_blocker("case_id_mismatch", "re-entry envelope case_id does not match recovery queue"))
    if safe_envelope and safe_envelope.get("resume_snapshot_id") != safe_resume_snapshot.get("resume_snapshot_id"):
        blockers.append(_blocker("resume_snapshot_id_mismatch", "re-entry envelope resume_snapshot_id does not match resume snapshot"))
    if safe_envelope and snapshot_check.get("resume_snapshot_id") and safe_envelope.get("resume_snapshot_id") != snapshot_check.get("resume_snapshot_id"):
        blockers.append(_blocker("resume_snapshot_verification_id_mismatch", "re-entry envelope resume_snapshot_id does not match resume snapshot verification"))
    if safe_envelope and safe_envelope.get("resume_snapshot_verification_status") != snapshot_check.get("status"):
        blockers.append(_blocker("resume_snapshot_verification_status_mismatch", "re-entry envelope verification status does not match resume snapshot verification"))
    if safe_envelope and safe_envelope.get("resume_snapshot_verified") is not True:
        blockers.append(_blocker("resume_snapshot_not_verified", "re-entry envelope resume_snapshot_verified flag is not true"))
    if safe_envelope and safe_envelope.get("safe_to_reenter_operations") is not True:
        blockers.append(_blocker("envelope_not_safe_to_reenter_operations", "re-entry envelope safe_to_reenter_operations flag is not true"))
    if safe_envelope and safe_envelope.get("ready_for_operations_dispatch") is not True:
        blockers.append(_blocker("not_ready_for_operations_dispatch", "re-entry envelope dispatch readiness flag is not true"))
    if safe_envelope and safe_envelope.get("next_action") != NEXT_ACTION:
        blockers.append(_blocker("next_action_mismatch", "re-entry envelope next_action is not dispatch_delivery_operations"))
    ...
    return {
        ...
        "resume_snapshot_id": safe_envelope.get("resume_snapshot_id") or safe_resume_snapshot.get("resume_snapshot_id"),
        "status": status,
        "verified": not blockers,
        "ready_for_operations_dispatch": not blockers,
        "next_action": NEXT_ACTION if not blockers else "resolve_operations_reentry_envelope",
...
def verify_case_delivery_operations_reentry_envelope_from_request(case_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    resume_snapshot_verification = safe_payload.get("resume_snapshot_verification") if isinstance(safe_payload.get("resume_snapshot_verification"), dict) else None
    reentry_envelope = safe_payload.get("reentry_envelope") if isinstance(safe_payload.get("reentry_envelope"), dict) else None
    if reentry_envelope is None:
        ...
        reentry_envelope = envelope_result.get("reentry_envelope") if isinstance(envelope_result.get("reentry_envelope"), dict) else None
