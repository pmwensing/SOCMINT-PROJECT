...
from .v10_26_final_delivery_audit_trail import (
    build_final_delivery_audit_trail_from_request,
)

FINAL_DELIVERY_EVIDENCE_CAPSULE_SCHEMA = (
    "socmint.v10_27.final_delivery_evidence_capsule"
)
FINAL_DELIVERY_EVIDENCE_CAPSULE_SUMMARY_SCHEMA = (
    "socmint.v10_27.final_delivery_evidence_capsule.summary"
)
VERSION = "v10.27.0"
...
def _console(audit_trail: dict[str, Any]) -> dict[str, Any]:
    return (
        audit_trail.get("console")
        if isinstance(audit_trail.get("console"), dict)
        else {}
    )
...
def summarize_evidence_capsule(capsule: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "audit_id": (capsule.get("audit_trail") or {}).get("audit_id")
        if isinstance(capsule.get("audit_trail"), dict)
        else None,
        "export_available": bool(
            (capsule.get("operator_receipt") or {}).get("export_available")
        )
...
def _capsule_core(audit_trail: dict[str, Any]) -> dict[str, Any]:
    ...
    receipt = (
        audit_trail.get("operator_receipt")
        if isinstance(audit_trail.get("operator_receipt"), dict)
        else {}
    )
    return {
        ...
        "card_types": [
            str(card.get("type"))
            for card in console.get("cards") or []
            if isinstance(card, dict) and card.get("type")
        ],
        "command_ids": [
            str(command.get("id"))
            for command in console.get("commands") or []
...
def build_final_delivery_evidence_capsule_from_audit_trail(
...
def build_final_delivery_evidence_capsule_from_request(
...
def build_final_delivery_evidence_capsule_summary_from_request(
    payload: dict[str, Any],
) -> dict[str, Any]:
    return deepcopy(
        build_final_delivery_evidence_capsule_from_request(payload).get("summary") or {}
    )
