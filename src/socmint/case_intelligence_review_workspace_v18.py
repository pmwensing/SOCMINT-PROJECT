...
CASE_INTELLIGENCE_REVIEW_WORKSPACE_SCHEMA = "socmint.case_intelligence_review_workspace.v18_0"
VERSION = "v18.0.0"
...
def _summary(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    unresolved_claims = sum(1 for item in claims if item.get("status") not in {"supported", "verified", "resolved"})
    low_confidence = sum(1 for item in identities + entities if float(item.get("confidence") or 0) < 0.7)
    return {
...
def _evidence_claim_panel(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    rows = []
    for claim in claims:
        links = claim.get("evidence_ids") if isinstance(claim.get("evidence_ids"), list) else []
        missing = [item for item in links if item not in evidence_ids]
        rows.append({**claim, "linked_evidence_count": len(links), "missing_evidence_ids": missing})
    return {
        "evidence": evidence,
        "claims": rows,
        "unsupported_claim_count": sum(1 for item in rows if not item.get("evidence_ids")),
...
def _identity_panel(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in candidates:
        item["review_required"] = float(item.get("confidence") or 0) < 0.7 or item.get("status") in {"candidate", "unresolved"}
    return {
...
def _timeline_panel(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    events.sort(key=lambda item: str(item.get("occurred_at") or item.get("timestamp") or ""))
    contradictions = _items(payload, "contradictions")
    return {
        "events": events,
        "contradictions": contradictions,
        "open_contradiction_count": sum(1 for item in contradictions if item.get("status") not in {"resolved", "dismissed"}),
    }
...
def build_case_intelligence_review_workspace(
    ...
    *,
    ...
    if summary["unresolved_claim_count"]:
        blockers.append({"key": "unresolved_claims", "count": summary["unresolved_claim_count"]})
    if summary["low_confidence_resolution_count"]:
        blockers.append({"key": "low_confidence_resolutions", "count": summary["low_confidence_resolution_count"]})
    if summary["contradiction_count"]:
        blockers.append({"key": "contradictions", "count": summary["contradiction_count"]})
    status = "ready_for_analyst_decision" if not blockers else "review_required"
    entries = [deepcopy(item) for item in (history or []) if isinstance(item, dict) and item.get("case_id") == case_id]
    entries.sort(key=lambda item: str(item.get("recorded_at") or ""), reverse=True)
    return {
        "schema": CASE_INTELLIGENCE_REVIEW_WORKSPACE_SCHEMA,
        "version": VERSION,
        "case_id": case_id,
        "case": deepcopy(safe.get("case") or {"case_id": case_id, "title": safe.get("title") or case_id}),
        "status": status,
        "summary": summary,
        ...
        "review_history": {"entries": entries, "entry_count": len(entries), "operator": operator, "persistence": "flask_session_only"},
        "next_action": "record_analyst_decision" if not blockers else "resolve_case_review_items",
    }
...
def record_case_review_decision(
    ...
    *,
    ...
    if decision not in DECISIONS:
        return {"status": "blocked", "blockers": [{"key": "unsupported_decision", "detail": decision or "missing"}], "next_action": "choose_supported_review_decision"}
...
def append_case_review_history(history: list[dict[str, Any]] | None, decision: dict[str, Any]) -> list[dict[str, Any]]:
...
def build_v18_product_review_checkpoint(root: str | Path = ".", *, routes: list[Any] | None = None) -> dict[str, Any]:
    root_path = Path(root)
    required = [
    ...
    notes = [f"release/V18_{index}_{name}.md" for index, name in (
    ...
    migrations = [str(path) for directory in (root_path / "migrations", root_path / "alembic") if directory.exists() for path in directory.rglob("*v18*")]
    if migrations:
        blockers.append({"key": "unexpected_migration", "detail": ", ".join(migrations)})
    return {
        ...
        "next_action": "run_case_intelligence_browser_e2e" if not blockers else "resolve_v18_product_review_blockers",
    }
