...
CASE_DELIVERY_RECOVERY_CLOSURE_AUDIT_PACKAGE_SCHEMA = "socmint.case_delivery_recovery_closure_audit_package.v16_8"
CASE_DELIVERY_RECOVERY_CLOSURE_AUDIT_RESULT_SCHEMA = "socmint.case_delivery_recovery_closure_audit_package.v16_8.result"
VERSION = "v16.8.0"
...
def _audit_payload(
    ...
    return {
        ...
        "version": VERSION,
        "case_id": recovery.get("case_id") or receipt.get("case_id") or closure.get("case_id"),
        "queue_id": recovery.get("queue_id") or receipt.get("queue_id") or closure.get("queue_id"),
...
def _package_blockers(recovery: dict[str, Any], receipt: dict[str, Any], closure: dict[str, Any], closure_verification: dict[str, Any]) -> list[dict[str, Any]]:
    blockers = []
    if not recovery:
    ...
    if not closure_verification:
        blockers.append(_blocker("missing_closure_verification", "closure verification artifact is missing"))
    ...
    if recovery and receipt and recovery.get("queue_id") != receipt.get("queue_id"):
        blockers.append(_blocker("receipt_queue_mismatch", "receipt queue_id does not match recovery queue_id"))
    if recovery and closure and recovery.get("queue_id") != closure.get("queue_id"):
        blockers.append(_blocker("closure_queue_mismatch", "closure queue_id does not match recovery queue_id"))
    if receipt and closure and receipt.get("receipt_id") != closure.get("receipt_id"):
        blockers.append(_blocker("closure_receipt_mismatch", "closure receipt_id does not match receipt"))
    return blockers
...
def build_case_delivery_recovery_closure_audit_package(
    ...
    *,
    ...
    if not safe_closure:
        closure_result = build_case_delivery_recovery_closure_record(safe_recovery, safe_receipt, closer=package_owner)
    ...
    if not safe_closure_verification:
        safe_closure_verification = verify_case_delivery_recovery_closure_record(safe_closure, safe_recovery, safe_receipt)

    blockers = _package_blockers(safe_recovery, safe_receipt, safe_closure, safe_closure_verification)
    if blockers:
        return {
            "schema": CASE_DELIVERY_RECOVERY_CLOSURE_AUDIT_RESULT_SCHEMA,
            "version": VERSION,
            "case_id": safe_recovery.get("case_id") or safe_receipt.get("case_id") or safe_closure.get("case_id"),
            "queue_id": safe_recovery.get("queue_id") or safe_receipt.get("queue_id") or safe_closure.get("queue_id"),
            "status": "blocked",
            "packaged": False,
            "audit_package": None,
    ...
    payload = _audit_payload(safe_recovery, safe_receipt, safe_closure, safe_closure_verification, package_owner)
    package_hash = sha256_text(canonical_json(payload))
    audit_package = {
        **payload,
        "package_sha256": package_hash,
        "audit_package_id": sha256_text(canonical_json({**payload, "package_sha256": package_hash})),
    }
    return {
        "schema": CASE_DELIVERY_RECOVERY_CLOSURE_AUDIT_RESULT_SCHEMA,
        "version": VERSION,
...
def build_case_delivery_recovery_closure_audit_package_from_request(case_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    package_owner = safe_payload.get("package_owner") if isinstance(safe_payload.get("package_owner"), str) else None
