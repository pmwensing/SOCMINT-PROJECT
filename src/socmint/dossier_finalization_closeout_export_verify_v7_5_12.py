...
CLOSEOUT_EXPORT_VERIFY_SCHEMA = (
    "socmint.v7_5_12.dossier_finalization_closeout_export_verification"
)
CLOSEOUT_EXPORT_VERIFY_SUMMARY_SCHEMA = (
...
def _finding(
    ...
    return {
        ...
        "action": ACTIONS.get(
            code,
            "Review this closeout export verification finding before archive handoff.",
        ),
    }
...
def verify_closeout_export_files(files: dict[str, bytes]) -> dict[str, Any]:
    ...
    manifest = (
        _load_json(safe_files, "manifest.json", failures, "invalid_manifest_json")
    ...
    if manifest and manifest.get("schema") != CLOSEOUT_EXPORT_MANIFEST_SCHEMA:
        failures.append(
            _finding(
                ...
                "Manifest schema does not match v7.5.11 closeout export manifest schema.",
                path="manifest.json",
            )
        )
    ...
    closeout_report = (
        _load_json(
            safe_files, "closeout_report.json", failures, "invalid_closeout_report_json"
        )
        if "closeout_report.json" in safe_files
        else {}
    )
    summary = (
        _load_json(
            ...
            "closeout_report_summary.json",
            failures,
            "invalid_closeout_summary_json",
        )
        if "closeout_report_summary.json" in safe_files
        else {}
    )
    report["closeout_action"] = (
        closeout_report.get("closeout_action") if closeout_report else None
    )
    report["verification_status"] = (
        closeout_report.get("verification_status") if closeout_report else None
    )
    ...
    if closeout_report and summary:
        if closeout_report.get("closeout_action") != summary.get("closeout_action"):
            failures.append(
                _finding(
                    "fail",
                    "closeout_action_mismatch",
                    "Closeout summary action differs from closeout report action.",
                    path="closeout_report_summary.json",
                )
            )
        if closeout_report.get("verification_status") != summary.get(
            "verification_status"
        ):
            failures.append(
                _finding(
                    ...
                    "Closeout summary verification status differs from closeout report verification status.",
                    path="closeout_report_summary.json",
                )
            )
        if closeout_report.get("closeout_action") in {
            ...
            warnings.append(
                _finding(
                    "warn",
                    "non_closeout_ready",
                    "Closeout export is structurally intact but not closeout-ready.",
    ...
    if not safe_files.get("closeout_report.md", b"").strip():
        warnings.append(
            _finding(
                "warn",
                "empty_closeout_markdown",
                "Closeout Markdown is empty.",
                path="closeout_report.md",
            )
        )
...
def verify_closeout_export_bundle(bundle: dict[str, Any]) -> dict[str, Any]:
    return verify_closeout_export_files(
        build_closeout_export_bundle_files(deepcopy(bundle or {}))
    )
