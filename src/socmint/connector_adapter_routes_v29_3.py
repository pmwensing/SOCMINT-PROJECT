...
def register_connector_adapter_routes_v29_3(app):
    ...
    def connector_adapter_workspace_get_v29_3():
        ...
        if not actor_is_administrator(actor):
            return render_template("connector_adapter_contract_v29_3.html", title="Connector Normalization and Adapter Contract", payload={"status":"forbidden","error":"administrator required","adapter_summaries":[],"adapter_findings":[],"adapter_history":[]}), 403
        return render_template("connector_adapter_contract_v29_3.html", title="Connector Normalization and Adapter Contract", payload=build_connector_adapter_workspace())
    ...
    def api_connector_adapter_create_post_v29_3():
        ...
        if error: return error
        payload = _payload()
        result = create_adapter_contract(actor=actor, connector_id=str(payload.get("connector_id") or ""), capabilities=payload.get("capabilities"), input_schema=payload.get("input_schema"), output_schema=payload.get("output_schema"), authorization_requirements=payload.get("authorization_requirements"), rate_limit_metadata=payload.get("rate_limit_metadata"), error_classes=payload.get("error_classes"), provenance_requirements=payload.get("provenance_requirements"), health_contract=payload.get("health_contract"), dossier_value_declaration=payload.get("dossier_value_declaration"), reason=str(payload.get("reason") or ""), confirmed=payload.get("confirmed") is True, ip_address=request.remote_addr)
    ...
    def api_connector_adapter_revise_post_v29_3(adapter_contract_id: str):
        ...
        if error: return error
        payload = _payload()
        result = revise_adapter_contract(adapter_contract_id, actor=actor, definition=payload.get("definition"), reason=str(payload.get("reason") or ""), confirmed=payload.get("confirmed") is True, ip_address=request.remote_addr)
