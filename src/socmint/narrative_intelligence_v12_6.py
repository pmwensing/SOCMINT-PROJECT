...
def evidence_to_timeline(
    root: str | None = None, subject_id: int | None = None
) -> dict[str, Any]:
...
def extract_claims(events: list[dict[str, Any]]) -> list[Claim]:
    ...
    for index, event in enumerate(events, start=1):
        ...
        filename = (
            (raw.get("metadata") or {}).get("filename")
            or event.get("title")
            or "unknown"
        )
        ...
        claims.append(
            Claim(
                ...
                confidence=0.85
                if hash_verified
                else float(event.get("confidence") or 0.5),
            )
        )
        claims.append(
            Claim(
...
def communication_graph(events: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for event in events:
        ...
        if evidence_id:
            nodes.setdefault(
                evidence_id,
                {"id": evidence_id, "type": "evidence", "label": evidence_id},
            )
...
def validate_claims(
    ...
    event_by_evidence = {
        event.get("evidence_id"): event for event in events if event.get("evidence_id")
    }
    validated = []
    for claim in claims:
        ...
        hash_verified = bool(
            (((event or {}).get("raw") or {}).get("court_safe") or {}).get(
                "hash_verified"
            )
        )
        ...
        maturity = (
            "court-grade"
            if hash_verified and confidence >= 0.8
            else "dossier-grade"
            if confidence >= 0.7
            else "review"
        )
        validated.append(
            {
                **claim,
                "validated": hash_verified or confidence >= 0.7,
...
def narrative_confidence(
    ...
    if not events:
        return {
            "score": 0.0,
            "rating": "insufficient",
            "factors": ["No narrative events found."],
        }
    avg_event = sum(float(event.get("confidence") or 0.0) for event in events) / len(
        events
    )
    court_grade = sum(1 for claim in claims if claim.get("maturity") == "court-grade")
    contradiction_penalty = min(
        0.35, contradictions.get("contradiction_count", 0) * 0.08
    )
    score = max(
        0.0,
        min(
            1.0,
            (avg_event * 0.55)
            + (min(1.0, court_grade / max(1, len(events))) * 0.35)
            + 0.1
    ...
    return {
        ...
        "rating": rating,
        "factors": [
            f"avg_event_confidence={avg_event:.2f}",
            f"court_grade_claims={court_grade}",
            f"contradictions={contradictions.get('contradiction_count', 0)}",
        ],
    }
...
def generate_court_narrative(
    ...
    for event in events[:50]:
        lines.append(
            f"- {event.get('timestamp')} — {event.get('title')} ({event.get('source')}; confidence {event.get('confidence')})"
        )
    lines.extend(["", "## Validated Claims"])
    for claim in validated.get("validated_claims", [])[:50]:
        lines.append(
            f"- {claim.get('maturity')}: {claim.get('subject')} {claim.get('predicate')} = {claim.get('value')} [confidence {claim.get('confidence')}]"
        )
    ...
    if contradictions.get("contradictions"):
        for issue in contradictions.get("contradictions", [])[:25]:
            lines.append(
                f"- {issue.get('severity')}: {issue.get('type')} on {issue.get('subject')} / {issue.get('predicate')}"
            )
    else:
        lines.append(
            "- No deterministic contradictions detected in the current claim set."
        )
    lines.extend(
        [
            "",
            "## Court/Lawyer Use Note",
            "This narrative is a structured analytical aid. It should be reviewed by an analyst before filing, service, or legal use.",
        ]
    )
    markdown = "\n".join(lines) + "\n"
...
def story_reconstruction_payload(
    root: str | None = None, subject_id: int | None = None
) -> dict[str, Any]:
    ...
    confidence = narrative_confidence(
        events, validated.get("validated_claims", []), contradictions
    )
