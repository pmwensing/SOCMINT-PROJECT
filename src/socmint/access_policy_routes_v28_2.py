...
def register_access_policy_routes_v28_2(app):
    ...
    def access_policy_workspace_get_v28_2():
        ...
        if not actor_is_administrator(actor):
            return render_template("access_policy_administration_v28_2.html", title="Role, Permission, and Access Policy Management", payload={"status":"forbidden","error":"administrator required","roles":[],"access_rules":[],"permission_matrix":[],"least_privilege_findings":[],"access_policy_history":[]}), 403
        return render_template("access_policy_administration_v28_2.html", title="Role, Permission, and Access Policy Management", payload=build_access_policy_workspace())
    ...
    def api_access_policy_evaluate_get_v28_2():
        ...
        if error:
            return error
        return jsonify(evaluate_effective_access(str(request.args.get("username") or ""), str(request.args.get("case_id") or "")))
