...
CASE_DELIVERY_HANDOFF_VERIFICATION_SCHEMA = "socmint.case_delivery_handoff_verification.v15_2"
VERSION = "v15.2.0"
...
def _expected_file_rows(package: dict[str, Any]) -> dict[str, dict[str, Any]]:
    workspace = package.get("workspace") if isinstance(package.get("workspace"), dict) else {}
    gate = package.get("gate") if isinstance(package.get("gate"), dict) else {}
    receipt = package.get("operator_receipt") if isinstance(package.get("operator_receipt"), dict) else {}
    contents = {
...
def _manifest_rows(package: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    files = manifest.get("files") if isinstance(manifest.get("files"), list) else package.get("files")
    rows = {}
...
def verify_case_delivery_handoff_package(package: dict[str, Any]) -> dict[str, Any]:
    ...
    for path in missing:
        blockers.append(_blocker("missing_manifest_file", f"{path} is missing from the handoff manifest", path))
    ...
    workspace = safe_package.get("workspace") if isinstance(safe_package.get("workspace"), dict) else {}
    gate = safe_package.get("gate") if isinstance(safe_package.get("gate"), dict) else {}
    workspace_gate = workspace.get("gate") if isinstance(workspace.get("gate"), dict) else {}
    gate_decision = gate.get("decision")
    package_decision = safe_package.get("gate_decision")
    expected_disposition = "deliver" if gate_decision == "READY_FOR_DELIVERY" else "hold"

    if gate != workspace_gate:
        blockers.append(_blocker("gate_mismatch", "package gate does not match workspace gate", "delivery_gate.json"))
    if package_decision != gate_decision:
        blockers.append(_blocker("gate_decision_mismatch", "package gate decision does not match the gate"))
    if safe_package.get("disposition") != expected_disposition:
        blockers.append(_blocker("disposition_mismatch", "package disposition does not match gate decision"))
    if safe_package.get("case_id") != workspace.get("case_id"):
        blockers.append(_blocker("case_id_mismatch", "package case_id does not match workspace case_id"))
...
def verify_case_delivery_handoff_package_from_request(case_id: str, payload: dict[str, Any]) -> dict[str, Any]:
