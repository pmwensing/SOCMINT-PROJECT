...
def register_collection_job_routes_v29_1(app):
    ...
    def collection_job_workspace_get_v29_1():
        ...
        if not actor_is_administrator(actor):
            return render_template(
                "collection_job_contract_v29_1.html",
                title="Collection Job Contract and State Machine",
                payload={
                    "status": "forbidden",
        ...
        return render_template(
            "collection_job_contract_v29_1.html",
            title="Collection Job Contract and State Machine",
            payload=build_collection_job_workspace(),
        )
    ...
    def api_collection_job_create_post_v29_1():
        ...
        if error:
            return error
        payload = _payload()
        result = create_collection_job_contract(
            actor=actor,
            ...
            authorization_binding=payload.get("authorization_binding"),
            purpose=str(payload.get("purpose") or ""),
            idempotency_key=str(payload.get("idempotency_key") or ""),
            legacy_scan_job_id=payload.get("legacy_scan_job_id"),
    ...
    def api_collection_job_transition_post_v29_1(collection_job_id: str):
        ...
        if to_state == "authorized" and not _policy_binding_valid(
        ...
        result = transition_collection_job(
            collection_job_id=collection_job_id,
            actor=actor,
            to_state=to_state,
