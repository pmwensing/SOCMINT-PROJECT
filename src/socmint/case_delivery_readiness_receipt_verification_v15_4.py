...
def verify_case_delivery_readiness_receipt(
    ...
    if not package_verification.get("verified"):
        blockers.append(
            _blocker("package_unverified", "handoff package verification did not pass")
        )
    ...
    if safe_receipt and safe_receipt.get("payload_sha256") != expected_payload_hash:
        blockers.append(
            _blocker(
                ...
                "receipt payload hash does not match receipt fields",
            )
        )

    expected_signature = (
        sha256_text(canonical_json(_signature_base(safe_receipt)))
        if safe_receipt
        else None
    )
    if safe_receipt and safe_receipt.get("signature_sha256") != expected_signature:
        blockers.append(
            _blocker(
                ...
                "receipt signature hash does not match signature base",
            )
        )
    ...
    expected_receipt_id = (
        sha256_text(
            canonical_json(
                {**_signature_base(safe_receipt), "type": "delivery_readiness_receipt"}
            )
        )
        if safe_receipt
        else None
    ...
    if safe_receipt and safe_receipt.get("receipt_id") != expected_receipt_id:
        blockers.append(
            _blocker("receipt_id_mismatch", "receipt id does not match signature base")
        )

    if safe_receipt and safe_receipt.get("package_id") != safe_package.get(
        "package_id"
    ):
        blockers.append(
            _blocker(
                "package_id_mismatch",
                "receipt package_id does not match handoff package",
            )
        )
    if safe_receipt and safe_receipt.get("case_id") != safe_package.get("case_id"):
        blockers.append(
            _blocker(
                "case_id_mismatch", "receipt case_id does not match handoff package"
            )
        )
    if safe_receipt and safe_receipt.get("verified") is not True:
        blockers.append(
            _blocker("receipt_not_verified", "receipt verified flag is not true")
        )
    if safe_receipt and safe_receipt.get("accepted_for_delivery") is not True:
        blockers.append(
            _blocker(
                "not_accepted_for_delivery", "receipt is not accepted for delivery"
            )
        )
...
def verify_case_delivery_readiness_receipt_from_request(
    ...
    if receipt is None:
        receipt_result = build_case_delivery_readiness_receipt(
            package, issuer=safe_payload.get("issuer")
        )
        receipt = (
