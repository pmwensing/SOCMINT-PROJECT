...
def _findings(limit: int = 5000) -> list[dict[str, Any]]:
    ...
    try:
        rows = (
            session.query(database.Finding)
            .order_by(database.Finding.created_at.desc())
            .limit(limit)
            .all()
        )
        return [
...
def _results(limit: int = 5000) -> list[dict[str, Any]]:
    ...
    try:
        rows = (
            session.query(database.Result)
            .order_by(database.Result.timestamp.desc())
            .limit(limit)
            .all()
        )
        return [
...
def _media(limit: int = 5000) -> list[dict[str, Any]]:
    ...
    try:
        rows = (
            session.query(database.Media)
            .order_by(database.Media.created_at.desc())
            .limit(limit)
            .all()
        )
        return [
...
def build_collection_operations_workspace(
    *, stale_after_hours: int = 24
    ...
    target_runs = Counter(
        item["target_id"] for item in runs if item["target_id"] is not None
    )
    ...
    for job in jobs:
        ...
        retryable = job["status"] in {"failed", "error", "blocked"} and bool(
            job["target_value"] and job["tools"]
        )
        retry_eligibility.append(
            {
                "job_id": job["job_id"],
                "eligible": retryable,
                "reason": "failed_with_reconstructable_input"
                if retryable
                else "not_retryable_from_current_state",
    ...
    for run in runs:
        grouped_runs[
            (str(run["connector"]), str(run["target_type"]), str(run["target_value"]))
        ].append(run)
    ...
    provenance_findings = [
        {
            "severity": "high",
            "key": "incomplete_connector_run_provenance",
            "run_id": item["run_id"],
        }
    ...
    scope_findings = [
        {
            "severity": "medium",
            "key": "collection_without_requesting_actor",
            "job_id": item["job_id"],
        }
        for item in jobs
    ...
    findings_all = [
        ...
        *[
            {
                "severity": "high",
                "key": "collection_job_stale",
                "job_id": item["job_id"],
            }
            for item in stale_jobs
        ],
        *[
            {"severity": "low", "key": "duplicate_collection_runs", **item}
            for item in duplicate_run_groups
        ],
        *provenance_findings,
        *scope_findings,
    ]
    ...
    return {
        ...
        "connector_status_counts": dict(
            sorted(Counter(item["status"] for item in runs).items())
        ),
        ...
        "observation_summary": {
            "finding_count": len(findings),
            "spine_observation_count": optional.get("spine_observations", {}).get(
                "record_count", 0
            ),
        },
        "provenance_summary": {
            "complete_connector_run_count": sum(
                item["provenance_complete"] for item in collection_inventory
            ),
            "incomplete_connector_run_count": sum(
                not item["provenance_complete"] for item in collection_inventory
            ),
            "media_checksum_complete_count": sum(
                item["checksum_present"] for item in media
            ),
            "media_checksum_incomplete_count": sum(
                not item["checksum_present"] for item in media
            ),
        },
        ...
        "dossier_value_summary": {
            "contributing_run_count": sum(
                item["dossier_value"] == "contributing" for item in collection_inventory
            ),
            "unproven_run_count": sum(
                item["dossier_value"] == "unproven" for item in collection_inventory
            ),
            "spine_assertion_count": optional.get("spine_dossier_assertions", {}).get(
                "record_count", 0
            ),
        },
