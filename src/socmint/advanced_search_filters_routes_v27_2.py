...
def _build():
    return build_advanced_search_filters(
        request.args.get("q", ""),
        record_types=_list("type"),
        case_ids=_list("case_id"),
        actors=_list("actor"),
        statuses=_list("status"),
        stages=_list("stage"),
        source_actions=_list("source_action"),
        confidences=_list("confidence"),
        priorities=_list("priority"),
        date_from=request.args.get("date_from"),
        date_to=request.args.get("date_to"),
        include_terms=_list("include_term"),
        exclude_terms=_list("exclude_term"),
...
def register_advanced_search_filters_routes_v27_2(app):
    ...
    def advanced_search_filters_get_v27_2():
        ...
        return render_template(
            "advanced_search_filters_v27_2.html",
            title="Advanced Filters and Search Facets",
            payload=_build(),
        )
