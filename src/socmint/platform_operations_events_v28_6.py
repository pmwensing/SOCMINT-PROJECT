...
def current_incidents() -> list[dict[str, Any]]:
    ...
    for event in history():
        ...
        if event_type == "incident_opened":
            incidents[incident_id] = {**event, "incident_status": "open", "acknowledgements": []}
            ...
            if event_type == "incident_acknowledged":
                ...
                incident["acknowledgements"] = [*incident.get("acknowledgements", []), event]
            elif event_type == "incident_resolved":
                incident["incident_status"] = "resolved"
                incident["resolution_event"] = event
            incidents[incident_id] = incident
    return sorted(incidents.values(), key=lambda item: str(item.get("recorded_at") or ""), reverse=True)


def find_incident(incident_id: str) -> dict[str, Any] | None:
    return next((item for item in current_incidents() if item.get("incident_id") == incident_id), None)


def open_incident(*, actor: str, title: str, severity: str, component: str, description: str, source_binding: Any, reason: str, confirmed: bool, ip_address: str | None = None) -> dict[str, Any]:
    ...
    definition = {"title": title, "severity": severity, "component": component, "description": str(description or "").strip(), "source_binding": binding}
    content = {"event_type": "incident_opened", "definition": definition, "definition_sha256": _sha(definition), "reason": reason}
    digest = _sha(content)
    event = {
        "schema": SCHEMA,
        "version": VERSION,
    ...
    return {**result, "status": "operational_incident_opened", "next_action": "acknowledge_operational_incident"}


def acknowledge_incident(incident_id: str, *, actor: str, note: str, reason: str, confirmed: bool, ip_address: str | None = None) -> dict[str, Any]:
    ...
    if not reason:
        return blocked("administrative_reason_required")
    binding = {"incident_id": incident_id, "incident_event_id": incident.get("incident_event_id"), "incident_event_sha256": incident.get("incident_event_sha256")}
    content = {"event_type": "incident_acknowledged", "incident_id": incident_id, "note": str(note or "").strip(), "reason": reason, "incident_binding": binding, "incident_binding_sha256": _sha(binding)}
    digest = _sha(content)
    event = {"schema": SCHEMA, "version": VERSION, **content, "incident_event_id": f"incident-event-{digest[:24]}", "incident_event_sha256": digest, "job_execution_performed": False, "configuration_mutated": False, "audit_records_mutated": False}
    result = _record(ACTIONS[1], actor, incident_id, event, ip_address)
    return {**result, "status": "operational_incident_acknowledged", "next_action": "resolve_operational_incident"}


def resolve_incident(incident_id: str, *, actor: str, resolution: str, reason: str, confirmed: bool, ip_address: str | None = None) -> dict[str, Any]:
    incident = find_incident(incident_id)
    if incident is None or incident.get("incident_status") == "resolved":
        return blocked("active_operational_incident_required")
    if confirmed is not True:
    ...
    if not reason:
        return blocked("administrative_reason_required")
    binding = {"incident_id": incident_id, "incident_event_id": incident.get("incident_event_id"), "incident_event_sha256": incident.get("incident_event_sha256")}
    content = {"event_type": "incident_resolved", "incident_id": incident_id, "resolution": resolution, "reason": reason, "incident_binding": binding, "incident_binding_sha256": _sha(binding)}
    digest = _sha(content)
    event = {"schema": SCHEMA, "version": VERSION, **content, "incident_event_id": f"incident-event-{digest[:24]}", "incident_event_sha256": digest, "job_execution_performed": False, "configuration_mutated": False, "audit_records_mutated": False}
    result = _record(ACTIONS[2], actor, incident_id, event, ip_address)
    return {**result, "status": "operational_incident_resolved", "next_action": "review_operational_history"}
