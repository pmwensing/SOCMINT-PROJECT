...
from .platform_operations_events_v28_6 import SCHEMA, VERSION, current_incidents, history
        ...
        return [
            {
                ...
                "job_type": getattr(row, "job_type", None) or getattr(row, "scan_type", None),
            }
            for row in rows
        ]
    finally:
...
def _audit_snapshot(limit: int = 2000) -> dict[str, Any]:
    ...
    session = database.Session()
    try:
        rows = session.query(database.AuditLog).order_by(database.AuditLog.id.asc()).limit(limit).all()
        ids = [int(row.id) for row in rows if row.id is not None]
        gaps = []
        for left, right in zip(ids, ids[1:]):
            if right != left + 1:
                gaps.append({"after_id": left, "before_id": right, "missing_count": right - left - 1})
...
def _configuration_state() -> dict[str, Any]:
    ...
    return {
        "variables": {key: {"configured": bool(os.getenv(key)), "value_exposed": False} for key in keys},
...
def build_platform_operations_workspace(*, stale_after_hours: int = 24) -> dict[str, Any]:
    ...
    failed_jobs = [item for item in jobs if item["status"] in {"failed", "error", "blocked"} or item["error_present"]]
    stalled_jobs = []
    for item in jobs:
        if item["status"] not in {"queued", "pending", "running", "in_progress"}:
            continue
        stamp = item.get("updated_at") or item.get("started_at") or item.get("created_at")
        try:
            parsed = datetime.fromisoformat(str(stamp).replace("Z", "+00:00")) if stamp else None
        except ValueError:
            parsed = None
    ...
    for name, items in sorted(connector_groups.items()):
        ...
        connector_continuity.append({
            ...
            "error_count": sum(item["error_present"] or item["status"] in {"failed", "error"} for item in items),
        })

    database_ready = bool(database.check_ready())
    storage = _storage_state()
    configuration = _configuration_state()
    open_incidents = [item for item in incidents if item.get("incident_status") != "resolved"]
    critical_open = sum((item.get("definition") or {}).get("severity") == "critical" for item in open_incidents)
    findings = []
    ...
    if failed_jobs:
        findings.append({"severity": "high", "key": "failed_jobs_present", "count": len(failed_jobs)})
    if stalled_jobs:
        findings.append({"severity": "high", "key": "stalled_jobs_present", "count": len(stalled_jobs)})
    if audit["id_gap_count"]:
        findings.append({"severity": "medium", "key": "audit_id_gaps_detected", "count": audit["id_gap_count"]})
    if any(item["error_count"] for item in connector_continuity):
        findings.append({"severity": "medium", "key": "connector_run_errors_present"})
    if critical_open:
        findings.append({"severity": "critical", "key": "critical_operational_incident_open", "count": critical_open})
