...
def run_connector_for_seed(
    subject_id: int, seed, connector_key: str, spec: dict
) -> int:
    ...
    artifact = write_json_artifact(
        "connector-runs", payload, prefix=f"{connector_key}-{subject_id}"
    )
    ...
    for observation in extract_observations(
        connector_key, seed, result, spec, artifact
    ):
...
def execute_connector(connector_key: str, seed) -> dict:
    ...
    if connector_key == "social-analyzer":
        ...
        return run_social_analyzer(
            seed.normalized_value, seed.seed_type, allow_dry_run=True
        )
    try:
        from .connectors import run_connector

        return run_connector(
            connector_key, seed.normalized_value, seed.seed_type, allow_dry_run=True
        )
...
def _same_as_seed(value, seed) -> bool:
    return (
        ...
        == str(seed.normalized_value or "").lower().strip()
    )
...
def extract_observations(connector_key, seed, raw_result, spec, artifact) -> list[dict]:
    ...
    normalized_findings = normalize_connector_output(
        connector_key, seed.normalized_value, seed.seed_type, raw_result
    )
        ...
        observations.append(
            {
                ...
                "deep_enrichment": bool(spec.get("deep_enrichment")),
...
def connector_quality_adjustments() -> dict:
    ...
    for connector, item in stats.items():
        ...
        adjustments[connector] = round(
            min(0.05, confirmation_rate * 0.05) - min(0.18, rejection_rate * 0.18), 3
        )
    return adjustments
