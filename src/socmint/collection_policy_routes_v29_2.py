...
def register_collection_policy_routes_v29_2(app):
    ...
    def collection_policy_workspace_get_v29_2():
        ...
        if not actor_is_administrator(actor):
            return render_template(
                "collection_policy_v29_2.html",
                title="Authorization, Scope, and Collection Policy",
                payload={
                    "status": "forbidden",
        ...
        try:
            review_due_within_days = int(
                request.args.get("review_due_within_days", "30")
            )
        except ValueError:
        ...
        return render_template(
            "collection_policy_v29_2.html",
            title="Authorization, Scope, and Collection Policy",
            payload=build_collection_policy_workspace(
                review_due_within_days=review_due_within_days
            ),
        )
    ...
    def api_collection_policy_create_post_v29_2():
        ...
        result = create_collection_policy(
            ...
            permitted_source_classes=payload.get("permitted_source_classes"),
            ...
            review_at=str(payload.get("review_at") or ""),
    ...
    def api_collection_policy_revise_post_v29_2(policy_id: str):
        ...
        if error:
            return error
        payload = _payload()
        result = revise_collection_policy(
            policy_id,
            actor=actor,
    ...
    def api_collection_policy_evaluate_post_v29_2(collection_job_id: str):
        ...
        if error:
            return error
        payload = _payload()
        result = evaluate_collection_job_policy(
            actor=actor,
