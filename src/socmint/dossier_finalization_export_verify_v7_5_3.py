...
VERIFY_SUMMARY_SCHEMA = "socmint.v7_5_3.dossier_finalization_export_verification.summary"
...
def _finding(severity: str, code: str, detail: str, *, path: str | None = None) -> dict[str, Any]:
    return {
        ...
        "path": path,
        "detail": detail,
        "action": ACTIONS.get(code, "Review this verification finding before archiving or disclosure."),
    }
...
def verify_finalization_export_files(files: dict[str, bytes]) -> dict[str, Any]:
    ...
    if manifest and manifest.get("schema") != EXPORT_MANIFEST_SCHEMA:
        failures.append(_finding("fail", "wrong_manifest_schema", "Manifest schema does not match v7.5.2 export manifest schema.", path="manifest.json"))
    ...
    finalization = _load_json(safe_files, "finalization_packet.json", failures, "invalid_finalization_json") if "finalization_packet.json" in safe_files else {}
    summary = _load_json(safe_files, "finalization_summary.json", failures, "invalid_summary_json") if "finalization_summary.json" in safe_files else {}
    if finalization and summary:
        if finalization.get("decision") != summary.get("decision"):
            failures.append(_finding("fail", "decision_mismatch", "Finalization decision differs from summary decision.", path="finalization_summary.json"))
        if bool(finalization.get("ready")) != bool(summary.get("ready")):
            failures.append(_finding("fail", "ready_mismatch", "Finalization ready flag differs from summary ready flag.", path="finalization_summary.json"))
        if finalization.get("decision") in {"blocked", "needs_human_review"}:
            warnings.append(_finding("warn", "non_ready_finalization", "Finalization packet is structurally intact but not ready.", path="finalization_packet.json"))
    ...
    if not safe_files.get("finalization_packet.md", b"").strip():
        warnings.append(_finding("warn", "empty_markdown", "Markdown finalization packet is empty.", path="finalization_packet.md"))
...
def verify_finalization_export_packet(packet: dict[str, Any]) -> dict[str, Any]:
    return verify_finalization_export_files(build_finalization_export_files(deepcopy(packet or {})))
