...
VALID_ACTIONS = {"accept_same_entity", "reject_collision", "mark_uncertain", "request_more_evidence"}
ACTION_TO_STATE = {
...
def _find_candidate(profile_payload: dict[str, Any], candidate_id: str) -> dict[str, Any] | None:
...
def _assertion_value(candidate: dict[str, Any]) -> str:
    ...
    return fp.get("profile_url") or f"{fp.get('platform', 'unknown')}:{fp.get('username', candidate.get('candidate_id'))}"


def _create_or_confirm_assertion(subject_id: int, candidate: dict[str, Any], actor: str | None, note: str | None) -> int:
    value = _assertion_value(candidate)
    payload = {
        "schema": SCHEMA,
        "candidate_id": candidate.get("candidate_id"),
        "relationship": candidate.get("identity_link_hypothesis", {}).get("relationship"),
...
def review_candidate_profile(subject_id: int, candidate_id: str, action: str, profile_payload: dict[str, Any], actor: str | None = None, note: str | None = None) -> dict[str, Any]:
    ...
    return {"schema": SCHEMA, "subject_id": subject_id, "candidate_id": candidate_id, "action": action, "review_state": state, "assertion_id": assertion_id}


def apply_profile_review_decisions(profile_payload: dict[str, Any], subject_id: int) -> dict[str, Any]:
    decisions = _load_decisions(subject_id).get("decisions", {})
    accepted = rejected = uncertain = needs_more_evidence = unreviewed = dossier_ready = 0
    for candidate in profile_payload.get("candidates") or []:
        ...
        if decision:
            ...
            if state == "accepted":
                accepted += 1
                candidate["identity_link_hypothesis"]["can_promote_to_dossier_assertion"] = True
                candidate["dossier_assertion_gate"]["dossier_ready"] = True
                candidate["dossier_assertion_gate"]["blocked_reason"] = None
                candidate["dossier_assertion_gate"]["assertion_id"] = decision.get("assertion_id")
                candidate["dossier_assertion_gate"]["assertion_type"] = "same_online_identity_cluster"
                dossier_ready += 1
            elif state == "rejected":
                rejected += 1
                candidate["dossier_assertion_gate"]["dossier_ready"] = False
                candidate["dossier_assertion_gate"]["blocked_reason"] = "Analyst rejected this candidate as a collision."
            elif state == "uncertain":
                uncertain += 1
                candidate["dossier_assertion_gate"]["blocked_reason"] = "Analyst marked this candidate uncertain."
            elif state == "needs_more_evidence":
                needs_more_evidence += 1
                candidate["dossier_assertion_gate"]["blocked_reason"] = "Analyst requested more evidence before identity linkage."
            else:
                unreviewed += 1
        else:
            unreviewed += 1
        candidate["dossier_ready"] = bool(candidate.get("dossier_assertion_gate", {}).get("dossier_ready"))
...
def export_profile_review_report(subject_id: int, profile_payload: dict[str, Any], fmt: str = "json") -> tuple[str, str, str]:
    ...
    if fmt == "md" or fmt == "markdown":
        ...
        return ("text/markdown", f"candidate-profile-review-subject-{subject_id}.md", "\n".join(lines))
    return ("application/json", f"candidate-profile-review-subject-{subject_id}.json", json.dumps(payload, indent=2, sort_keys=True))
