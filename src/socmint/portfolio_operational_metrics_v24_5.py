...
from .portfolio_operations_dashboard_v24_0 import (
    _case_events,
    build_portfolio_operations_dashboard,
)
...
def build_operational_metrics(*, now: dt.datetime | None = None) -> dict[str, Any]:
    ...
    stage_counts = Counter(
        item.get("current_stage") for item in stage_overview.get("cases") or []
    )
    closure_count = sum(
        1 for item in cases if item.get("stage") in {"closed", "archived", "reopened"}
    )
    archive_count = sum(
        1 for item in cases if item.get("stage") in {"archived", "reopened"}
    )
    reopen_count = sum(1 for item in cases if item.get("stage") == "reopened")
    completed_count = sum(
        1
        for item in cases
        if item.get("stage") in {"delivered", "closed", "archived", "reopened"}
    )
    blocked_count = int(portfolio.get("counts", {}).get("blocked") or 0)
    overdue_case_ids = {
        item.get("case_id")
        for item in blocked_queue.get("queue") or []
        if item.get("stage_overdue") or item.get("assignment_overdue")
    }
    ...
    stage_duration_metrics = {
        stage: _duration_summary(durations.get(stage, [])) for stage in STAGES
    }

    reviewer_throughput = []
    for reviewer in workload.get("reviewers") or []:
        completed = int(reviewer.get("reviewed") or 0) + int(
            reviewer.get("accepted") or 0
        )
        reviewer_throughput.append(
            {
                ...
                "completion_rate_percent": _rate(
                    completed, int(reviewer.get("total_assigned") or 0)
                ),
                "average_assignment_age_hours": reviewer.get(
                    "average_assignment_age_hours"
                ),
            }
        )
    reviewer_throughput.sort(
        key=lambda item: (-item["completed_reviews"], str(item["reviewer"]))
    )
    ...
    for days in _windows():
        ...
        window_events = [
            (time, case_id, stage)
            for time, case_id, stage in event_times
