...
def _unwrap_request(
    payload: dict,
) -> tuple[dict, list[dict] | None, list[dict] | None, str, str | None]:
    ...
    return (
        ...
        payload.get("package_name") or request.args.get("package_name"),
    )
...
def register_dossier_finalization_export_routes(app):
    ...
    def api_dossier_finalization_export_packet():
        ...
        dossier, connectors, policy_events, export_mode, package_name = _unwrap_request(
            payload
        )
