...
from .case_delivery_recovery_closure_audit_package_v16_8 import build_case_delivery_recovery_closure_audit_package
from .case_delivery_recovery_closure_record_verification_v16_7 import verify_case_delivery_recovery_closure_record
from .case_delivery_recovery_v16_3 import build_case_delivery_recovery_from_request


CASE_DELIVERY_RECOVERY_CLOSURE_AUDIT_PACKAGE_VERIFICATION_SCHEMA = "socmint.case_delivery_recovery_closure_audit_package_verification.v16_9"
VERSION = "v16.9.0"
...
def _artifact_manifest(name: str, artifact: dict[str, Any] | None, sequence: int) -> dict[str, Any]:
...
def _manifest_blockers(package: dict[str, Any], artifacts: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    blockers = []
    manifest = package.get("manifest") if isinstance(package.get("manifest"), list) else []
    if package.get("artifact_count") != len(manifest):
        blockers.append(_blocker("artifact_count_mismatch", "audit package artifact_count does not match manifest"))
    names = [row.get("name") for row in manifest if isinstance(row, dict)]
    if tuple(names) != AUDIT_ARTIFACT_ORDER:
        blockers.append(_blocker("manifest_order_mismatch", "audit package manifest order is invalid"))
    for sequence, name in enumerate(AUDIT_ARTIFACT_ORDER, start=1):
        row = manifest[sequence - 1] if len(manifest) >= sequence and isinstance(manifest[sequence - 1], dict) else {}
        expected = _artifact_manifest(name, artifacts.get(name), sequence)
        if not row:
            blockers.append(_blocker("missing_manifest_row", f"manifest row {name} is missing"))
            continue
        for field in MANIFEST_PAYLOAD_FIELDS:
            if row.get(field) != expected.get(field):
                blockers.append(_blocker(f"manifest_{field}_mismatch", f"manifest {name} {field} does not match artifact"))
        expected_manifest_id = sha256_text(canonical_json(_manifest_payload(row)))
        if row.get("manifest_id") != expected_manifest_id:
            blockers.append(_blocker("manifest_id_mismatch", f"manifest id for {name} is invalid"))
    return blockers
...
def verify_case_delivery_recovery_closure_audit_package(
    ...
    closure_check = verify_case_delivery_recovery_closure_record(safe_closure, safe_recovery, safe_receipt)
    blockers = []
    ...
    if not closure_check.get("verified"):
        ...
        blockers.append(_blocker("closure_verification_blocked", "closure verification did not pass"))

    expected_package_hash = sha256_text(canonical_json(_audit_payload(safe_package))) if safe_package else None
    if safe_package and safe_package.get("package_sha256") != expected_package_hash:
        blockers.append(_blocker("package_hash_mismatch", "audit package hash does not match package fields"))

    expected_package_id = (
        sha256_text(canonical_json({**_audit_payload(safe_package), "package_sha256": safe_package.get("package_sha256")}))
        if safe_package
        else None
    )
    if safe_package and safe_package.get("audit_package_id") != expected_package_id:
        blockers.append(_blocker("audit_package_id_mismatch", "audit package id does not match canonical package payload"))

    if safe_package and safe_package.get("queue_id") != safe_recovery.get("queue_id"):
        blockers.append(_blocker("queue_id_mismatch", "audit package queue_id does not match recovery"))
    if safe_package and safe_package.get("case_id") != safe_recovery.get("case_id"):
        blockers.append(_blocker("case_id_mismatch", "audit package case_id does not match recovery"))
    if safe_package and safe_package.get("receipt_id") != safe_receipt.get("receipt_id"):
        blockers.append(_blocker("receipt_id_mismatch", "audit package receipt_id does not match receipt"))
    if safe_package and safe_package.get("closure_id") != safe_closure.get("closure_id"):
        blockers.append(_blocker("closure_id_mismatch", "audit package closure_id does not match closure"))
    if safe_package and safe_package.get("closure_verification_status") != safe_closure_verification.get("status"):
        blockers.append(_blocker("closure_verification_status_mismatch", "audit package verification status does not match closure verification"))
    if safe_package and safe_package.get("verified") is not True:
        blockers.append(_blocker("audit_package_not_verified", "audit package verified flag is not true"))

    blockers.extend(_manifest_blockers(safe_package, {
...
def verify_case_delivery_recovery_closure_audit_package_from_request(case_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    closure_verification = safe_payload.get("closure_verification") if isinstance(safe_payload.get("closure_verification"), dict) else None
    ...
    if audit_package is None:
        ...
        audit_package = package_result.get("audit_package") if isinstance(package_result.get("audit_package"), dict) else None
    return verify_case_delivery_recovery_closure_audit_package(audit_package, recovery, receipt, closure, closure_verification)
