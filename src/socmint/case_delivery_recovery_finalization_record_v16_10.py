...
CASE_DELIVERY_RECOVERY_FINALIZATION_RECORD_SCHEMA = "socmint.case_delivery_recovery_finalization_record.v16_10"
CASE_DELIVERY_RECOVERY_FINALIZATION_RESULT_SCHEMA = "socmint.case_delivery_recovery_finalization_record.v16_10.result"
VERSION = "v16.10.0"
...
def _finalization_blockers(
    ...
    if not audit_verification:
        blockers.append(_blocker("missing_audit_verification", "audit package verification is missing"))
    ...
    if recovery and audit_package and recovery.get("queue_id") != audit_package.get("queue_id"):
        blockers.append(_blocker("audit_package_queue_mismatch", "audit package queue_id does not match recovery queue_id"))
    if receipt and audit_package and receipt.get("receipt_id") != audit_package.get("receipt_id"):
        blockers.append(_blocker("audit_package_receipt_mismatch", "audit package receipt_id does not match receipt"))
    if closure and audit_package and closure.get("closure_id") != audit_package.get("closure_id"):
        blockers.append(_blocker("audit_package_closure_mismatch", "audit package closure_id does not match closure"))
    if audit_package and audit_verification and audit_package.get("audit_package_id") != audit_verification.get("audit_package_id"):
    ...
    *,
    ...
    if blockers:
        return {
            ...
            "version": VERSION,
            "case_id": safe_recovery.get("case_id") or safe_audit_package.get("case_id"),
            "queue_id": safe_recovery.get("queue_id") or safe_audit_package.get("queue_id"),
    ...
    finalization = {
        **payload,
        ...
        "finalization_id": sha256_text(canonical_json({**payload, "payload_sha256": payload_hash})),
    }
    return {
        "schema": CASE_DELIVERY_RECOVERY_FINALIZATION_RESULT_SCHEMA,
        "version": VERSION,
...
def build_case_delivery_recovery_finalization_record_from_request(case_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    finalizer = safe_payload.get("finalizer") if isinstance(safe_payload.get("finalizer"), str) else None
