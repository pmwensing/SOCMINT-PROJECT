...
PLATFORM_LINE_RE = re.compile(
    r"(?im)^\s*([A-Za-z0-9_. -]{2,60})\s*[:=]\s*(found|exists|registered|used|true|yes|valid|invalid|not found|false|no)\s*$"
)
PHONE_META_RE = re.compile(
    r"(?im)^\s*(country|carrier|line type|number type|valid|possible|region|timezone)\s*[:=]\s*(.+)$"
)
BREACH_RE = re.compile(
    r"(?i)(breach|leak|compromised|paste|pwned|exposure|database|found).*?([A-Za-z0-9_. -]{3,120})"
)
...
def _add_structured_findings(
    ...
    for finding in raw_result.get("findings") or []:
        ...
        _add(
            ...
            kind,
            value,
            finding.get("source") or connector,
            confidence,
            context,
        )
...
def normalize_connector_output(
    connector: str, seed_value: str, seed_type: str, raw_result: dict[str, Any]
) -> list[dict[str, Any]]:
            ...
            if isinstance(item, dict):
                ...
                if url and status not in {"not found", "false", "unknown"}:
                    _add(
                        ...
                        connector,
                        0.76,
                        {"platform": site, "status": status},
                    )
    return findings


def normalize_socialscan(
    seed_value: str,
    seed_type: str,
    raw_result: dict[str, Any],
    connector: str = "socialscan",
) -> list[dict[str, Any]]:
    findings: list[dict[str, Any]] = []
    seen: set[tuple[str, str]] = set()
    _add_structured_findings(findings, seen, raw_result, connector)
...
def normalize_social_analyzer(
    seed_value: str,
    seed_type: str,
    raw_result: dict[str, Any],
    connector: str = "social-analyzer",
) -> list[dict[str, Any]]:
    findings: list[dict[str, Any]] = []
    seen: set[tuple[str, str]] = set()
    _add_structured_findings(findings, seen, raw_result, connector)
    ...
    for url in PROFILE_URL_RE.findall(text):
        ...
        _add(
            ...
            connector,
            0.78,
            {"platform_hint": _platform_from_url(url), "deep_enrichment": True},
        )
    return findings
...
def normalize_h8mail(
    seed_value: str,
    seed_type: str,
    raw_result: dict[str, Any],
    connector: str = "h8mail",
) -> list[dict[str, Any]]:
    findings: list[dict[str, Any]] = []
    seen: set[tuple[str, str]] = set()
    _add_structured_findings(findings, seen, raw_result, connector)
...
def normalize_phoneinfoga(
    seed_value: str,
    seed_type: str,
    raw_result: dict[str, Any],
    connector: str = "phoneinfoga",
) -> list[dict[str, Any]]:
    findings: list[dict[str, Any]] = []
    seen: set[tuple[str, str]] = set()
    _add_structured_findings(findings, seen, raw_result, connector)
    ...
    for label, value in PHONE_META_RE.findall(text):
        _add(
            findings,
            seen,
            f"phone_{label.lower().replace(' ', '_')}",
            value.strip(),
            connector,
            0.7,
            {"target": seed_value},
...
def normalize_archivebox(
    seed_value: str,
    seed_type: str,
    raw_result: dict[str, Any],
    connector: str = "archivebox",
) -> list[dict[str, Any]]:
    findings: list[dict[str, Any]] = []
    seen: set[tuple[str, str]] = set()
    ...
    for finding in raw_result.get("findings") or []:
        ...
        if value:
            _add(
                findings,
                seen,
                finding.get("type", "archive_snapshot"),
                value,
                connector,
                finding.get("confidence", 0.78),
...
def _walk_social_analyzer_json(
    data: Any,
    findings: list[dict[str, Any]],
    seen: set[tuple[str, str]],
    connector: str,
    ...
    if isinstance(data, dict):
        ...
        if url and status not in {"false", "not found", "missing", "unknown", "none"}:
            _add(
                findings, seen, "profile_url", str(url), connector, confidence, context
            )
