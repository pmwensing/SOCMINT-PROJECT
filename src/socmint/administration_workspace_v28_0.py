...
ADMIN_ACTION_MARKERS = (
    TEAM_ACTION_MARKERS
    + ACCESS_ACTION_MARKERS
    + POLICY_ACTION_MARKERS
    + SESSION_ACTION_MARKERS
    + ("user_", "connector_")
)
OPEN_STATES = {
    "pending",
...
def _connectors() -> list[dict[str, Any]]:
    ...
    try:
        rows = (
            ...
            .order_by(
                database.ConnectorRun.created_at.desc(), database.ConnectorRun.id.desc()
            )
            .limit(1000)
            .all()
        )
...
def build_administration_workspace(
    *,
    ...
    user_summary = {
        "total": len(user_rows),
        "active": sum(bool(item.get("is_active")) for item in user_rows),
        "inactive": sum(not bool(item.get("is_active")) for item in user_rows),
        "administrators": sum(bool(item.get("is_admin")) for item in user_rows),
        "users": user_rows,
    }
    role_summary = {
        "role_counts": dict(sorted(role_counts.items())),
        "distinct_role_count": len(role_counts),
        "least_privilege_review_required": any(
            role == "admin" and count > 1 for role, count in role_counts.items()
        ),
    }
    ...
    team_events = [
        ...
        if _contains(str(item.get("action") or ""), TEAM_ACTION_MARKERS)
    ]
    access_events = [
        item
        for item in source_events
        if _contains(str(item.get("action") or ""), ACCESS_ACTION_MARKERS)
    ]
    policy_events = [
        item
        for item in source_events
        if _contains(str(item.get("action") or ""), POLICY_ACTION_MARKERS)
    ]
    session_events = [
        item
        for item in source_events
        if _contains(str(item.get("action") or ""), SESSION_ACTION_MARKERS)
    ]
    governance_events = [
        item
        for item in source_events
        if _contains(str(item.get("action") or ""), ADMIN_ACTION_MARKERS)
    ]
            ...
            active_sessions.append(
                {
                    ...
                    "last_seen_at": item.get("occurred_at"),
    ...
    def event_summary(items: list[dict[str, Any]], category: str) -> dict[str, Any]:
        return {
            ...
            "action_counts": dict(
                sorted(
                    Counter(
                        str(item.get("action") or "unknown") for item in items
                    ).items()
                )
            ),
            "latest_events": items[:25],
        }
    ...
    connector_status_counts = Counter(
        str(item.get("latest_status") or "unknown").lower() for item in connector_rows
    )
    connector_summary = {
        "connector_count": len(connector_rows),
        "healthy_count": sum(
            status in {"success", "completed", "ready", "passed"}
            for status in connector_status_counts.elements()
        ),
        "failed_count": sum(
    ...
    system_health = {
        ...
        "jobs": job_state,
        "connector_failures": connector_summary["failed_count"],
        "overall_status": "healthy"
        if database_ready
        and not job_state.get("failed_count")
        and not connector_summary["failed_count"]
        else "attention_required",
    }
    core = {
        "users": user_summary,
        "roles": role_summary,
        "teams": len(team_events),
        "access": len(access_events),
        "policies": len(policy_events),
    ...
    return {
        ...
        "team_summary": event_summary(team_events, "team"),
        "active_sessions": active_sessions,
        "active_session_count": len(active_sessions),
        "access_grant_summary": event_summary(access_events, "access"),
        "policy_summary": event_summary(policy_events, "policy"),
        "connector_summary": connector_summary,
        "system_health": system_health,
        "pending_admin_actions": pending,
        "pending_admin_action_count": len(pending),
        "recent_governance_events": governance_events[:50],
        ...
        "next_action": "review_administration_state",
    }
