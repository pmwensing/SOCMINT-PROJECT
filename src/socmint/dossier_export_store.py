...
def _audit_export_scope_decision(
    *,
    allowed: bool,
    subject_id: str | None,
    case_id: str | None,
    ...
    audit_subject_id = expected_subject_id or subject_id or "unknown-subject"
    audit_case_id = expected_case_id or case_id or "unknown-case"
    detail = {
        "allowed": allowed,
        "subject_id": _normalize_scope_value(subject_id),
        "case_id": _normalize_scope_value(case_id),
        "expected_subject_id": _normalize_scope_value(expected_subject_id),
        "expected_case_id": _normalize_scope_value(expected_case_id),
    }
    return _audit_event(
        "export_scope_allowed" if allowed else "export_scope_blocked",
        case_id=str(audit_case_id),
        subject_id=str(audit_subject_id),
        actor=actor,
        detail=detail,
        root=root,
    )
...
def persist_export_pack(
    ...
    try:
        ...
        raise
    if audit and (expected_subject_id is not None or expected_case_id is not None):
        scope_event = _audit_export_scope_decision(
            allowed=True,
            subject_id=subject_id,
            case_id=case_id,
            expected_subject_id=expected_subject_id,
    ...
    if scope_event:
        result["scope_audit_event"] = scope_event
