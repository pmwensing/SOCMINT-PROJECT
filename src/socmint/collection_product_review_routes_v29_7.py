...
def register_collection_product_review_routes_v29_7(app):
    ...
    def collection_product_review_get_v29_7():
        ...
        if not actor_is_administrator(actor):
            return render_template("collection_product_review_v29_7.html", title="Collection Product Review", payload={"status":"forbidden","error":"administrator required","ready":False,"blockers":[]}), 403
        payload = build_collection_product_review(routes=list(app.url_map.iter_rules()))
        return render_template("collection_product_review_v29_7.html", title="Collection Product Review", payload=payload), 200 if payload.get("ready") else 503
