...
def register_dossier_finalization_master_delivery_index_routes(app):
    ...
    def api_master_delivery_index():
        ...
        return jsonify(
            build_master_delivery_index(report, operator=operator, notes=notes)
        )
    ...
    def api_master_delivery_index_markdown():
        ...
        return Response(
            render_master_delivery_index_markdown(index), mimetype="text/markdown"
        )
    ...
    def api_master_delivery_index_from_bundle():
        ...
        return jsonify(
            build_master_delivery_index_from_bundle(
                bundle, operator=operator, notes=notes
            )
        )
    ...
    def api_master_delivery_index_from_zip():
        ...
        if not isinstance(encoded, str) or not encoded.strip():
            return jsonify(
                _invalid_zip_index(payload, "zip_base64 is missing or empty.")
            )
        try:
