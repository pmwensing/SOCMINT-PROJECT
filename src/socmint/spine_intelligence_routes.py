...
from .dossier_assertion_handoff_certification_v12_10_13 import export_dossier_assertion_handoff_certification_report
...
def register_spine_intelligence_routes(app) -> None:
    ...
    def spine_dossier_assertion_handoff_bundle_report(subject_id: int):
        ...
        try:
            payload = spine_intelligence_payload(subject_id)
            mime_type, filename, body = export_dossier_assertion_handoff_bundle_report(payload.get("dossier_assertion_handoff_bundle", {}), fmt=fmt)
        except ValueError:
            abort(404)
        return Response(body, mimetype=mime_type, headers={"Content-Disposition": f"attachment; filename={filename}"})
    ...
    def spine_dossier_assertion_handoff_seal_report(subject_id: int):
        ...
        try:
            payload = spine_intelligence_payload(subject_id)
            mime_type, filename, body = export_dossier_assertion_handoff_seal_report(payload.get("dossier_assertion_handoff_seal", {}), fmt=fmt)
        except ValueError:
            abort(404)
        return Response(body, mimetype=mime_type, headers={"Content-Disposition": f"attachment; filename={filename}"})
    ...
    def spine_dossier_assertion_handoff_certification_report(subject_id: int):
        ...
        try:
            payload = spine_intelligence_payload(subject_id)
            mime_type, filename, body = export_dossier_assertion_handoff_certification_report(payload.get("dossier_assertion_handoff_certification", {}), fmt=fmt)
        except ValueError:
            abort(404)
        return Response(body, mimetype=mime_type, headers={"Content-Disposition": f"attachment; filename={filename}"})
    ...
    app.add_url_rule("/spine/subjects/<int:subject_id>/dossier-assertion-handoff-certification/report", endpoint="spine_dossier_assertion_handoff_certification_report", view_func=spine_dossier_assertion_handoff_certification_report, methods=["GET"])
    ...
    app.add_url_rule("/api/v1/spine/subjects/<int:subject_id>/dossier-assertion-handoff-certification", endpoint="api_spine_dossier_assertion_handoff_certification", view_func=api_spine_dossier_assertion_handoff_certification, methods=["GET"])
