...
def register_recovery_operations_routes_v29_5(app):
    ...
    def recovery_operations_workspace_get_v29_5():
        ...
        if not actor_is_administrator(actor):
            return render_template(
                "recovery_operations_v29_5.html",
                title="Retry, Recovery, and Operator Intervention",
                payload={
                    "status": "forbidden",
        ...
        return render_template(
            "recovery_operations_v29_5.html",
            title="Retry, Recovery, and Operator Intervention",
            payload=build_recovery_operations_workspace(),
        )
    ...
    def api_retry_request_post_v29_5(collection_job_id: str):
        ...
        result = request_retry(
            ...
            earliest_retry_at=str(payload.get("earliest_retry_at") or ""),
            retry_window_ends_at=str(payload.get("retry_window_ends_at") or ""),
    ...
    @app.post(
        "/api/v1/collection-operations/retry-requests/<retry_request_id>/decision"
    )
    ...
    def api_recovery_plan_post_v29_5(collection_job_id: str):
        ...
        if error:
            return error
        payload = _payload()
        result = create_recovery_plan(
            actor=actor,
            collection_job_id=collection_job_id,
            retry_request_id=str(payload.get("retry_request_id") or ""),
            plan_type=str(payload.get("plan_type") or ""),
            steps=payload.get("steps"),
            operator_required=payload.get("operator_required") is True,
            replacement_job_id=str(payload.get("replacement_job_id") or ""),
            reason=str(payload.get("reason") or ""),
            confirmed=payload.get("confirmed") is True,
            ip_address=request.remote_addr,
        )
    ...
    def api_operator_intervention_post_v29_5(collection_job_id: str):
        ...
        if error:
            return error
        payload = _payload()
        result = record_operator_intervention(
            actor=actor,
            collection_job_id=collection_job_id,
            intervention_type=str(payload.get("intervention_type") or ""),
            resolution=str(payload.get("resolution") or ""),
            replacement_job_id=str(payload.get("replacement_job_id") or ""),
            apply_terminal_transition=payload.get("apply_terminal_transition") is True,
            reason=str(payload.get("reason") or ""),
            confirmed=payload.get("confirmed") is True,
            ip_address=request.remote_addr,
        )
        return jsonify(result), _code(
            result, "collection_operator_intervention_recorded"
        )
