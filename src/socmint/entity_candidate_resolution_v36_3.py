...
def decision_history(candidate_id: str | None = None) -> list[dict[str, Any]]:
    ...
    return (
        [item for item in rows if item.get("candidate_id") == candidate_id]
        if candidate_id
        else rows
    )
...
def current_candidates() -> list[dict[str, Any]]:
    decisions = {
        str(event.get("candidate_id") or ""): event
        ...
        [
            {
                **assessment,
                ...
                "decision_recorded": str(
                    assessment.get("candidate_id") or ""
                )
                in decisions,
            }
        ...
        normalized.append(
            {
                ...
                "signal_class": _signal_class(signal_type),
                "reason": reason,
...
def _score(signals: list[dict[str, Any]]) -> dict[str, Any]:
    counts = {
        signal_class: sum(
            1 for item in signals if item["signal_class"] == signal_class
        )
        for signal_class in ("strong", "supporting", "weak", "negative")
    }
    components = {
