...
def compute_dossier_readiness(data: ReadinessInput | dict[str, Any]) -> dict[str, Any]:
    ...
    _append_if(blockers, data.hash_mismatch_count > 0, "Resolve evidence hash mismatches.")

    _append_if(warnings, data.finding_count <= 0, "Run or import collection to create findings.")
    _append_if(warnings, data.pending_review_count > 0, "Review pending findings before final export.")
    _append_if(
        warnings,
    ...
    if not data.subject_exists:
        ...
        next_actions.append({"key": "create_subject", "label": "Create or open a subject", "href": "/spine"})
    elif data.seed_count <= 0:
        state = STATE_BLOCKED
        label = "Needs seed intake"
        next_actions.append({"key": "add_seed", "label": "Add seed or target", "href": "/targets"})
    elif data.hash_mismatch_count > 0:
        state = STATE_BLOCKED
        label = "Evidence integrity issue"
        next_actions.append({"key": "fix_hashes", "label": "Open evidence integrity", "href": "/evidence/integrity/gate"})
    elif data.report_count > 0:
        state = STATE_EXPORTED
        label = "Dossier export exists"
        next_actions.append({"key": "export_package", "label": "Open export center", "href": "/reports/export-center"})
    elif data.finding_count <= 0:
        state = STATE_NEEDS_REVIEW
        label = "Needs collection"
        next_actions.append({"key": "run_collection", "label": "Run or import collection", "href": "/targets"})
    elif (
        ...
        next_actions.append({"key": "review_findings", "label": "Open enrichment review", "href": "/spine/enrichment-review"})
    else:
        state = STATE_DRAFT_READY
        label = "Draft dossier ready"
        next_actions.append({"key": "generate_dossier", "label": "Generate dossier", "href": f"/spine/subjects/{data.subject_id}/full-report"})

    final_export_allowed = state in {STATE_FINAL_READY, STATE_EXPORTED}
    draft_export_allowed = state in {STATE_DRAFT_READY, STATE_FINAL_READY, STATE_EXPORTED}
