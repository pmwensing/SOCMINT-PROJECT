...
CASE_DELIVERY_RECOVERY_RESUME_OPERATIONS_SNAPSHOT_SCHEMA = "socmint.case_delivery_recovery_resume_operations_snapshot.v16_14"
CASE_DELIVERY_RECOVERY_RESUME_OPERATIONS_RESULT_SCHEMA = "socmint.case_delivery_recovery_resume_operations_snapshot.v16_14.result"
VERSION = "v16.14.0"
...
def _resume_payload(
    ...
    return {
        ...
        "continuation_gate_verification_status": continuation_gate_verification.get("status"),
        "continuation_gate_verified": continuation_gate_verification.get("verified") is True,
        "gate_open": continuation_gate_verification.get("gate_open") is True,
        "safe_to_reenter_operations": continuation_gate_verification.get("ready_for_delivery_continuation") is True,
...
def _resume_blockers(
    ...
    if not continuation_gate_verification:
        blockers.append(_blocker("missing_continuation_gate_verification", "continuation gate verification is missing"))
    if continuation_gate_verification and continuation_gate_verification.get("verified") is not True:
    ...
    if continuation_gate_verification and continuation_gate_verification.get("gate_open") is not True:
        blockers.append(_blocker("continuation_gate_not_open", "continuation gate verification is not open"))
    if continuation_gate_verification and continuation_gate_verification.get("ready_for_delivery_continuation") is not True:
    ...
    if recovery and continuation_gate and recovery.get("queue_id") != continuation_gate.get("queue_id"):
        blockers.append(_blocker("continuation_gate_queue_mismatch", "continuation gate queue_id does not match recovery queue_id"))
    if continuation_gate and continuation_gate_verification and continuation_gate.get("continuation_gate_id") != continuation_gate_verification.get("continuation_gate_id"):
        blockers.append(_blocker("continuation_gate_id_mismatch", "continuation gate id does not match verification"))
    return blockers
...
def build_case_delivery_recovery_resume_operations_snapshot(
    ...
    *,
    ...
    if not safe_continuation_gate_verification:
        safe_continuation_gate_verification = verify_case_delivery_recovery_continuation_gate(
    ...
    blockers = _resume_blockers(safe_recovery, safe_continuation_gate, safe_continuation_gate_verification)
    if blockers:
        return {
            "schema": CASE_DELIVERY_RECOVERY_RESUME_OPERATIONS_RESULT_SCHEMA,
            "version": VERSION,
            "case_id": safe_recovery.get("case_id") or safe_continuation_gate.get("case_id"),
            "queue_id": safe_recovery.get("queue_id") or safe_continuation_gate.get("queue_id"),
    ...
    resume_snapshot = {
        **payload,
        ...
        "resume_snapshot_id": sha256_text(canonical_json({**payload, "payload_sha256": payload_hash})),
    }
    return {
        "schema": CASE_DELIVERY_RECOVERY_RESUME_OPERATIONS_RESULT_SCHEMA,
        "version": VERSION,
...
def build_case_delivery_recovery_resume_operations_snapshot_from_request(case_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    resume_operator = safe_payload.get("resume_operator") if isinstance(safe_payload.get("resume_operator"), str) else None
