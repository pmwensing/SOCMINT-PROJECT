...
def history(username: str | None = None) -> list[dict[str, Any]]:
    ...
    try:
        ...
        rows = query.order_by(
            database.AuditLog.created_at.asc(), database.AuditLog.id.asc()
        ).all()
        return [
            {
                **_json_details(row),
                ...
                "actor": row.actor,
                "action": row.action,
                "target_username": row.target_value,
...
def record(
    ...
    *,
    ...
    event = {
        "schema": SCHEMA,
        "version": VERSION,
        "account_event_id": f"account-event-{digest[:24]}",
        "account_event_sha256": digest,
        "reason": reason,
        "before": before,
        "after": after,
