    ...
    if manifest_dir.exists():
        for path in sorted(manifest_dir.glob("*.json"), reverse=True)[:20]:
            ...
            manifests.append({
                ...
                "item_count": (data or {}).get("item_count") or len((data or {}).get("events", [])),
            })
    ...
    if vault_dir.exists():
        for path in sorted(vault_dir.rglob("original*"))[:200]:
            vault_items.append({
                ...
                "kind": path.parent.parent.name if len(path.parents) > 1 else "unknown",
            })
    payload.update({
...
def register_forensic_intake_routes(app) -> None:
    ...
    def forensic_intake_run():
        ...
        flash(f"Forensic intake completed: {report['ingested_count']} items preserved.", "success")
    ...
    app.add_url_rule("/forensic/intake", endpoint="forensic_intake_dashboard", view_func=forensic_intake_dashboard, methods=["GET"])
    app.add_url_rule("/forensic/intake/run", endpoint="forensic_intake_run", view_func=forensic_intake_run, methods=["POST"])
    app.add_url_rule("/api/v1/forensic/intake", endpoint="api_forensic_intake_dashboard", view_func=api_forensic_intake_dashboard, methods=["GET"])
