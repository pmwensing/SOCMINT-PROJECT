...
def _source_payload(
    section: dict[str, Any], *, user: str, allowed_case_ids: set[str] | None, limit: int
) -> dict[str, Any]:
    ...
    if section_type == "saved_view":
        ...
        return {
            ...
            "summary": {
                ...
                "result_set_sha256": payload.get("result_set_sha256"),
            },
        }
    ...
    return {
        "status": "ready",
        "source": {
            "watchlist_id": section["source_id"],
            "monitoring_run_id": result.get("monitoring_run_id"),
            "monitoring_run_sha256": result.get("watchlist_event_sha256"),
        },
        "results": payload.get("results") or [],
        "summary": {
            "result_count": payload.get(
            ...
            "access_scope": payload.get("access_scope"),
            "result_set_sha256": result.get("result_set_sha256"),
...
def _render_html(package: dict[str, Any]) -> str:
    parts = [
        ...
        "</title></head><body>",
        f"<h1>{html.escape(package['name'])}</h1>",
    ]
    for section in package["sections"]:
        ...
        if section.get("include_summary"):
            parts.append(
                f"<pre>{html.escape(json.dumps(section.get('summary') or {}, indent=2, sort_keys=True, default=str))}</pre>"
            )
        if section.get("include_results"):
            parts.append(
                "<table><thead><tr><th>Type</th><th>Case</th><th>Title</th><th>Summary</th></tr></thead><tbody>"
            )
            for item in section.get("results") or []:
                parts.append(
                    "<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>".format(
                        html.escape(
                            str(
                                item.get("record_type") or item.get("result_type") or ""
                            )
                        ),
                        html.escape(str(item.get("case_id") or "")),
...
def latest_packages(report_id: str | None = None) -> list[dict[str, Any]]:
    packages = [
        item for item in history() if item.get("event_type") == "package_generated"
    ]
    if report_id:
...
def generate_report_package(
    ...
    *,
    ...
    if report is None or report.get("report_status") != "active":
        return blocked("active_visible_report_required")
    if confirmed is not True:
        return blocked("explicit_report_generation_confirmation_required")
    requested_formats = sorted(
        {
            str(item)
            for item in (
                formats or report.get("definition", {}).get("export_formats") or []
            )
            if str(item) in EXPORT_FORMATS
        }
    )
    ...
    for index, definition in enumerate(
        report.get("definition", {}).get("sections") or []
    ):
        source = (
            resolved_sections[index]
            if resolved_sections is not None
            else _source_payload(
                definition,
                user=user_identity,
                allowed_case_ids=allowed_case_ids,
                limit=limit,
        ...
        rendered_sections.append(
            {
                **definition,
                ...
                "source_binding_sha256": _sha(
                    source.get("source") or {"section_type": definition["section_type"]}
                ),
                "summary": source.get("summary") or {},
                "text": source.get("text") or definition.get("text"),
                "results": source.get("results") or [],
            }
        )
    package_core = {
        "name": report.get("name"),
        "description": report.get("definition", {}).get("description"),
        "report_id": report_id,
    ...
    content = {
        ...
        "report_binding": {
            ...
            "report_event_sha256": report.get("report_event_sha256"),
            "definition_sha256": report.get("definition_sha256"),
            "revision": report.get("revision"),
        },
        "executed_access_scope": package_core["executed_access_scope"],
        "section_count": len(rendered_sections),
        "result_count": sum(
            len(item.get("results") or []) for item in rendered_sections
        ),
        "formats": requested_formats,
        "file_manifest": manifest,
        "file_manifest_sha256": _sha(manifest),
    }
    digest = _sha(content)
    event = {
        "schema": SCHEMA,
        "version": VERSION,
        **content,
        "package_id": f"report-package-{digest[:24]}",
        "package_event_id": f"report-package-event-{digest[:24]}",
        "package_sha256": digest,
        "source_records_mutated": False,
        "report_definition_mutated": False,
    ...
    return {
        **recorded,
        ...
        "files": files,
        "next_action": "download_or_review_report_package",
    }
