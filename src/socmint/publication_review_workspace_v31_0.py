...
def build_publication_review_workspace(root: str | Path | None = None) -> dict[str, Any]:
    ...
    contracts = [{"path": path, "available": (root_path / path).exists()} for path in DOSSIER_CONTRACTS]
    ...
    release_previews = [
        item
        for item in releases
        if item.get("action") in {"case_dossier_release_preview", "case_dossier_release_authorization"}
    ]
    published = [
        item
        for item in releases
        if item.get("action") in {"case_dossier_secure_distribution", "case_dossier_recipient_acknowledgement"}
    ]
