...
from .assertion_trust_gate_v12_8_1 import (
    assertion_release_gate,
    assertion_trust_summary,
)
...
def _status_from_counts(
    good: int, review: int = 0, bad: int = 0, empty: bool = False
) -> str:
    if bad > 0:
        return "red"
...
def evidence_step(root: str | None = None) -> ProgressStep:
    ...
    status = _status_from_counts(
        item_count, review=pending_count, empty=item_count == 0
    )
    actions = []
    if pending_count:
        actions.append(
            _action(
                ...
                "Run forensic intake",
                "/forensic/intake",
                f"{pending_count} pending files are waiting in drop folders.",
            )
        )
    if item_count == 0:
        actions.append(
            _action(
                "high",
                "Add evidence to drop folders",
                "/forensic/intake",
                "No preserved evidence is available yet.",
            )
        )
    return ProgressStep(
        "evidence",
        "Evidence Intake",
        status,
        _score_for_status(status),
        "/forensic/intake",
        f"{item_count} preserved items · {pending_count} pending files",
        actions,
    )
...
def integrity_step(root: str | None = None) -> ProgressStep:
    ...
    status = {"GO": "green", "HOLD": "yellow", "FAIL": "red"}.get(
        gate.get("release_gate_decision"), "yellow"
    )
    actions = []
    if summary.get("hold_count", 0):
        actions.append(
            _action(
                "critical",
                "Resolve hold evidence",
                "/evidence/integrity/gate",
                "One or more evidence items are on hold.",
            )
        )
    if summary.get("review_count", 0):
        actions.append(
            _action(
                ...
                "/evidence/integrity/gate",
                "Flagged evidence must be reviewed before dossier use.",
            )
        )
    if summary.get("item_count", 0) == 0:
        actions.append(
            _action(
                "high",
                "Run evidence integrity after intake",
                "/evidence/integrity",
                "No integrity-scored evidence exists yet.",
            )
        )
    return ProgressStep(
        "integrity",
        ...
        _score_for_status(status),
        "/evidence/integrity/gate",
        f"Decision {gate.get('release_gate_decision')} · usable {summary.get('usable_count', 0)} · review {summary.get('review_count', 0)} · hold {summary.get('hold_count', 0)}",
        actions,
    )
...
def narrative_step(
    ...
    status = (
        "green"
        if rating in {"strong", "moderate"} and contradictions == 0
        else "yellow"
        if events
        else "yellow"
    )
    ...
    if events == 0:
        actions.append(
            _action(
                "high",
                "Generate narrative from preserved evidence",
    ...
    if contradictions:
        actions.append(
            _action(
                "high",
                "Review narrative contradictions",
                "/narrative/storyboard",
                f"{contradictions} contradiction review actions are pending.",
            )
        )
    return ProgressStep(
        "narrative",
        ...
        _score_for_status(status),
        "/narrative/storyboard",
        f"{events} timeline events · confidence {rating} · {contradictions} contradiction actions",
        actions,
    )
...
def assertion_step(
    ...
    if summary.get("hold_count", 0):
        actions.append(
            _action(
                "critical",
                "Resolve hold assertions",
                "/assertions/trust/gate",
                "Hold assertions block dossier readiness.",
            )
        )
    if summary.get("review_queue_count", 0):
        actions.append(
            _action(
                "high",
                "Review assertion trust queue",
                "/assertions/trust/gate",
                f"{summary.get('review_queue_count')} assertions require analyst review.",
            )
        )
    if summary.get("dossier_ready_count", 0) == 0:
        actions.append(
            _action(
                "high",
                "Build dossier-ready assertion set",
                "/assertions/trust",
                "No assertions are currently dossier-ready.",
            )
        )
    return ProgressStep(
        "assertions",
        ...
        _score_for_status(status),
        "/assertions/trust/gate",
        f"Decision {gate.get('release_gate_decision')} · ready {summary.get('dossier_ready_count', 0)} · review {summary.get('review_queue_count', 0)} · hold {summary.get('hold_count', 0)}",
        actions,
    )
...
def release_step(subject_id: int | None = None) -> ProgressStep:
    ...
    for check in gate.get("checks", []):
        if check.get("status") in {"fail", "review"}:
            actions.append(
                _action(
                    "critical" if check.get("status") == "fail" else "high",
                    f"Fix {check.get('name')}",
                    "/command-center",
                    f"Gate check status: {check.get('status')} · actual: {check.get('actual')}",
                )
            )
    return ProgressStep(
        "release",
        ...
        status,
        _score_for_status(status),
        "/api/v1/production/release-gate",
        f"Decision {gate.get('release_gate_decision')} · {gate.get('summary', {}).get('passed_checks', 0)}/{gate.get('summary', {}).get('total_checks', 0)} checks passed",
        actions,
    )


def guided_investigation_payload(
    subject_id: int | None = None, root: str | None = None
) -> dict[str, Any]:
    steps = [
    ...
    for step in rows:
        for action in step.get("actions", []):
            action_queue.append(
                {**action, "step": step["key"], "step_label": step["label"]}
            )
    ...
    readiness = (
        "red"
        if any(step["status"] == "red" for step in rows)
        else "yellow"
        if any(step["status"] == "yellow" for step in rows)
        else "green"
    )
    ...
    next_action = (
        ...
        if action_queue
            ...
            "/api/v1/production/release-gate",
            "All guided workflow steps are green.",
        )
    )
    flow_edges = [
        {
            "source": "evidence",
            "target": "integrity",
            "label": "preserved files scored",
        },
        {
            "source": "integrity",
            "target": "narrative",
            "label": "trusted evidence builds story",
        },
        {
            "source": "narrative",
            "target": "assertions",
            "label": "claims become assertions",
        },
        {
            "source": "assertions",
            "target": "release",
            "label": "trusted assertions feed dossier",
        },
    ]
    return {
        "schema": SCHEMA,
