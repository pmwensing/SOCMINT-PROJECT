...
def current_sources() -> list[dict[str, Any]]:
    registrations = [
        item
        for item in _history()
        if item.get("event_type") == REGISTER_ACTION
    ]
    profiles: dict[str, list[dict[str, Any]]] = {}
    for assessment in current_reliability_profiles():
    ...
    for registration in registrations:
        ...
        result.append(
            {
                **registration,
                "source_reliability_profile": profiles.get(source_id, []),
                "reliability_assessed": bool(profiles.get(source_id)),
            }
        )
    ...
    *,
    ...
    return {
        ...
        "artifact_event_sha256": (
            artifact.get("state_history") or [artifact]
...
def assess_source_reliability(
    *,
    ...
    digest = _sha(assessment)
    assessment_id = f"source-reliability-{digest[:24]}"
    if any(
    ...
    content["source_binding_sha256"] = _sha(content["source_binding"])
