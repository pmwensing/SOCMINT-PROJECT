...
from .integrity_gate_v12_7_1 import (
    ...
    integrity_drilldown_for_claims,
)
...
def _release_state(
    score: float, contradiction_state: str, integrity_state: str, analyst_state: str
) -> str:
    ...
    if (
        score >= 0.8
        and analyst_state in {"confirmed", "reviewed"}
        and integrity_state in {"usable", "review", "unknown"}
    ):
...
def _connector_sources(subject_id: int | None = None) -> list[AssertionSource]:
    ...
    for run in payload.get("runs", []):
        ...
        confidence = (
            0.82
            if badge == "real" or run.get("real_observation_count", 0)
            else 0.35
            if status == "dry_run"
            else 0.45
...
def _integrity_by_evidence(root: str | None = None) -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        for row in summary.get("item_decisions", [])
...
def _score_assertion(
    ...
    if sources:
        ...
        reasoning.append(
            f"source_component={source_component:.2f}; sources={len(sources)}; unique_types={unique_types}"
        )
    else:
    ...
    integrity_component = {
        ...
        "unknown": 0.06,
        "hold": -0.25,
    }.get(integrity_state, 0.04)
    reasoning.append(
        f"integrity_component={integrity_component:.2f}; state={integrity_state}"
    )
    analyst_component = {
        "confirmed": 0.18,
        "reviewed": 0.12,
        "unreviewed": 0.0,
        "rejected": -0.35,
    }.get(analyst_state, 0.0)
    reasoning.append(
        f"analyst_component={analyst_component:.2f}; state={analyst_state}"
    )
    contradiction_penalty = (
        -0.3
        if contradiction_state == "contradicted"
        else -0.08
        if contradiction_state == "review"
        else 0.04
    )
    reasoning.append(
        f"contradiction_component={contradiction_penalty:.2f}; state={contradiction_state}"
    )
    score = max(
        0.0,
        min(
...
def build_assertion_trust(
    ...
    for evidence_id, claims in claims_by_evidence.items():
        evidence_integrity = integrity.get(
            str(evidence_id), {"usable_state": "unknown", "composite_score": 0}
        )
        ...
        for claim in claims:
            ...
            contradiction_state = (
                "contradicted"
                if (str(claim.get("subject")), str(claim.get("predicate")))
                in contradiction_index
                else "clear"
            )
            ...
            score, reasoning = _score_assertion(
                claim, sources, contradiction_state, integrity_state, analyst_state
            )
            assertions.append(
                TrustedAssertion(
                    ...
                    release_state=_release_state(
                        score, contradiction_state, str(integrity_state), analyst_state
                    ),
                    reasoning=reasoning,
                )
            )
    ...
    return {
        ...
        "summary": {
            ...
            "avg_trust_score": round(
                sum(row["trust_score"] for row in rows) / max(1, len(rows)), 3
            ),
            "dossier_ready": counts.get("dossier-ready", 0),
            "hold": counts.get("hold", 0),
            "analyst_review": counts.get("analyst-review", 0),
        },
        "integrity_drilldown": integrity_drilldown_for_claims(
            claims_by_evidence, root=root
        ),
    }


def corroboration_dashboard_payload(
    subject_id: int | None = None, root: str | None = None
) -> dict[str, Any]:
    ...
    return {
        ...
        "review_queue": [
            row
            for row in trust.get("assertions", [])
