...
HANDOFF_EXPORT_VERIFY_SCHEMA = (
    "socmint.v7_5_9.dossier_finalization_handoff_export_verification"
)
HANDOFF_EXPORT_VERIFY_SUMMARY_SCHEMA = (
...
def _finding(
    ...
    return {
        ...
        "detail": detail,
        "action": ACTIONS.get(
            code, "Review this handoff export verification finding before handoff."
        ),
    }
...
def verify_handoff_export_files(files: dict[str, bytes]) -> dict[str, Any]:
    ...
    if manifest and manifest.get("schema") != HANDOFF_EXPORT_MANIFEST_SCHEMA:
        failures.append(
            _finding(
                ...
                "Manifest schema does not match v7.5.8 handoff export manifest schema.",
                path="manifest.json",
            )
        )
    ...
    handoff_index = (
        _load_json(
            safe_files, "handoff_index.json", failures, "invalid_handoff_index_json"
        )
        if "handoff_index.json" in safe_files
        else {}
    )
    summary = (
        _load_json(
            ...
            "handoff_index_summary.json",
            failures,
            "invalid_handoff_summary_json",
        )
        if "handoff_index_summary.json" in safe_files
        else {}
    )
    report["recommended_action"] = (
        handoff_index.get("recommended_action") if handoff_index else None
    )
    report["verification_status"] = (
        handoff_index.get("verification_status") if handoff_index else None
    )
    report["certificate_status"] = (
        handoff_index.get("certificate_status") if handoff_index else None
    )
        ...
        if handoff_index.get("recommended_action") != summary.get("recommended_action"):
            failures.append(
                _finding(
                    ...
                    "Handoff summary recommended action differs from handoff index recommended action.",
                    path="handoff_index_summary.json",
                )
            )
        if handoff_index.get("verification_status") != summary.get(
            "verification_status"
        ):
            failures.append(
                _finding(
                    ...
                    "Handoff summary verification status differs from handoff index verification status.",
                    path="handoff_index_summary.json",
                )
            )
        if handoff_index.get("recommended_action") in {
            ...
            warnings.append(
                _finding(
                    ...
                    "Handoff export is structurally intact but not archive-ready.",
    ...
    if not safe_files.get("handoff_index.md", b"").strip():
        warnings.append(
            _finding(
                "warn",
                "empty_handoff_markdown",
                "Handoff Markdown is empty.",
                path="handoff_index.md",
            )
        )
...
def verify_handoff_export_bundle(bundle: dict[str, Any]) -> dict[str, Any]:
    return verify_handoff_export_files(
        build_handoff_export_bundle_files(deepcopy(bundle or {}))
    )
