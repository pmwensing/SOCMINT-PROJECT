...
OPERATOR_WORKFLOW_ACTION_RECEIPT_VERIFICATION_SCHEMA = (
    "socmint.operator_workflow_action_receipt_verification.v17_4"
)
VERSION = "v17.4.0"
...
def verify_operator_workflow_action_receipt(
    ...
    *,
    ...
    if not safe_receipt:
        blockers.append(
            _blocker("missing_action_receipt", "operator action receipt is missing")
        )

    if (
        safe_receipt
        and safe_receipt.get("schema") != OPERATOR_WORKFLOW_ACTION_RECEIPT_SCHEMA
    ):
        blockers.append(
            _blocker("receipt_schema_mismatch", "receipt schema does not match v17.3")
        )
    ...
    if safe_receipt and safe_receipt.get("receipt_sha256") != expected_hash:
        blockers.append(
            _blocker(
                ...
                "receipt_sha256 does not match canonical receipt payload",
            )
        )
    ...
    if safe_receipt and safe_receipt.get("action_receipt_id") != expected_receipt_id:
        blockers.append(
            _blocker(
                ...
                "action_receipt_id does not match canonical receipt payload",
            )
        )

    if safe_receipt and not _timestamp_present_and_valid(
        safe_receipt.get("recorded_at")
    ):
        blockers.append(
            _blocker(
                "invalid_recorded_at", "recorded_at timestamp is missing or invalid"
            )
        )
    ...
    if (
        expected_operator is not None
        and safe_receipt.get("operator") != expected_operator
    ):
        blockers.append(
            _blocker(
                "operator_mismatch",
                "receipt operator does not match authenticated operator",
            )
        )
    if expected_case_id is not None and safe_receipt.get("case_id") != expected_case_id:
        blockers.append(
            _blocker(
                "case_id_mismatch", "receipt case_id does not match requested case"
            )
        )
    ...
    if safe_result:
        comparisons = (
            ...
            (
                "confirmed",
                safe_result.get("confirmed") is True,
            ...
            (
                "state_change",
                safe_result.get("state_change") is True,
            ...
            (
                "blocker_count",
                int(safe_result.get("blocker_count") or 0),
        ...
        for field, expected, key in comparisons:
            if safe_receipt.get(field) != expected:
                blockers.append(
                    _blocker(key, f"receipt {field} does not match action result")
                )
        ...
        if safe_receipt.get("action_target") != expected_target:
            blockers.append(
                _blocker(
                    ...
                    "receipt action target does not match action plan",
                )
            )
        expected_plan_type = (
        ...
        if safe_receipt.get("action_plan_type") != expected_plan_type:
            blockers.append(
                _blocker(
                    "action_plan_type_mismatch",
                    "receipt action plan type does not match action result",
                )
            )
    ...
    return {
        ...
        "receipt_hash_valid": safe_receipt.get("receipt_sha256") == expected_hash
        if safe_receipt
        else False,
        "receipt_id_valid": safe_receipt.get("action_receipt_id") == expected_receipt_id
        if safe_receipt
        else False,
        "timestamp_valid": _timestamp_present_and_valid(safe_receipt.get("recorded_at"))
        if safe_receipt
        else False,
        "operator_consistent": not any(
            item["key"] in {"operator_missing", "operator_mismatch"}
...
def verify_operator_workflow_action_receipt_from_request(
    ...
    *,
    ...
    receipt = (
        safe_payload.get("action_receipt")
        if isinstance(safe_payload.get("action_receipt"), dict)
        else None
    )
    action_result = (
        safe_payload.get("action_result")
        if isinstance(safe_payload.get("action_result"), dict)
        else None
    )
