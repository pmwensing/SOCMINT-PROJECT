...
def _env_bool(name: str, default: str = "") -> bool:
    return os.environ.get(name, default).strip().lower() in {"1", "true", "yes", "on", "authorized"}
...
def run_social_analyzer(target: str, target_type: str, timeout: int = DEFAULT_TIMEOUT, allow_dry_run: bool = True) -> dict[str, Any]:
    ...
    try:
        ...
        payload = {
            ...
            "stderr": _text(exc.stderr) or f"social-analyzer timed out after {timeout}s",
            "started_at": started.isoformat(),
            "finished_at": finished.isoformat(),
        }
...
def _extract_findings(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for part in text.split():
        if part.startswith("http://") or part.startswith("https://"):
            ...
            if key not in seen:
                seen.add(key)
                findings.append({"type": "profile_url", "value": value, "source": "social-analyzer", "confidence": 0.78, "context": {"deep_enrichment": True}})
    return findings


def _walk(data: Any, findings: list[dict[str, Any]], seen: set[tuple[str, str]]) -> None:
    if isinstance(data, dict):
        url = data.get("url") or data.get("link") or data.get("profile") or data.get("profile_url")
        status = str(data.get("status") or data.get("found") or data.get("exists") or "").lower()
        ...
        if url and status not in {"false", "not found", "missing", "unknown"}:
            ...
            if key not in seen:
                seen.add(key)
                findings.append({"type": "profile_url", "value": str(url), "source": "social-analyzer", "confidence": confidence, "context": {"platform": site, "status": status, "deep_enrichment": True}})
        elif site and status in {"true", "found", "exists", "registered", "used", "valid"}:
            key = ("platform_presence", str(site).lower())
            if key not in seen:
                seen.add(key)
                findings.append({"type": "platform_presence", "value": str(site), "source": "social-analyzer", "confidence": confidence, "context": {"status": status, "deep_enrichment": True}})
