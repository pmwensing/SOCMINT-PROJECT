...
def quality_assessments() -> list[dict[str, Any]]:
    return [
        item
        for item in history()
        if item.get("event_type") == "collection_quality_assessed"
    ]
...
def contribution_reviews() -> list[dict[str, Any]]:
    return [
        item
        for item in history()
        if item.get("event_type") == "dossier_contribution_reviewed"
    ]
...
def find_assessment(assessment_id: str) -> dict[str, Any] | None:
    return next(
        (
            ...
            if item.get("quality_assessment_id") == assessment_id
        ),
        None,
    )
...
def latest_assessment_for_artifact(artifact_id: str) -> dict[str, Any] | None:
    matches = [
        item for item in quality_assessments() if item.get("artifact_id") == artifact_id
    ]
...
def latest_contribution_review(assessment_id: str) -> dict[str, Any] | None:
    matches = [
        item
        for item in contribution_reviews()
        if item.get("quality_assessment_id") == assessment_id
    ]
    return matches[-1] if matches else None
...
def _score_artifact(artifact: dict[str, Any]) -> tuple[int, list[dict[str, str]]]:
    ...
    if artifact.get("contract_binding_sha256") and artifact.get("acquisition_sha256"):
        ...
        findings.append(
            {"severity": "high", "key": "deterministic_provenance_binding_missing"}
        )
    if state == "accepted":
        score += 25
...
def assess_collection_quality(
    *,
    ...
    observation_ids = [
        item.get("observation_id") or item.get("artifact_event_id")
    ...
    event = {
        ...
        "version": VERSION,
        **content,
        "quality_assessment_id": f"quality-assessment-{digest[:24]}",
        "quality_assessment_sha256": digest,
        "dossier_mutated": False,
        "evidence_mutated": False,
        "connector_execution_performed": False,
    }
    result = _record(
        ACTIONS[0], actor, event["quality_assessment_id"], event, ip_address
    )
    return {
        **result,
        "status": "collection_quality_assessed",
        "next_action": "review_dossier_contribution",
    }
...
def review_dossier_contribution(
    *,
    ...
    if decision == "approved" and assessment.get("trust_tier") not in {
        "supported",
        "trusted",
    }:
    ...
    binding = {
        ...
        "quality_assessment_sha256": assessment.get("quality_assessment_sha256"),
        "artifact_id": assessment.get("artifact_id"),
        "case_id": assessment.get("case_id"),
        "entity_id": assessment.get("entity_id"),
    }
    content = {
        "event_type": "dossier_contribution_reviewed",
        "quality_assessment_id": quality_assessment_id,
        "artifact_id": assessment.get("artifact_id"),
        "case_id": assessment.get("case_id"),
        "entity_id": assessment.get("entity_id"),
        "decision": decision,
        "rationale": rationale,
        "quality_binding": binding,
        "quality_binding_sha256": _sha(binding),
    }
    digest = _sha(content)
    event = {
        "schema": SCHEMA,
        "version": VERSION,
        **content,
        "dossier_contribution_review_id": f"dossier-contribution-review-{digest[:24]}",
        "dossier_contribution_review_sha256": digest,
    ...
    result = _record(
        ACTIONS[1], actor, event["dossier_contribution_review_id"], event, ip_address
    )
    return {
        **result,
        "status": "dossier_contribution_reviewed",
        "next_action": "contribute_to_dossier_pipeline"
        if decision == "approved"
        else "resolve_quality_findings",
    }
