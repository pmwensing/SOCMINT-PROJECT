...
CASE_DELIVERY_RECOVERY_RESUME_OPERATIONS_SNAPSHOT_VERIFICATION_SCHEMA = (
    "socmint.case_delivery_recovery_resume_operations_snapshot_verification.v16_15"
)
VERSION = "v16.15.0"
...
def verify_case_delivery_recovery_resume_operations_snapshot(
    ...
    gate_check = deepcopy(
        continuation_gate_verification or {}
    ) or verify_case_delivery_recovery_continuation_gate(
    ...
    if not safe_snapshot:
        blockers.append(
            _blocker("missing_resume_snapshot", "resume operations snapshot is missing")
        )
    ...
    expected_payload_hash = (
        sha256_text(canonical_json(_snapshot_payload(safe_snapshot)))
        if safe_snapshot
        else None
    )
    if safe_snapshot and safe_snapshot.get("payload_sha256") != expected_payload_hash:
        blockers.append(
            _blocker(
                ...
                "resume snapshot payload hash does not match snapshot fields",
            )
        )
    ...
    expected_snapshot_id = (
        sha256_text(
            canonical_json(
                {
                    **_snapshot_payload(safe_snapshot),
                    "payload_sha256": safe_snapshot.get("payload_sha256"),
                }
            )
        )
        if safe_snapshot
    ...
    if (
        safe_snapshot
        and safe_snapshot.get("resume_snapshot_id") != expected_snapshot_id
    ):
        blockers.append(
            _blocker(
                "resume_snapshot_id_mismatch",
                "resume snapshot id does not match canonical snapshot payload",
            )
        )
    ...
    if safe_snapshot and safe_snapshot.get("queue_id") != safe_recovery.get("queue_id"):
        blockers.append(
            _blocker(
                "queue_id_mismatch",
                "resume snapshot queue_id does not match recovery queue",
            )
        )
    if safe_snapshot and safe_snapshot.get("case_id") != safe_recovery.get("case_id"):
        blockers.append(
            _blocker(
                "case_id_mismatch",
                "resume snapshot case_id does not match recovery queue",
            )
        )
    if safe_snapshot and safe_snapshot.get(
        "continuation_gate_id"
    ) != safe_continuation_gate.get("continuation_gate_id"):
        blockers.append(
            _blocker(
                "continuation_gate_id_mismatch",
                "resume snapshot continuation_gate_id does not match continuation gate",
            )
        )
    if (
        safe_snapshot
        and gate_check.get("continuation_gate_id")
        and safe_snapshot.get("continuation_gate_id")
        != gate_check.get("continuation_gate_id")
    ):
        blockers.append(
            _blocker(
                "continuation_gate_verification_id_mismatch",
                "resume snapshot continuation_gate_id does not match gate verification",
            )
        )
    if safe_snapshot and safe_snapshot.get(
        "continuation_gate_verification_status"
    ) != gate_check.get("status"):
        blockers.append(
            _blocker(
                "continuation_gate_verification_status_mismatch",
                "resume snapshot verification status does not match gate verification",
            )
        )
    if safe_snapshot and safe_snapshot.get("continuation_gate_verified") is not True:
        blockers.append(
            _blocker(
                "continuation_gate_not_verified",
                "resume snapshot continuation_gate_verified flag is not true",
            )
        )
    if safe_snapshot and safe_snapshot.get("gate_open") is not True:
        blockers.append(
            _blocker("gate_not_open", "resume snapshot gate_open flag is not true")
        )
    if safe_snapshot and safe_snapshot.get("safe_to_reenter_operations") is not True:
        blockers.append(
            _blocker(
                "not_safe_to_reenter_operations",
                "resume snapshot safe_to_reenter_operations flag is not true",
            )
        )
    if safe_snapshot and safe_snapshot.get("next_action") != NEXT_ACTION:
        blockers.append(
            _blocker(
                ...
                "resume snapshot next_action is not execute_delivery_operations",
            )
        )
    ...
    return {
        ...
        "continuation_gate_id": safe_snapshot.get("continuation_gate_id")
        or safe_continuation_gate.get("continuation_gate_id"),
        "status": status,
        "verified": not blockers,
        "safe_to_reenter_operations": not blockers,
        "next_action": NEXT_ACTION
        if not blockers
        else "resolve_recovery_resume_snapshot",
...
def verify_case_delivery_recovery_resume_operations_snapshot_from_request(
    ...
    if resume_snapshot is None:
        ...
        resume_snapshot = (
            snapshot_result.get("resume_snapshot")
            if isinstance(snapshot_result.get("resume_snapshot"), dict)
            else None
        )
