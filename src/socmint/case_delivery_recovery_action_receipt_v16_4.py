...
CASE_DELIVERY_RECOVERY_ACTION_RECEIPT_SCHEMA = (
    "socmint.case_delivery_recovery_action_receipt.v16_4"
)
CASE_DELIVERY_RECOVERY_ACTION_RECEIPT_RESULT_SCHEMA = (
    "socmint.case_delivery_recovery_action_receipt.v16_4.result"
)
VERSION = "v16.4.0"
...
def _normalized_status(action: dict[str, Any] | None) -> str:
    if not isinstance(action, dict):
        return "pending"
    status = (
        str(action.get("status") or action.get("action_status") or "pending")
        .strip()
        .lower()
    )
...
def _action_operator(action: dict[str, Any] | None, fallback: str | None) -> str:
    if (
        ...
        and isinstance(action.get("operator"), str)
        and action["operator"].strip()
    ):
...
def _find_action(
    recovery: dict[str, Any], actions: list[dict[str, Any]]
) -> dict[str, Any] | None:
...
def _receipt_rows(
    ...
    for sequence, recovery in enumerate(recoveries, start=1):
        ...
        rows.append(
            {**payload, "action_receipt_id": sha256_text(canonical_json(payload))}
        )
    return rows
...
def build_case_delivery_recovery_action_receipt(
    ...
    actions = (
        safe_payload.get("actions")
        if isinstance(safe_payload.get("actions"), list)
        else []
    )
    ...
    receipt_payload = {
        ...
        "completed_count": sum(
            1 for row in action_rows if row.get("completed") is True
        ),
        "pending_count": sum(
            1 for row in action_rows if row.get("completed") is not True
        ),
    ...
    return {
        ...
        else "complete_recovery_actions",
    }


def build_case_delivery_recovery_action_receipt_from_request(
