...
from .dossier_assertion_handoff_seal_v12_10_11 import (
    verify_dossier_assertion_handoff_seal,
)
...
def build_dossier_assertion_handoff_verification(
    handoff_bundle: dict[str, Any], handoff_seal: dict[str, Any]
) -> dict[str, Any]:
    seal_check = verify_dossier_assertion_handoff_seal(handoff_bundle, handoff_seal)
    counts_match = (
        int(handoff_bundle.get("total_count", 0) or 0)
        == int(handoff_seal.get("packet_count", 0) or 0)
        and int(handoff_bundle.get("ready_count", 0) or 0)
        == int(handoff_seal.get("ready_count", 0) or 0)
        and int(handoff_bundle.get("blocked_count", 0) or 0)
        == int(handoff_seal.get("blocked_count", 0) or 0)
    )
    schema_match = handoff_seal.get("sealed_payload_schema") == handoff_bundle.get(
        "schema"
    )
    checks = [
        ...
        {
            "name": "sealed_schema_matches_bundle",
...
def export_dossier_assertion_handoff_verification_report(
    ...
    if fmt in {"md", "markdown"}:
        ...
        return (
            "text/markdown",
            "dossier-assertion-handoff-verification.md",
            "\n".join(lines),
        )
    return (
        "application/json",
        "dossier-assertion-handoff-verification.json",
