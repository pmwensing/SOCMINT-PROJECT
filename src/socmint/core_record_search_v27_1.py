...
FIELD_CATALOG = {
    ...
    "evidence": ("evidence_id", "title", "description", "source", "citation", "hash", "filename"),
    "finding": ("finding_id", "finding", "title", "summary", "category", "status", "confidence"),
}
...
def _type(action: str, details: dict[str, Any]) -> str | None:
    ...
    if "evidence" in text or "citation" in text or "source" in text:
        return "evidence"
    if "entity" in text or any(key in details for key in ("entity_id", "entity_type", "aliases")):
        return "entity"
    return None
...
def _field_values(record_type: str, case_id: str, details: dict[str, Any]) -> dict[str, str]:
...
def _match(query: str, fields: dict[str, str]) -> tuple[float, list[dict[str, Any]], list[str]]:
    ...
    for field, raw in fields.items():
        ...
        partial_hits = sorted({token for token in tokens if token in value_l and token not in token_hits})
        ...
        if field.endswith("_id") or field == "case_id":
            ...
            weight = 1.5
        field_score = (100 if exact else 0) + (45 if phrase else 0) + 14 * len(token_hits) + 7 * len(partial_hits)
        field_score *= weight
        score += field_score
    ...
    return round(score, 3), sorted(matches, key=lambda item: item["field_score"], reverse=True), sorted(matched_tokens)


def _preview(record_type: str, fields: dict[str, str], matches: list[dict[str, Any]]) -> dict[str, Any]:
    preferred = {
        "case": ("title", "case_id", "stage", "status"),
        "entity": ("name", "display_value", "entity_id", "entity_type", "aliases"),
        "evidence": ("title", "evidence_id", "description", "source", "filename", "hash"),
        "finding": ("finding", "title", "finding_id", "summary", "category", "status", "confidence"),
    }[record_type]
    selected = []
    seen = set()
    for match in matches:
        ...
        if field not in seen:
            selected.append({"field": field, "value": fields.get(field, "")[:300], "matched": True})
            seen.add(field)
    for field in preferred:
        if field in fields and field not in seen and len(selected) < 6:
            selected.append({"field": field, "value": str(fields[field])[:300], "matched": False})
            seen.add(field)
    return {"fields": selected, "field_count": len(fields), "matched_field_count": len(matches)}
...
def _links(case_id: str, record_type: str) -> dict[str, str]:
    ...
    return {"primary": links[record_type], "case": links["case"], "evidence": links["evidence"]}
...
def build_core_record_search(
    ...
    *,
    ...
    requested_types = {item for item in map(str, record_types or []) if item in CORE_TYPES}
    ...
    for record in source:
        ...
        if query and not field_matches:
            continue
        binding = {"source_record_id": record.get("record_id"), "source_action": record.get("action"), "case_id": case_id}
        results.append({
    ...
    results.sort(key=lambda item: (item["score"], item.get("occurred_at") or "", item["result_id"]), reverse=True)
    safe_limit = max(1, min(int(limit or 100), 500))
    results = results[:safe_limit]
    applied_filters = {
        "record_types": sorted(requested_types), "case_ids": sorted(requested_cases),
        "actors": sorted(requested_actors), "statuses": sorted(requested_statuses), "limit": safe_limit,
    }
    result_counts = Counter(item["record_type"] for item in results)
    facets = {name: dict(sorted(counter.items())) for name, counter in available_facets.items()}
    core = {"query": query, "applied_filters": applied_filters, "results": results}
    return {
        "schema": SCHEMA, "version": VERSION, "status": "ready", "query": query,
        "record_types": list(CORE_TYPES), "field_catalog": FIELD_CATALOG,
        "applied_filters": applied_filters, "facets": facets, "results": results,
        "result_count": len(results), "record_type_counts": dict(sorted(result_counts.items())),
        "visible_case_ids": sorted({item["case_id"] for item in results}),
        "search_sha256": _sha(core),
        "access_scope": {"mode": "restricted" if visible is not None else "all_visible_cases", "allowed_case_ids": sorted(visible) if visible is not None else None},
        ...
        "next_action": "refine_core_record_search" if query else "enter_case_entity_evidence_or_finding_query",
    }
